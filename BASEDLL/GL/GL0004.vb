...
Public Class GL0004DestinationList
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property TYPE() As LC_DEST_TYPE
    ''' <summary>
    ''' 会社コード
    ''' </summary>
    ''' <value></value>
    ...
    Public Overrides Sub getList()
        ...
        Using SQLcon = sm.getConnection
            ...
            Select Case TYPE
                Case LC_DEST_TYPE.ALL,
                     LC_DEST_TYPE.EXCEPT_JXCOSMO,
                     LC_DEST_TYPE.JXCOSMO
                    getDistinationList(SQLcon)

                Case LC_DEST_TYPE.WITHTERM
    ...
    Protected Sub getDistinationList(ByVal SQLcon As SqlConnection)
        ...
        Try
            ...
            '〇抽出条件追加
            If TYPE = LC_DEST_TYPE.EXCEPT_JXCOSMO Then
                'JX/COSMO除外
                SQLStr = SQLStr _
                        & "     and substring(A.TODOKECODE,1,2)  <> 'JX'  " _
                        & "     and substring(A.TODOKECODE,1,5)  <> 'COSMO'  "
            ElseIf TYPE = LC_DEST_TYPE.JXCOSMO Then
                'JX/COSMOのみ
                SQLStr = SQLStr _
                        & "     and (substring(A.TODOKECODE,1,2)  = 'JX'  " _
                        & "      or  substring(A.TODOKECODE,1,5)  = 'COSMO')  "
            End If
            ...
            Select Case DEFAULT_SORT
                Case C_DEFAULT_SORT.CODE
                    SQLStr = SQLStr & " ORDER BY CODE, NAMES, TSEQ "
                Case C_DEFAULT_SORT.NAMES
                    SQLStr = SQLStr & " ORDER BY NAMES, CODE, TSEQ "
                Case C_DEFAULT_SORT.SEQ, String.Empty
                    SQLStr = SQLStr & " ORDER BY TSEQ, CODE, NAMES "
                Case Else
            End Select

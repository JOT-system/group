...
''' </summary>
''' <remarks></remarks>
Public Structure CS0030REPORTtbl
    ...
    Public Sub CS0030REPORTtbl()
        ...
        '■APSRVname
        If (CS0050SESSION.APSV_ID = Nothing Or CS0050SESSION.APSV_ID = "") Then
        ...
        If Not isNormal(CS0021UPROFXLS.ERR) Then
            ERR = C_MESSAGE_NO.DLL_IF_ERROR
        ...
        For i As Integer = 0 To CS0021UPROFXLS.STRUCT.Count - 1
        ...
        For i As Integer = 0 To TBLDATA.Rows.Count - 1
            ...
            For CNT As Integer = 0 To WW_I_TITOLKBN.Count - 1
                If WW_I_SORT(CNT) <> 0 And WW_I_TITOLKBN(CNT) = "I" Then
                    If WW_RowKey = "" Then
            ...
            For CNT As Integer = 0 To WW_R_TITOLKBN.Count - 1
                If WW_R_TITOLKBN(CNT) = "I_DataKey" Then
                    If WW_CellKey = "" Then
        ...
        Try
            ...
            If CS0021UPROFXLS.EXCELFILE = "" Or File.Exists(CS0050SESSION.UPLOAD_PATH & "\PRINTFORMAT\" & CS0050SESSION.USERID & "\" & MAPID & "\" & CS0021UPROFXLS.EXCELFILE) = False Then
                If File.Exists(CS0050SESSION.UPLOAD_PATH & "\PRINTFORMAT\Default\" & MAPID & "\" & CS0021UPROFXLS.EXCELFILE) = True Then
                    '既存のファイルを開く場合
                    W_ExcelBook = W_ExcelBooks.Open(CS0050SESSION.UPLOAD_PATH & "\PRINTFORMAT\Default\" & MAPID & "\" & CS0021UPROFXLS.EXCELFILE)
            ...
            Else
                '既存のファイルを開く場合
                W_ExcelBook = W_ExcelBooks.Open(CS0050SESSION.UPLOAD_PATH & "\PRINTFORMAT\" & CS0050SESSION.USERID & "\" & MAPID & "\" & CS0021UPROFXLS.EXCELFILE)
                W_ExcelSheets = W_ExcelBook.Worksheets
                'シート名の取得
                Dim xlElement As Excel.Worksheet = Nothing
                Dim W_FIND As String = "OFF"
                ...
                If CS0021UPROFXLS.TITOLKBN(i) = "T" And CS0021UPROFXLS.EFFECT(i) = "Y" And CS0021UPROFXLS.POSIX(i) > 0 And CS0021UPROFXLS.POSIY(i) > 0 Then
                    Select Case CS0021UPROFXLS.FIELD(i)
                        Case "EXCELTITOL"                'CS0021UPROFXLSパラメータ(FIELDNAME)をセット
                            ...
                            WW_HENSYUrange(CS0021UPROFXLS.POSIY(i), CS0021UPROFXLS.POSIX(i)) = CS0021UPROFXLS.FIELDNAME(i)
                        Case "REPORTID"                  'CS0021UPROFXLSパラメータ(REPORTID)をセット
                            ...
                            WW_HENSYUrange(CS0021UPROFXLS.POSIY(i), CS0021UPROFXLS.POSIX(i)) = "ID:" & CS0021UPROFXLS.REPORTID
                        Case Else                        'Tableの1行目の該当項目値をセット
                            Try
                                ...
                                WW_HENSYUrange(CS0021UPROFXLS.POSIY(i), CS0021UPROFXLS.POSIX(i)) = TBLDATA.Rows(0).Item(CS0021UPROFXLS.FIELD(i)).ToString
                            Catch ex As Exception
                                '項目名が無い場合、無視
                            End Try
                    End Select
        ...
        Try
            ...
            '　明細タイトル範囲指定
            WW_Cells = W_ExcelSheet.Cells
            WW_STARTpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART, 1), Excel.Range)         '指定された行開始のA列
            WW_ENDpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART + CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_I_X_MAX), Excel.Range)       '指定された行+明細行
        ...
        If WW_CELL_KEY.Count > 0 Then
            Try
                ...
                For i As Integer = 0 To WW_R_TITOLKBN.Count - 1
                    If WW_R_TITOLKBN(i) = "I_DataKey" And WW_R_POSIY(i) <> 0 And WW_R_POSIX(i) <> 0 Then
                ...
                If WW_DataKeyPrint = "Print OK" Then
                    WW_Range_str = "MaxX:" & CS0021UPROFXLS.POSI_R_Y_MAX.ToString & "_MaxY:" & CS0021UPROFXLS.POSI_R_X_MAX.ToString
                    'Dim WW_HENSYUrange(0, WW_CELL_KEY.Count * CS0021UPROFXLS.POSI_R_X_MAX - 1) As Object                                      '行編集領域　　※開始位置(0,0) …　object

                    '　明細タイトル範囲指定
                    WW_Cells = W_ExcelSheet.Cells
                    WW_STARTpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART, CS0021UPROFXLS.POSI_I_X_MAX + 1), Excel.Range)       '指定された行開始のA列
                    WW_ENDpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART, WW_CELL_KEY.Count * CS0021UPROFXLS.POSI_R_X_MAX + CS0021UPROFXLS.POSI_I_X_MAX), Excel.Range)       '指定された行+明細行
                    WW_EXCELrange = W_ExcelSheet.Range(WW_STARTpoint, WW_ENDpoint)                                                          'データの入力セル範囲

                    '　明細タイトル設定
                    For i As Integer = 0 To WW_CELL_KEY.Count - 1
                        WW_HENSYUrange(0, i * CS0021UPROFXLS.POSI_R_X_MAX) = WW_CELL_KEY(i)
                    Next
            ...
            Try
                ...
                For wLoopCNT As Integer = 0 To (Math.Ceiling(WW_View.Count / 3000) - 1)
                    ...
                    If wLoopCNT = (Math.Ceiling(WW_View.Count / 3000) - 1) Then
                        ...
                        ReDim WW_HENSYUrange((WW_View.Count Mod 3000) * CS0021UPROFXLS.POSI_I_Y_MAX - 1, CS0021UPROFXLS.POSI_I_X_MAX - 1)       '行編集領域　　※開始位置(0,0) …　object

                        '編集処理(WW_View-->WW_HENSYUrange)
                        For i As Integer = (wLoopCNT * 3000) To WW_View.Count - 1
                            For CNT As Integer = 0 To WW_I_TITOLKBN.Count - 1
                                Try
                                    WW_HENSYUrange((i - wLoopCNT * 3000) * CS0021UPROFXLS.POSI_I_Y_MAX + WW_I_POSIY(CNT) - 1, WW_I_POSIX(CNT) - 1) = WW_View(i)(WW_I_FIELD(CNT)).ToString
                                Catch ex As Exception
                        ...
                        '〇Excel貼付
                        WW_Cells = W_ExcelSheet.Cells
                        WW_STARTpoint = DirectCast(WW_Cells.Item(((wLoopCNT * 3000) + 1) * CS0021UPROFXLS.POSI_I_Y_MAX + CS0021UPROFXLS.POSISTART, 1), Excel.Range)
                        WW_ENDpoint = DirectCast(WW_Cells.Item(((wLoopCNT * 3000 + WW_View.Count Mod 3000) + 1) * CS0021UPROFXLS.POSI_I_Y_MAX + CS0021UPROFXLS.POSISTART - 1, CS0021UPROFXLS.POSI_I_X_MAX), Excel.Range)
                    ...
                    Else
                        '〇出力編集
                        '3000件の編集Work領域定義         ※開始位置(0,0)
                        ReDim WW_HENSYUrange(3000 * CS0021UPROFXLS.POSI_I_Y_MAX - 1, CS0021UPROFXLS.POSI_I_X_MAX - 1)       '行編集領域　　※開始位置(0,0) …　object
                        ...
                        WW_Cells = W_ExcelSheet.Cells
                        WW_STARTpoint = DirectCast(WW_Cells.Item(((wLoopCNT) * 3000 + 1) * CS0021UPROFXLS.POSI_I_Y_MAX + CS0021UPROFXLS.POSISTART, 1), Excel.Range)
                        WW_ENDpoint = DirectCast(WW_Cells.Item(((wLoopCNT + 1) * 3000 + 1) * CS0021UPROFXLS.POSI_I_Y_MAX + CS0021UPROFXLS.POSISTART - 1, CS0021UPROFXLS.POSI_I_X_MAX), Excel.Range)
                        WW_EXCELrange = W_ExcelSheet.Range(WW_STARTpoint, WW_ENDpoint)
        ...
        Else
            ...
            Try
                'ERR表示用文字編集
                WW_Range_str = "MaxY:" & CS0021UPROFXLS.POSI_R_Y_MAX.ToString & "_MaxX:" & CS0021UPROFXLS.POSI_I_X_MAX.ToString
                ...
                ReDim WW_HENSYUrange((TBLDATA.Rows.Count + 1) * Math.Max(CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_R_Y_MAX) - 1 + 1, _
                                   CS0021UPROFXLS.POSI_I_X_MAX + (CS0021UPROFXLS.POSI_R_X_MAX * WW_CELL_KEY.Count) - 1 + 1)

                'Excel操作対象領域定義
                WW_Cells = W_ExcelSheet.Cells
                '・行：開始＋見出、列：１
                WW_STARTpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART + CS0021UPROFXLS.POSI_I_Y_MAX, 1), Excel.Range)
                ...
                WW_ENDpoint = DirectCast(WW_Cells.Item(CS0021UPROFXLS.POSISTART + CS0021UPROFXLS.POSI_I_Y_MAX + (TBLDATA.Rows.Count + 1) * Math.Max(CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_R_Y_MAX), _
                                                      CS0021UPROFXLS.POSI_I_X_MAX + CS0021UPROFXLS.POSI_R_X_MAX * WW_CELL_KEY.Count + 1), Excel.Range)
                'データの入力セル範囲
                WW_EXCELrange = W_ExcelSheet.Range(WW_STARTpoint, WW_ENDpoint)
                ...
                For i As Integer = 0 To WW_View.Count - 1
                    ...
                    For CNT As Integer = 0 To WW_I_TITOLKBN.Count - 1
                        Try
                            ...
                            WW_HENSYUrange(WW_LineCNT * Math.Max(CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_R_Y_MAX) + WW_I_POSIY(CNT) - 1, WW_I_POSIX(CNT) - 1) = WW_View(i)(WW_I_FIELD(CNT)).ToString
                        Catch ex As Exception
                            '項目名が無い場合、無視
                        End Try
                    Next
                    ...
                    If WW_View(i)("CELLNO") > (WW_CELL_KEY.Count - 1) Then
                        ...
                        WW_HENSYUrange(WW_LineCNT * Math.Max(CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_R_Y_MAX), CS0021UPROFXLS.POSI_I_X_MAX + WW_CELL_KEY.Count * CS0021UPROFXLS.POSI_R_X_MAX) = "★表示出来ないデータ有(該当列無)"
                    Else
                        'I_Dataは表示。I_DataKeyは表示しない(表題に存在する為)。
                        For CNT As Integer = 0 To WW_R_TITOLKBN.Count - 1
                            If (WW_R_TITOLKBN(CNT) = "I_Data") And WW_R_POSIY(CNT) > 0 And WW_R_POSIX(CNT) > 0 Then
                                Try
                                    ...
                                    WW_HENSYUrange(WW_LineCNT * Math.Max(CS0021UPROFXLS.POSI_I_Y_MAX, CS0021UPROFXLS.POSI_R_Y_MAX) + WW_R_POSIY(CNT) - 1, CS0021UPROFXLS.POSI_I_X_MAX + WW_View(i)("CELLNO") * CS0021UPROFXLS.POSI_R_X_MAX + WW_R_POSIX(CNT) - 1) = WW_View(i)(WW_R_FIELD(CNT)).ToString
                                Catch ex As Exception
                                    '項目名が無い場合、無視
                                End Try
                            End If
            ...
            Else
                For i As Integer = 0 To CS0021UPROFXLS.TITOLKBN.Count - 1
                    If CS0021UPROFXLS.POSIX(i) <> 0 And CS0021UPROFXLS.WIDTH(i) <> 0 Then
                        ...
                        WW_ColumnsA = DirectCast(WW_Columns(CS0021UPROFXLS.POSIX(i)), Excel.Range)
                        WW_ColumnsA.ColumnWidth = CS0021UPROFXLS.WIDTH(i)
                ...
                With WW_PageSetup
                    ...
                    .PrintTitleRows = "$1:$" & (CS0021UPROFXLS.POSISTART + CS0021UPROFXLS.POSI_I_Y_MAX - 1).ToString    'ページタイトル固定
                End With
        ...
        Try
            ...
            WW_Dir = CS0050SESSION.UPLOAD_PATH & "\PRINTWORK\" & CS0050SESSION.APSV_ID
            If Directory.Exists(WW_Dir) Then
            Else
                Directory.CreateDirectory(WW_Dir)
            End If
        ...
        Try
            Select Case FILEtyp
                Case "pdf"
                    FILEpath = CS0050SESSION.UPLOAD_PATH & "\" & "PRINTWORK" & "\" & CS0050SESSION.APSV_ID & "\" & WW_datetime & ".pdf"
                    'URL = "http://" & Dns.GetHostName & "/PRINT/" & WW_Term & "/" & WW_datetime & ".pdf"
                    URL = HttpContext.Current.Request.Url.Scheme & "://" & HttpContext.Current.Request.Url.Host & "/PRINT/" & CS0050SESSION.APSV_ID & "/" & WW_datetime & ".pdf"
                ...
                Case "csv"
                    FILEpath = CS0050SESSION.UPLOAD_PATH & "\PRINTWORK\" & CS0050SESSION.APSV_ID & "\" & WW_datetime & ".CSV"
                    'URL = "http://" & Dns.GetHostName & "/PRINT/" & WW_Term & "/" & WW_datetime & ".CSV"
                    URL = HttpContext.Current.Request.Url.Scheme & "://" & HttpContext.Current.Request.Url.Host & "/PRINT/" & CS0050SESSION.APSV_ID & "/" & WW_datetime & ".CSV"
                    W_ExcelApp.DisplayAlerts = False
                    W_ExcelSheet.SaveAs(Filename:=FILEpath, FileFormat:=Excel.XlFileFormat.xlCSV)
                Case "xls"
                    FILEpath = CS0050SESSION.UPLOAD_PATH & "\PRINTWORK\" & CS0050SESSION.APSV_ID & "\" & WW_datetime & ".XLS"
                    'URL = "http://" & Dns.GetHostName & "/PRINT/" & WW_Term & "/" & WW_datetime & ".XLS"
                    URL = HttpContext.Current.Request.Url.Scheme & "://" & HttpContext.Current.Request.Url.Host & "/PRINT/" & CS0050SESSION.APSV_ID & "/" & WW_datetime & ".XLS"
                    W_ExcelBook.SaveAs(FILEpath)
                Case "xlsx"
                    FILEpath = CS0050SESSION.UPLOAD_PATH & "\PRINTWORK\" & CS0050SESSION.APSV_ID & "\" & WW_datetime & ".XLSX"
                    'URL = "http://" & Dns.GetHostName & "/PRINT/" & WW_Term & "/" & WW_datetime & ".XLSX"
                    URL = HttpContext.Current.Request.Url.Scheme & "://" & HttpContext.Current.Request.Url.Host & "/PRINT/" & CS0050SESSION.APSV_ID & "/" & WW_datetime & ".XLSX"
                    W_ExcelBook.SaveAs(FILEpath)
                Case "xlsm"
                    FILEpath = CS0050SESSION.UPLOAD_PATH & "\PRINTWORK\" & CS0050SESSION.APSV_ID & "\" & WW_datetime & ".XLSM"
                    'URL = "http://" & Dns.GetHostName & "/PRINT/" & WW_Term & "/" & WW_datetime & ".XLSM"
                    URL = HttpContext.Current.Request.Url.Scheme & "://" & HttpContext.Current.Request.Url.Host & "/PRINT/" & CS0050SESSION.APSV_ID & "/" & WW_datetime & ".XLSM"
                    'W_ExcelBook.SaveAs(FILEpath)

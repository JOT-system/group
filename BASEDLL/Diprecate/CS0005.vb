...
''' File格納ディレクトリ取得
''' </summary>
''' <remarks>INIファイルからEXCEL用ディレクトリのみを取得する</remarks>
Public Structure CS0005FILEdir

    ''' <summary>
    ''' File格納ディレクトリ
    ''' </summary>
    ''' <value></value>
    ''' <returns>File格納ディレクトリ</returns>
    ''' <remarks></remarks>
    ...
    Public Sub CS0005FILEDIRget()
        ...
        Try
            ...
            FILEDIRString = ""
            'File内容の画面退避格納Dir文字情報をすべて読み込む
            While (Not sr.EndOfStream)
                FILEDIRStringBuf = sr.ReadLine().Replace(vbTab, "")
                '開始キーワード(<File directory>)～終了キーワード(</File directory>)間に含まれる文字列を取得
                ...
                End If
                '終了キーワード(</File directory>)が出現したら、不要文字を取り除く

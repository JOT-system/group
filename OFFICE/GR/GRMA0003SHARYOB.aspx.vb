...
Public Class GRMA0003SHARYOB
        ...
        For Each XLSTBLrow As DataRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            If WW_COLUMNS.IndexOf("CAMPCODE") >= 0 AndAlso
               ...
               WW_COLUMNS.IndexOf("TSHABAN") >= 0 Then
                ...
                For Each MA0003row As DataRow In MA0003tbl.Rows
                    If XLSTBLrow("CAMPCODE").ToString.PadLeft(2, "0") = MA0003row("CAMPCODE") AndAlso
    ...
    Protected Sub PDF_Readonly()
        ...
        If Directory.Exists(WW_Dir_Temp) Then
            ...
            For Each tempFile As String In WW_UPfiles
                Dim file = New FileInfo(tempFile)
                Dim parseName = file.Name.Split("_")
                Dim fileShaban = parseName(0)
                Dim fileSharyoType = fileShaban.Substring(0, 1)
                Dim fileTShaban = fileShaban.Substring(1)
                Dim fileKbn = parseName(parseName.Count - 1)
                Dim fileNendo = String.Empty
                If parseName.Count > 2 Then
                    '年度あり
                    fileNendo = parseName(1)
                End If

                '統一車番で抽出
                If (WF_Rep11_PDFselect.SelectedValue = "01" AndAlso fileShaban = (WF_SHARYOTYPE.Text & WF_TSHABAN.Text)) Then
                    '書類種類で抽出
                    If fileKbn = WF_Rep11_PDFselect.SelectedValue Then
                        '年度による抽出
                        If WF_Rep11_PDFselect.SelectedValue = "01" Then
                            '01:車検証 … 年度アリフォルダ構成
                            If Val(fileNendo) >= Val(WW_nendoS) AndAlso Val(fileNendo) <= Val(WW_nendoE) Then
                                WW_PDFFile = CS0050Session.PDF_PATH & "\MA0004_SHARYOC"
                                WW_PDFFile = WW_PDFFile & "\" & WF_SHARYOTYPE.Text & WF_TSHABAN.Text & "_" & fileNendo & "_" & WF_Rep11_PDFselect.SelectedValue
            ...
            If isNormal(WW_CS0024FCHECKERR) Then
                ...
                Else
                    ...
                    If Not isNormal(WW_RTN_SW) Then
                        ...
                        WW_CheckMES2 = " マスタに存在しません。(" & MA0003INProw("CAMPCODE") & ")"
            ...
            If isNormal(WW_CS0024FCHECKERR) Then
                ...
                Else
                    ...
                    If Not isNormal(WW_RTN_SW) Then
                        ...
                        WW_CheckMES2 = " 削除フラグにエラーがあります。(" & MA0003INProw("DELFLG") & ")"
                        WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0003INProw)
                        WW_LINEERR_SW = "ERR"
                        O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
                    End If
        ...
        For i As Integer = 0 To MA0003INPtbl.Rows.Count - 1
            ...
            If MA0003INProw("OPERATION") <> C_LIST_OPERATION_CODE.ERRORED Then
                ...
                For Each MA0003row As DataRow In MA0003tbl.Rows
                    ...
                    End If
                Next
                If I_EXCEL = "EXCEL" AndAlso MA0003INProw("OPERATION") = "Insert" Then
                    If MA0003INProw("TSHABAN") <> "" Then
                        Dim WW_CheckMES1 As String = "・更新できないレコード(該当なしエラー)です。"
                        Dim WW_CheckMES2 As String = " 該当する車番が存在しません。(新規の場合、統一車番を空白にして下さい)"
                        WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0003INProw)
                        MA0003INProw("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
    ...
    Protected Sub WW_ItemCheck(ByVal I_FIELD As String, ByVal I_ITEMNM As String, ByRef O_VALUE As String, ByVal MA0003INProw As DataRow, ByRef WW_LINEERR_SW As String, ByRef O_RTN As String)
        ...
        If Not isNormal(WW_CS0024FCHECKERR) Then
            WW_CheckMES1 = "・更新できないレコード(" & I_ITEMNM & "エラー)です。"
            WW_CheckMES2 = WW_CS0024FCHECKREPORT
            WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0003INProw)
            WW_LINEERR_SW = "ERR"
            O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
        ...
        If isNormal(WW_CS0024FCHECKERR) AndAlso WW_FIELD <> "" Then
            ...
            If Not isNormal(WW_RTN_SW) Then
                WW_CheckMES1 = "・更新できないレコード(" & I_ITEMNM & "エラー)です。"
                WW_CheckMES2 = "マスタに存在しません。(" & WW_FIELD & ")"
                WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0003INProw)
                WW_LINEERR_SW = "ERR"
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            End If

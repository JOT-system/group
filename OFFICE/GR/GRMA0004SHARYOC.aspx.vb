...
Public Class GRMA0004SHARYOC
        ...
        Else
            Select Case WF_FIELD_REP.Value
                Case "BASERDATE", "OTNKINSYMD", "HPRSINSIYMD", "CHEMINSYMD", "LICNYMD", "TAXLINSYMD", "TAXLINSYMD", "OTNKTINSYMD", "OTNKTINSNYMD", "HPRSJINSYMD", "HPRSINSYMD", "HPRSINSNYMD", "CHEMTINSYMD", "CHEMTINSNYMD"
                    ...
                    WW_SelectTEXT = ""
                Case "LICNPLTNO1"
                    '登録番号（陸運局）はFIXVALUEの名称を設定
                    WW_SelectValue = WW_SelectTEXT
                    WW_SelectTEXT = ""
            End Select
        ...
        For Each XLSTBLrow As DataRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            If WW_COLUMNS.IndexOf("CAMPCODE") >= 0 AndAlso
               ...
               WW_COLUMNS.IndexOf("TSHABAN") >= 0 Then
                ...
                For Each MA0004row As DataRow In MA0004tbl.Rows
                    If XLSTBLrow("CAMPCODE").ToString.PadLeft(2, "0") = MA0004row("CAMPCODE") AndAlso
    ...
    Protected Sub PDF_reset()
        ...
        'フォルダ作成処理
        Dim WW_DirIN As String() = Directory.GetDirectories(CS0050Session.PDF_PATH & "\MA0004_SHARYOC", "*", SearchOption.AllDirectories)
        Dim WW_DirOUT As String = ""

        For Each tempFile As String In WW_DirIN

            WW_DirOUT = tempFile
            WW_DirOUT = WW_DirOUT.Replace(CS0050Session.PDF_PATH & "\MA0004_SHARYOC\", "")

            If Right(WW_DirOUT, 2) = "01" Then
                WW_DirOUT = CS0050Session.PDF_PATH & "\MA0004_SHARYOC_new\" & WW_DirOUT
            Else
                WW_DirOUT = CS0050Session.PDF_PATH & "\MA0004_SHARYOC_new\" & Mid(WW_DirOUT, 1, 8) & Right(WW_DirOUT, 2)
            End If
        ...
        'ファイルコピー処理
        Dim WW_filesIN As String() = Directory.GetFiles(CS0050Session.PDF_PATH & "\MA0004_SHARYOC", "*", SearchOption.AllDirectories)
        ...
        For Each tempFile As String In WW_filesIN
            ...
            Else
                WW_filesOUT = tempFile
                WW_filesOUT = WW_filesOUT.Replace(CS0050Session.PDF_PATH & "\MA0004_SHARYOC\", "")
                ...
                If Mid(WW_filesOUT, 12, 2) = "01" Then
                    WW_filesOUT = CS0050Session.PDF_PATH & "\MA0004_SHARYOC_new\" & WW_filesOUT
                Else
                    WW_filesOUT = CS0050Session.PDF_PATH & "\MA0004_SHARYOC_new\" & WW_SYARYO & "_" & WW_KBN & "\" & WW_NENDO & "_" & WW_PDF
                End If
            ...
            If isNormal(WW_CS0024FCHECKERR) Then
                ...
                Else
                    ...
                    If Not isNormal(WW_RTN_SW) Then
                        ...
                        WW_CheckMES2 = " マスタに存在しません。(" & MA0004INProw("CAMPCODE") & ")"
            ...
            If isNormal(WW_CS0024FCHECKERR) Then
                ...
                Else
                    ...
                    If Not isNormal(WW_RTN_SW) Then
                        ...
                        WW_CheckMES2 = " マスタに存在しません。(" & MA0004INProw("SHARYOTYPE") & ")"
                        WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0004INProw)
                        WW_LINEERR_SW = "ERR"
                        O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
                    End If
            ...
            If isNormal(WW_CS0024FCHECKERR) Then
                ...
                Else
                    ...
                    If Not isNormal(WW_RTN_SW) Then
                        ...
                        WW_CheckMES2 = " マスタに存在しません。(" & MA0004INProw("DELFLG") & ")"
                        WW_CheckERR(WW_CheckMES1, WW_CheckMES2, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, MA0004INProw)
                        WW_LINEERR_SW = "ERR"
                        O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
                    End If

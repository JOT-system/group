...
Public Class GRT00006EXPORT
            ...
            If IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    Select Case WF_ButtonClick.Value
                        ...
                        Case "WF_ButtonPutPlan"                 '光英送信(計画)
                            WF_ButtonPutPlan_Click()
                        Case "WF_ButtonPutShipper"              '光英送信(元売)
                            WF_ButtonPutShipper_Click()
    ...
    Protected Sub DisplayGrid()
        ...
        If WW_TBLview.ToTable IsNot Nothing AndAlso WW_TBLview.ToTable.Rows.Count > 0 Then
            ...
            'JXオーダー時はチェック外させない
            Dim targetCheckBoxLineCnt2 = (From dr As DataRow In WW_TBLview.ToTable
                                          Where Convert.ToString(dr.Item("OPERATION")) <> "" AndAlso
                                                Convert.ToString(dr.Item("JXORDERID")) <> ""
                                          Select Convert.ToInt32(dr.Item("LINECNT")))
            For Each lineCnt As Integer In targetCheckBoxLineCnt2
                Dim chkObjId As String = "chk" & Me.pnlListArea.ID & "OPERATION" & lineCnt.ToString
                Dim tmpObj As Control = Me.pnlListArea.FindControl(chkObjId)
                If Not IsNothing(tmpObj) Then
                    Dim chkObj As CheckBox = DirectCast(tmpObj, CheckBox)
                    chkObj.InputAttributes.Add("onclick", "return false;")
                End If
            Next
    ...
    Protected Sub WF_ButtonALLCANCEL_Click()
        ...
        For Each row In T00006tbl.Rows
            'JXオーダーはチェック外せない
            If row("JXORDERID") <> "" Then Continue For
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub WF_ButtonPutPlan_Click()
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub WF_ButtonPutShipper_Click()
    ...
    Protected Sub EditFileKOUEI(ByRef koueiOrder As Dictionary(Of String, GRW0001KOUEIORDER.KOUEI_ORDER),
        ...
        '○選択データかつJXオーダーが編集対象
        ' ※但し、編集はしないが送信は行う（受信分は全て送り返す）
        Dim tbl = T00006tbl.AsEnumerable.
                            Where(Function(x) x.Item("OPERATION") = "on" AndAlso
                            Not String.IsNullOrEmpty(x.Item("JXORDERID")))

        For Each T00006row As DataRow In tbl
            ...
            '出社指定時刻設定
            If Not String.IsNullOrEmpty(T00006row("STTIME")) Then
                order.SetSTTime(T00006row("STTIME"))
            End If
        ...
        ' 出社指定時刻同一乗務員反映
        '**********************
        '始業レコード、乗務員コード、指定出勤分（出社時刻）が設定されている場合に
        '同一出庫日で乗務員が同じ始業レコードに指定出勤分を反映する
        Dim staffGroup = koueiOrder.Where(Function(x) x.Value.TRIPSEQ = GRW0001KOUEIORDER.TRIPSEQ_TYPE.START AndAlso
                                                      x.Value.STAFFCODE <> "").
                                    GroupBy(Function(x) New With {Key x.Value.KIJUNDATE, Key x.Value.STAFFCODE})
        For Each trips In staffGroup
            '基準日・乗務員が同一のうち、最早な出社指定時刻を取得
            Dim base = trips.Where(Function(x) x.Value.STTIME <> "")
            Dim targets = trips.Where(Function(x) x.Value.STTIME = "")
            If base.Count > 0 AndAlso targets.Count > 0 Then
                '出社指定時刻が未設定のレコードに上記出社指定時刻を反映
                Dim stTime = base.Min(Function(x) Val(x.Value.STTIME))
                For Each target In targets
                    target.Value.STTIME = stTime
                Next
            End If
        Next
        ...
        For Each target In lstTarget
            ...
            Else
                ...
                If result = False Then
                    ...
                    Master.Output(C_MESSAGE_NO.FILE_SEND_ERROR, C_MESSAGE_TYPE.ABORT, FtpControl.TargetID)
                    Return False
                End If
            End If
    ...
    Private Sub DBselect_T4SELECT()
        ...
        For Each T00006row In T00006tbl.Rows
            ...
            If T00006row("CAMPCODE") <> GRT00006WRKINC.C_COMP_ENEX AndAlso
                T00006row("TSHABANBACK") = "" Then
                T00006row("SHARYOTYPEB") = T00006row("SHARYOTYPEF")
                T00006row("TSHABANB") = T00006row("TSHABANF")
                T00006row("TSHABANBACK") = T00006row("TSHABANFRONT")
            End If

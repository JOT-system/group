...
Public Class GRTA0009LMKINTAICREW
    ...
    Protected Sub Initialize()
        ...
        '〇従業員時間の色設定
        SetStaffWkTimeColor(pnlListArea.FindControl("pnlListArea_DL").Controls(0), pnlListArea.FindControl("pnlListArea_DR").Controls(0), CS0013ProfView.SRCDATA)
    ...
    Protected Sub WF_ButtonEMS_Click()
        ...
        TA0009tblView.RowFilter = "HAIACTTIME > 0 or PAYACTTIME > 0 or PAYKBN <> '00'"
        ...
        '早出補填時間
        TA0009tbl4EMS.Columns.Add("HAYADE", Type.GetType("System.String"))
        ...
        For i As Integer = 0 To TA0009tbl4EMS.Rows.Count - 1
            ...
            If CDate(row("PAYSHUSHADATE")).ToString("HHmmss") = "000000" AndAlso
               CDate(row("PAYTAISHADATE")).ToString("HHmmss") = "000000" Then
                ...
                '早出補填時間
                If row("PAYHAYADETIME") <> 0 Then row("HAYADE") = CStr(Int(row("PAYHAYADETIME") / 60)) & ":" & Format(row("PAYHAYADETIME") Mod 60, "00")
                'デジタコ実績区分
                If DateDiff("n", row("NACHAISTDATE"), row("NACHAIENDDATE")) > 0 Then '拘束時間がゼロより大きいとき
                    row("JISSEKIKBN") = "日報仮"
                    row("PAYKBN") = "00"
                    row("PAYKBNNAME") = "通常"
                    row("PAYSHUSHADATE") = row("NACHAISTDATE")
                    row("PAYTAISHADATE") = row("NACHAIENDDATE")
                Else
                    row("JISSEKIKBN") = ""
                End If
            Else
    ...
    Private Sub GetTA0009Wk(ByRef IO_TBL As DataTable)
        ...
        Using SQLcon As SqlConnection = CS0050Session.getConnection
            ...
            SQLStr.AppendLine("        , isnull(rtrim(L01.PAYHAYADETIME), '0') as PAYHAYADETIME ")
                ...
                For Each WI_ORG As String In W_ORGLst
                    ...
                    Try
                        ...
                        Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                            ...
                            Dim wSUM_PAYHAYADETIME As Integer = 0                                                 '早出補填時間
                            ...
                            While SQLdr.Read
                                ...
                                Else
                                    ...
                                    Else
                                        ...
                                        wMin = DateDiff("n", CDate(TA0009row("PAYSHUSHADATE")), CDate(TA0009row("PAYTAISHADATE")))
                                        '計算結果がマイナス値か24時間超の場合は、ゼロとする
                                        If wMin <= 0 Or wMin >= 4320 Then
                                            TA0009row("PAYACTTIME") = 0
                                        Else
                                            TA0009row("PAYACTTIME") = wMin
                                        End If
                                        ...
                                        TA0009row("PAYHAYADETIME") = wSUM_PAYHAYADETIME                                     '早出補填時間
                                ...
                                If SQLdr("ACACHANTEI") = "HSD" OrElse SQLdr("ACACHANTEI") = "HSC" Then
                                    ...
                                    wMin = DateDiff("n", CDate(TA0009row("NACHAISTDATE")), CDate(TA0009row("NACHAIENDDATE")))
                                    '計算結果がマイナス値か24時間超の場合は、ゼロとする
                                    If wMin <= 0 Or wMin >= 4320 Then
                                        TA0009row("HAIACTTIME") = 0
                                    Else
                                        TA0009row("HAIACTTIME") = wMin
                                    End If
                                End If
                                ...
                                wINT = Val(SQLdr("PAYHAYADETIME"))
                                wSUM_PAYHAYADETIME = wSUM_PAYHAYADETIME + wINT                                       '早出補填時間
                            End While
                            '〇最終レコード出力
    ...
    Private Sub GetTA0009WkSum3(ByRef IO_TBL As DataTable)
        ...
        Using SQLcon As SqlConnection = CS0050Session.getConnection
            Try
                ...
                SQLstr.Append("       isnull(rtrim(L04.CAMPCODE), '') as CAMPCODE ")
                SQLstr.Append("     , isnull(rtrim(M01.NAMES), '') as CAMPNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.KEIJOYMD), '1950/01/01') as KEIJOYMD ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHORG), '') as ACKEIJOORG ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHORGNAME), '') as ACKEIJOORGNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.DENYMD), '1950/01/01') as DENYMD ")
                SQLstr.Append("     , isnull(rtrim(L04.DENNO), '') as DENNO ")
                SQLstr.Append("     , isnull(rtrim(L04.KANRENDENNO), '') as KANRENDENNO ")
                SQLstr.Append("     , isnull(rtrim(L04.DTLNO), '') as DTLNO ")
                SQLstr.Append("     , isnull(rtrim(L04.ACACHANTEI), '') as ACACHANTEI ")
                SQLstr.Append("     , isnull(rtrim(L04.ACACHANTEINAME), '') as ACACHANTEINAME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACSHUKODATE), '1950/01/01') as NACSHUKODATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACHAISTDATE), '1950/01/01') as NACHAISTDATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACHAIENDDATE), '1950/01/01') as NACHAIENDDATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACHAIWORKTIME), '0') as NACHAIWORKTIME ")
                SQLstr.Append("     , '1950/01/01' as NACGESSTDATE ")
                SQLstr.Append("     , '1950/01/01' as NACGESENDDATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACGESWORKTIME), '0') as NACGESWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACCHOWORKTIME), '0') as NACCHOWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACTTLWORKTIME), '0') as NACTTLWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACOUTWORKTIME), '0') as NACOUTWORKTIME ")
                SQLstr.Append("     , 0 as NACJITTLETIME ")
                SQLstr.Append("     , 0 as NACKUTTLTIME ")
                SQLstr.Append("     , '1950/01/01' as NACBREAKSTDATE ")
                SQLstr.Append("     , '1950/01/01' as NACBREAKENDDATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACBREAKTIME), '0') as NACBREAKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACCHOBREAKTIME), '0') as NACCHOBREAKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACTTLBREAKTIME), '0') as NACTTLBREAKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACOFFICESORG), '') as NACOFFICESORG ")
                SQLstr.Append("     , isnull(rtrim(L04.NACOFFICESORG), '') as NACOFFICESORGNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSHUSHADATE), '1950/01/01') as PAYSHUSHADATE ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYTAISHADATE), '1950/01/01') as PAYTAISHADATE ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSTAFFKBN), '') as PAYSTAFFKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSTAFFKBNNAME), '') as PAYSTAFFKBNNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSTAFFCODE), '') as PAYSTAFFCODE ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSTAFFCODENAME), '') as PAYSTAFFCODENAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYMORG), '') as PAYMORG ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYMORGNAME), '') as PAYMORGNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHORG), '') as PAYHORG ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHORGNAME), '') as PAYHORGNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHOLIDAYKBN), '') as PAYHOLIDAYKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHOLIDAYKBNNAME), '') as PAYHOLIDAYKBNNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYKBN), '') as PAYKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYKBNNAME), '') as PAYKBNNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSHUKCHOKKBN), '') as PAYSHUKCHOKKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSHUKCHOKKBNNAME), '') as PAYSHUKCHOKKBNNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYJYOMUKBN), '') as PAYJYOMUKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYJYOMUKBNNAME), '') as PAYJYOMUKBNNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYWORKTIME), '0') as PAYWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYNIGHTTIME), '0') as PAYNIGHTTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYORVERTIME), '0') as PAYORVERTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYWNIGHTTIME), '0') as PAYWNIGHTTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYWSWORKTIME), '0') as PAYWSWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSNIGHTTIME), '0') as PAYSNIGHTTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHWORKTIME), '0') as PAYHWORKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHNIGHTTIME), '0') as PAYHNIGHTTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYBREAKTIME), '0') as PAYBREAKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYHAYADETIME), '0') as PAYHAYADETIME ")
                SQLstr.Append("     , '0' as SUPPORTKBN ")
                SQLstr.Append("    FROM L0004_SUMMARYK L04 ")
                SQLstr.Append("    INNER JOIN M0001_CAMP M01 ")
                SQLstr.Append("       ON M01.CAMPCODE = L04.CAMPCODE ")
                SQLstr.Append("       and M01.STYMD  <= L04.NACSHUKODATE ")
                SQLstr.Append("       and M01.ENDYMD >= L04.NACSHUKODATE ")
                SQLstr.Append("       and M01.DELFLG <> '1' ")
                SQLstr.Append("    WHERE ")
                SQLstr.Append("           L04.CAMPCODE      = @P02 ")
                SQLstr.Append("       And L04.NACSHUKODATE <= @P03 ")
                SQLstr.Append("       And L04.NACSHUKODATE >= @P04 ")
                SQLstr.Append("       And L04.PAYHORG       = @P05 ")
                SQLstr.Append("       And L04.RECODEKBN     = '0' ")
                SQLstr.Append("       and L04.PAYSTAFFKBN like '03%' ")
                SQLstr.Append("       and L04.DELFLG       <> '1' ")
                ...
                SQLstr.Append("     , isnull(rtrim(L04.NACHAISTDATE), '1950/01/01') as NACHAISTDATE ")
                SQLstr.Append("     , isnull(rtrim(L04.NACHAIENDDATE), '1950/01/01') as NACHAIENDDATE ")
                SQLstr.Append("     , '0' as NACHAIWORKTIME ")
                SQLstr.Append("     , '1950/01/01' as NACGESSTDATE ")
                SQLstr.Append("     , '1950/01/01' as NACGESENDDATE ")
                SQLstr.Append("     , '0' as NACGESWORKTIME ")
                SQLstr.Append("     , '0' as NACCHOWORKTIME ")
                SQLstr.Append("     , '0' as NACTTLWORKTIME ")
                SQLstr.Append("     , '0' as NACOUTWORKTIME ")
                SQLstr.Append("     , isnull(L03.NACJITTLETIME,0) as NACJITTLETIME")
                SQLstr.Append("     , isnull(L03.NACKUTTLTIME,0) as NACKUTTLTIME ")
                SQLstr.Append("     , '1950/01/01' as NACBREAKSTDATE ")
                SQLstr.Append("     , '1950/01/01' as NACBREAKENDDATE ")
                SQLstr.Append("     , '0' as NACBREAKTIME ")
                SQLstr.Append("     , '0' as NACCHOBREAKTIME ")
                SQLstr.Append("     , '0' as NACTTLBREAKTIME ")
                SQLstr.Append("     , isnull(rtrim(L04.NACOFFICESORG), '') as NACOFFICESORG ")
                SQLstr.Append("     , isnull(rtrim(L04.NACOFFICESORG), '') as NACOFFICESORGNAME ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYSHUSHADATE), '1950/01/01') as PAYSHUSHADATE ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYTAISHADATE), '1950/01/01') as PAYTAISHADATE ")
                ...
                SQLstr.Append("     , isnull(rtrim(L04.PAYJYOMUKBN), '') as PAYJYOMUKBN ")
                SQLstr.Append("     , isnull(rtrim(L04.PAYJYOMUKBNNAME), '') as PAYJYOMUKBNNAME ")
                SQLstr.Append("     , '0' as PAYWORKTIME ")
                SQLstr.Append("     , '0' as PAYNIGHTTIME ")
                SQLstr.Append("     , '0' as PAYORVERTIME ")
                SQLstr.Append("     , '0' as PAYWNIGHTTIME ")
                SQLstr.Append("     , '0' as PAYWSWORKTIME ")
                SQLstr.Append("     , '0' as PAYSNIGHTTIME ")
                SQLstr.Append("     , '0' as PAYHWORKTIME ")
                SQLstr.Append("     , '0' as PAYHNIGHTTIME ")
                SQLstr.Append("     , '0' as PAYBREAKTIME ")
                SQLstr.Append("     , '0' as PAYHAYADETIME ")
                SQLstr.Append("     , '0' as SUPPORTKBN ")
                SQLstr.Append("    FROM L0004_SUMMARYK L04 ")
                SQLstr.Append("    INNER JOIN M0001_CAMP M01 ")
                SQLstr.Append("       ON  M01.CAMPCODE = L04.CAMPCODE ")
                ...
                SQLstr.Append("       and M01.ENDYMD  >= L04.NACSHUKODATE ")
                SQLstr.Append("       and M01.DELFLG  <> '1' ")
                SQLstr.Append("    INNER JOIN L0003_SUMMARYN L03 ")
                SQLstr.Append("	      ON  L03.CAMPCODE     = L04.CAMPCODE ")
                SQLstr.Append("	      and L03.NACSHUKODATE = L04.NACSHUKODATE ")
                SQLstr.Append("	      and L03.KEYSTAFFCODE = L04.PAYSTAFFCODE ")
                SQLstr.Append("       and L03.DELFLG  <> '1' ")
                SQLstr.Append("    WHERE ")
                SQLstr.Append("           L04.CAMPCODE      = @P02 ")
                SQLstr.Append("       And L04.NACSHUKODATE <= @P03 ")
                SQLstr.Append("       And L04.NACSHUKODATE >= @P04 ")
                ...
                SQLstr.Append("       PAYHORG, PAYSTAFFCODE, NACSHUKODATE ")
    ...
    Protected Sub AddColumnToTA0009tbl(ByRef IO_TBL As DataTable)
        ...
        IO_TBL.Columns.Add("PAYHAYADETIME", GetType(String))              '早出補填時間
        ...
        IO_TBL.Columns.Add("HAIACTTIME", GetType(Integer))
        IO_TBL.Columns.Add("PAYACTTIME", GetType(Integer))
    End Sub
    ...
    ''' 従業員作業時間による色の設定
    ''' </summary>
    ''' <param name="I_TABLE_R"></param>
    ''' <param name="I_TABLE_L"></param>
    ''' <param name="I_TBL"></param>
    ''' <remarks></remarks>
    Protected Sub SetStaffWkTimeColor(ByRef I_TABLE_R As Control, ByRef I_TABLE_L As Control, ByVal I_TBL As DataTable)

        For i As Integer = 0 To I_TBL.Rows.Count - 1
            Dim row As DataRow = I_TBL.Rows.Item(i)
            Dim maxTime As Integer = 0
            Dim maxDayTime As Integer = 0
            Dim overCntFlg As Boolean = False
            Dim overTimeFlg As Boolean = False

            '月合計時間チェック

            If Val(row("OVERCNT")) > 0 Then
                '超過回数あり
                overCntFlg = True
            End If

            If row("RECKBN") = "01" Then
                '残業時間
                maxTime = HHMMToMinutes(row("MAXORVERTIME") & ":00")
                maxDayTime = 1440
            ElseIf row("RECKBN") = "02" Then
                'X拘束時間
                maxTime = HHMMToMinutes(row("MAXWORKTIME") & ":00")
                maxDayTime = 960
            ElseIf row("RECKBN") = "03" Then
                'ハンドル時間
                maxTime = Int32.MaxValue
                maxDayTime = 540
            End If
            If HHMMToMinutes(row("TTL")) > maxTime Then
                '時間超過あり
                overTimeFlg = True
            End If

            Dim tableRow = I_TABLE_R.Controls.Item(i)
            If tableRow.GetType.Name.ToLower = "tableheaderrow" Then
                For Each cell As TableCell In tableRow.Controls
                    If cell.GetType.Name.ToLower = "tablecell" Then
                        Dim cellName As String = cell.Attributes("name")
                        If overCntFlg = True AndAlso cellName.StartsWith("L_" & "OVERCNT") Then
                            cell.Style.Add("color", "red")
                        End If
                        If overTimeFlg = True AndAlso cellName.StartsWith("L_" & "TTL") Then
                            '[TTL]のみ赤文字設定。時間差[TTLSA]は除外
                            If Not cellName.Contains("TTLSA") Then
                                cell.Style.Add("color", "red")
                            End If
                        End If
                    End If
            ...
            '日別時間チェック
            Dim tableLRow = I_TABLE_L.Controls.Item(i)
            For j As Integer = 1 To 31
                Dim dayTime As Integer = HHMMToMinutes(row("DAY" & j.ToString("00")))
                If dayTime > maxDayTime Then
                    '日別時間超過あり
                    Dim tableCell As TableCell = tableLRow.Controls.Item(j - 1)
                    If tableCell.GetType.Name.ToLower = "tablecell" Then
                        Dim cellName As String = tableCell.Attributes("name")
                        If cellName.StartsWith("R_" & "DAY" & j.ToString("00")) Then

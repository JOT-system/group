...
Public Class GRMA0006SHABANORG
    ...
    Protected Sub DetailBoxToMA0006tbl()
        ...
        For i As Integer = 0 To MA0006tbl.Rows.Count - 1
            Dim MA0006row = MA0006tbl.Rows(i)

            '業務車番
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "GSHABAN" & (i + 1))) AndAlso
                MA0006row("GSHABAN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "GSHABAN" & (i + 1))) Then
                MA0006row("GSHABAN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "GSHABAN" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("GSHABAN"))

            '統一車番(前)
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHARYOF" & (i + 1))) AndAlso
                MA0006row("TSHARYOF") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOF" & (i + 1))) Then
                MA0006row("TSHARYOF") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOF" & (i + 1)))
                MA0006row("SHARYOTYPEF") = Mid(MA0006row("TSHARYOF"), 1, 1)
                MA0006row("TSHABANF") = Mid(MA0006row("TSHARYOF"), 2, 19)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHARYOF"))
            Master.EraseCharToIgnore(MA0006row("SHARYOTYPEF"))
            Master.EraseCharToIgnore(MA0006row("TSHABANF"))

            '登録車番(前)
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHABANFNAMES" & (i + 1))) AndAlso
                MA0006row("TSHABANFNAMES") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANFNAMES" & (i + 1))) Then
                MA0006row("TSHABANFNAMES") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANFNAMES" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHABANFNAMES"))

            '統一車番(後)
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHARYOB" & (i + 1))) AndAlso
                MA0006row("TSHARYOB") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOB" & (i + 1))) Then
                MA0006row("TSHARYOB") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOB" & (i + 1)))
                MA0006row("SHARYOTYPEB") = Mid(MA0006row("TSHARYOB"), 1, 1)
                MA0006row("TSHABANB") = Mid(MA0006row("TSHARYOB"), 2, 19)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHARYOB"))
            Master.EraseCharToIgnore(MA0006row("SHARYOTYPEB"))
            Master.EraseCharToIgnore(MA0006row("TSHABANB"))

            '登録車番(後)
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHABANBNAMES" & (i + 1))) AndAlso
                MA0006row("TSHABANBNAMES") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANBNAMES" & (i + 1))) Then
                MA0006row("TSHABANBNAMES") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANBNAMES" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHABANBNAMES"))

            '統一車番(後)2
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHARYOBB" & (i + 1))) AndAlso
                MA0006row("TSHARYOBB") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOBB" & (i + 1))) Then
                MA0006row("TSHARYOBB") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHARYOBB" & (i + 1)))
                MA0006row("SHARYOTYPEBB") = Mid(MA0006row("TSHARYOBB"), 1, 1)
                MA0006row("TSHABANBB") = Mid(MA0006row("TSHARYOBB"), 2, 19)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHARYOBB"))
            Master.EraseCharToIgnore(MA0006row("SHARYOTYPEBB"))
            Master.EraseCharToIgnore(MA0006row("TSHABANBB"))

            '登録車番(後)2
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "TSHABANBBNAMES" & (i + 1))) AndAlso
                MA0006row("TSHABANBBNAMES") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANBBNAMES" & (i + 1))) Then
                MA0006row("TSHABANBBNAMES") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "TSHABANBBNAMES" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("TSHABANBBNAMES"))

            '契約区分
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "MANGOWNCONT" & (i + 1))) AndAlso
                MA0006row("MANGOWNCONT") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "MANGOWNCONT" & (i + 1))) Then
                MA0006row("MANGOWNCONT") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "MANGOWNCONT" & (i + 1)))
                CODENAME_get("MANGOWNCONT", MA0006tbl(i)("MANGOWNCONT"), MA0006tbl(i)("MANGOWNCONTNAMES"), WW_DUMMY)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("MANGOWNCONT"))

            '傭車
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "MANGSUPPL" & (i + 1))) AndAlso
                MA0006row("MANGSUPPL") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "MANGSUPPL" & (i + 1))) Then
                MA0006row("MANGSUPPL") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "MANGSUPPL" & (i + 1)))
                CODENAME_get("MANGSUPPL", MA0006tbl(i)("MANGSUPPL"), MA0006tbl(i)("MANGSUPPLNAMES"), WW_DUMMY)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("MANGSUPPL"))

            '矢崎車端用車番コード
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "YAZKSHABAN" & (i + 1))) AndAlso
                MA0006row("YAZKSHABAN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "YAZKSHABAN" & (i + 1))) Then
                MA0006row("YAZKSHABAN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "YAZKSHABAN" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("YAZKSHABAN"))

            '光英車端用車番コード
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "KOEISHABAN" & (i + 1))) AndAlso
                MA0006row("KOEISHABAN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "KOEISHABAN" & (i + 1))) Then
                MA0006row("KOEISHABAN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "KOEISHABAN" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("KOEISHABAN"))

            'JSR車番コード
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "JSRSHABAN" & (i + 1))) AndAlso
                MA0006row("JSRSHABAN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "JSRSHABAN" & (i + 1))) Then
                MA0006row("JSRSHABAN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "JSRSHABAN" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("JSRSHABAN"))

            '水素車
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SUISOKBN" & (i + 1))) AndAlso
                MA0006row("SUISOKBN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SUISOKBN" & (i + 1))) Then
                MA0006row("SUISOKBN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SUISOKBN" & (i + 1)))
                CODENAME_get("SUISOKBN", MA0006tbl(i)("SUISOKBN"), MA0006tbl(i)("SUISOKBNNAMES"), WW_DUMMY)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SUISOKBN"))

            '勤怠用油種区分
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "OILKBN" & (i + 1))) AndAlso
                MA0006row("OILKBN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "OILKBN" & (i + 1))) Then
                MA0006row("OILKBN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "OILKBN" & (i + 1)))
                CODENAME_get("OILKBN", MA0006tbl(i)("OILKBN"), MA0006tbl(i)("OILKBNNAMES"), WW_DUMMY)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("OILKBN"))

            '勤怠用車両区分
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOKBN" & (i + 1))) AndAlso
                MA0006row("SHARYOKBN") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOKBN" & (i + 1))) Then
                MA0006row("SHARYOKBN") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOKBN" & (i + 1)))
                CODENAME_get("SHARYOKBN", MA0006tbl(i)("SHARYOKBN"), MA0006tbl(i)("SHARYOKBNNAMES"), WW_DUMMY)
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOKBN"))

            'SEQ
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SEQ" & (i + 1))) AndAlso
                MA0006row("SEQ") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SEQ" & (i + 1))) Then
                MA0006row("SEQ") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SEQ" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SEQ"))

            '削除
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "DELFLG" & (i + 1))) AndAlso
                MA0006row("DELFLG") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "DELFLG" & (i + 1))) Then
                MA0006row("DELFLG") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "DELFLG" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("DELFLG"))

            '車両情報１
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO1" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO1") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO1" & (i + 1))) Then
                MA0006row("SHARYOINFO1") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO1" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO1"))

            '車両情報２
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO2" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO2") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO2" & (i + 1))) Then
                MA0006row("SHARYOINFO2") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO2" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO2"))

            '車両情報３
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO3" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO3") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO3" & (i + 1))) Then
                MA0006row("SHARYOINFO3") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO3" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO3"))

            '車両情報４
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO4" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO4") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO4" & (i + 1))) Then
                MA0006row("SHARYOINFO4") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO4" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO4"))

            '車両情報５
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO5" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO5") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO5" & (i + 1))) Then
                MA0006row("SHARYOINFO5") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO5" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO5"))

            '車両情報６
            If Not IsNothing(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO6" & (i + 1))) AndAlso
                MA0006row("SHARYOINFO6") <> Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO6" & (i + 1))) Then
                MA0006row("SHARYOINFO6") = Convert.ToString(Request.Form("txt" & pnlListArea.ID & "SHARYOINFO6" & (i + 1)))
                MA0006row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
            End If
            Master.EraseCharToIgnore(MA0006row("SHARYOINFO6"))
        Next
    ...
    Protected Sub WF_ButtonDownload_Click()
        ...
        '空行を除外する
        Dim tbl = MA0006tbl.AsEnumerable.Where(Function(x) Not String.IsNullOrEmpty(x.Item("GSHABAN").ToString))
        Dim wkTbl As DataTable = If(tbl.Any, tbl.CopyToDataTable, MA0006tbl.Clone())
        ...
        CS0030REPORT.TBLDATA = wkTbl                             'データ参照  Table

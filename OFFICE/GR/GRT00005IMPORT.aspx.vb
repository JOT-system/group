...
Public Class GRT00005IMPORT
    ...
    Private WW_ERRLIST_KOUEI As List(Of String)                     'インポート全体のエラー
    ...
    Protected Sub WF_ButtonSAVE_Click()
        ...
        '一時保存ファイルに出力
        If Not Master.SaveTable(T0005PARMtbl, work.WF_T5_XMLsavePARM.Text) Then Exit Sub
...
#Region "<<YAZAKI>>"
    ...
    Protected Sub UPLOAD_YAZAKI(ByRef O_RTN As String)
        ...
        '■項目変換（T0005INPtblより）
        '-----------------------------------------------
        ConvT0005tblData(T0005INPtbl, False, WW_ERRCODE)
        If Not isNormal(WW_ERRCODE) Then
            O_RTN = WW_ERRCODE
            Exit Sub
        End If
            ...
            For i As Integer = 0 To YHAISOtbl.Rows.Count - 1
                ...
                Else
                    ...
                    Else
                        ...
                        If WW_ENDYMD = "" Then
                            WW_ENDYMD = YHAISOrow("FIELD9")
                        End If
                        WW_MIN = DateDiff("n", WW_ENDYMD, YHAISOrow("FIELD9"))
                        T0005INProw("MOVETIME") = T0005COM.MinutesToHHMM(WW_MIN)
                    End If
                    WW_ACT += WW_MIN
                End If
...
#Region "<<KOUEI>>"
    ...
    Protected Sub UPLOAD_KOUEI(ByVal I_MODE As String, ByRef O_RTN As String)
        rightview.SetErrorReport("")
        WW_ERRLIST_KOUEI = New List(Of String)
    ...
    Protected Sub UPLOAD_KOUEI(ByVal I_MODE As String, ByVal I_FILEDIR As String, ByRef O_RTN As String)
        ...
        For Each WW_PREFIX As String In WW_SYASAI_PREFIX
            ...
            If WW_Files.Count = 0 Then
                ...
                WW_OLD_MODE = True
            Else
                WW_FileName1 = WW_Files(0)
            End If

            '〇JX対象は旧仕様の読み込みも行う
            If I_MODE = GRT00005WRKINC.TERM_TYPE.JX AndAlso WW_OLD_MODE = True Then
                Try
                ...
                Catch ex As Exception
                    ...
                    Exit Sub
                End Try
            ElseIf I_MODE = GRT00005WRKINC.TERM_TYPE.JOT AndAlso WW_OLD_MODE = True Then
                Try
                    For Each tempFile2 As String In System.IO.Directory.GetFiles(I_FILEDIR, "*.*")
                        ' ファイルパスからファイル名を取得
            ...
            If WW_OLD_MODE = True OrElse WW_FileName1.IndexOf("UPLOAD_TMP") > 0 Then
                '旧ファイル処理
            ...
            Else
                'WF_KoueiLoadFile.Items.Clear()
                '光英受信ファイル処理（取得した全てのファイルを処理する）
                Array.Reverse(WW_Files)
                For Each tempFile As String In WW_Files
                    Dim WW_tmpfile As String() = Split(tempFile, "_")
                    Dim WW_YMD As String = WW_tmpfile(WW_tmpfile.Count - 2)
                    Dim WW_FROM As String = Replace(work.WF_SEL_STYMD.Text, "/", "")
                    Dim WW_TO As String = Replace(work.WF_SEL_ENDYMD.Text, "/", "")

                    '指定範囲外は、読み飛ばす
                    If WW_YMD < WW_FROM OrElse WW_YMD > WW_TO Then
                        Continue For
                    End If

                    '同一ファイル名は処理しない（ファイル名の「稼働日_日報番号」をキーワードとする）
                    Dim WW_FIND As Boolean = False
                    For Each file As ListItem In WF_KoueiLoadFile.Items
                        Dim f = New System.IO.FileInfo(file.Value)
                        Dim WW_SPRIT As String() = Split(f.ToString, "_")
                        Dim WW_KEYWORD As String = WW_SPRIT(3) & "_" & WW_SPRIT(4)
                        If tempFile.IndexOf(WW_KEYWORD) > 0 Then
                            WW_FIND = True
                            Exit For
                        End If
                    Next

                    '個別ファイル名の保存（DB更新時、USED（処理済み）に変更するため
                    WF_KoueiLoadFile.Items.Add(New ListItem(tempFile))

                    '同一ファイル名は処理しない
                    If WW_FIND = True Then Continue For

                    '車載CSV
                    ReadCsvFile(tempFile, KSYASAItbl, WW_ERRCODE)
                    If Not isNormal(WW_ERRCODE) Then
                        O_RTN = WW_ERRCODE
                        Exit Sub
                    End If
            ...
            '出荷場所、届先登録後、LeftBox再ロード
            Dim prmData = work.CreateDistinationParam(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text, "", "1")
            prmData.Item(GRIS0005LeftBox.C_PARAMETERS.LP_RELOAD) = True
            leftview.CodeToName(LIST_BOX_CLASSIFICATION.LC_DISTINATION, "", WW_DUMMY, WW_ERRCODE, prmData)
            prmData = work.CreateDistinationParam(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text, "", "2")
            prmData.Item(GRIS0005LeftBox.C_PARAMETERS.LP_RELOAD) = True
            leftview.CodeToName(LIST_BOX_CLASSIFICATION.LC_DISTINATION, "", WW_DUMMY, WW_ERRCODE, prmData)
            ...
            '■項目変換（T0005INPtblより）
            '-----------------------------------------------
            ConvT0005tblData(T0005INPtbl, WW_JX_OLD_MODE, WW_ERRCODE)
            If Not isNormal(WW_ERRCODE) Then
                O_RTN = WW_ERRCODE
                Exit Sub
            End If
        ...
        '○メッセージ表示
        WW_ERRLIST_KOUEI.AddRange(WW_ERRLIST_ALL)

        If WW_ERRLIST_KOUEI.Count > 0 Then
            If WW_ERRLIST_KOUEI.IndexOf(C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR) >= 0 Then
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            ElseIf WW_ERRLIST_KOUEI.IndexOf(C_MESSAGE_NO.BOX_ERROR_EXIST) >= 0 Then
                O_RTN = C_MESSAGE_NO.BOX_ERROR_EXIST
            Else
                O_RTN = C_MESSAGE_NO.WORNING_RECORD_EXIST
            End If
    ...
    Protected Sub CreateT0005tblHeaderForKouei(ByVal I_MODE As String, ByRef O_RTN As String)
        ...
        Try
            ...
            If KSYASAItbl.Rows.Count > 0 Then
                'ヘッダーレコードの作成
                WW_ROW("YMD") = WW_OLD_YYYYMMDD
                WW_ROW("NIPPONO") = WW_OLD_NIPPONO
                WW_ROW("STAFFCODE") = WW_OLD_STAFFCODE
            ...
            CS0026TBLSORT.TABLE = T0005WKtbl
            CS0026TBLSORT.SORTING = ""
            CS0026TBLSORT.FILTER = "ERR = '0'"
            T0005WKtbl = CS0026TBLSORT.Sort()
    ...
    Protected Sub CreateT0005tblDetailForKouei(ByVal I_MODE As String,
        ...
        Dim WW_JIDISTANCETTL As Decimal = 0                         '走行距離＋積卸敷地内距離（実車）
        Dim WW_KUDISTANCETTL As Decimal = 0                         '走行距離＋積卸敷地内距離（空車）
        ...
        Try
            ...
            For i As Integer = I_STSEQ To I_ENDSEQ
                ...
                Select Case KSYASAIrow("FIELD13")
                    Case "A1", "F1", "F7"
                        'ENEX-G（光英システム）
                        If I_LEGACY_MODE = False Then

                            '作業区分  A1：カード入出  -> A1 始業（出社指定時刻ありの場合）
                            '          F1：始業        -> A1 始業（出社指定時刻なしの場合）
                            '          F7：２マン開始
                            If KSYASAIrow("FIELD13") = "A1" AndAlso KSYASAIrow("FIELD27") <> "000" Then
                                WW_A1 = True
                                '作業区分（F1）：始業
                                T0005INProw("CTRL") = "ON"
                                '始業レコード作成
                                ...
                                '情報３（始業日）
                                '①必須・項目属性チェック
                                I_VALUE = Mid(KSYASAIrow("FIELD33"), 1, 4) & "/" & Mid(KSYASAIrow("FIELD33"), 5, 2) & "/" & Mid(KSYASAIrow("FIELD33"), 7, 2)
                                Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "STDATE", I_VALUE, O_MESSAGE_NO, O_CHECKREPORT, O_VALUE, S0013tbl)
                                If isNormal(O_MESSAGE_NO) Then
                                    WW_STYMD = O_VALUE
                                Else
                                    WW_STYMD = KSYASAIrow("FIELD31")
                                    OutputErrorMessageByKouei(KSYASAIrow, "情報３（始業日）", O_CHECKREPORT, C_MESSAGE_NO.BOX_ERROR_EXIST, I_MODE)
                                End If
                            ...
                            'A1 カード入出に出社指定時刻が指定されている場合、始業はA1で作成されているため終業のみ作成
                            If KSYASAIrow("FIELD13") = "F1" AndAlso WW_A1 = True Then
                                '作業区分（F1）：始業・終業
                                T0005INProw("CTRL") = "ON"
                                '始業レコード作成
                                T0005INProw("WORKKBN") = "Z1" '終業
                                ...
                                If isNormal(O_MESSAGE_NO) Then
                                    If Not String.IsNullOrEmpty(O_VALUE) Then
                                        WW_STTIME = CDate(O_VALUE).ToString("HH:mm")
                                    Else
                                        WW_STTIME = KSYASAIrow("FIELD27")
                                        OutputErrorMessageByKouei(KSYASAIrow, "開始時刻", O_CHECKREPORT, C_MESSAGE_NO.BOX_ERROR_EXIST, I_MODE)
                                    End If
                                Else
                                    WW_STTIME = KSYASAIrow("FIELD27")
                            ...
                            ElseIf KSYASAIrow("FIELD13") = "F1" AndAlso WW_A1 = False Then
                                'A1 カード入出に出社指定時刻が指定されていない場合、F1から始業、終業を作成
                                '作業区分（F1）：始業・終業
                                T0005INProw("CTRL") = "OFF"
                                '始業レコード作成
                                T0005INProw("WORKKBN") = "A1" '始業
                    ...
                    Case "F3"
                        ...
                        Else
                            WW_DISTANCE = 0
                            OutputErrorMessageByKouei(KSYASAIrow, "情報１（距離）", O_CHECKREPORT, C_MESSAGE_NO.BOX_ERROR_EXIST, I_MODE)
                        End If
                        WW_JIDISTANCETTL = WW_DISTANCE

                        '情報２（距離）
                        '①必須・項目属性チェック
                        Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "SOUDISTANCE", KSYASAIrow("FIELD32"), O_MESSAGE_NO, O_CHECKREPORT, O_VALUE, S0013tbl)
                        If isNormal(O_MESSAGE_NO) Then
                                    WW_DISTANCE = O_VALUE
                                Else
                                    WW_DISTANCE = 0
                            OutputErrorMessageByKouei(KSYASAIrow, "情報２（距離）", O_CHECKREPORT, C_MESSAGE_NO.BOX_ERROR_EXIST, I_MODE)
                        End If
                        WW_KUDISTANCETTL = WW_DISTANCE
                        ...
                        If WW_STYMD < WW_ENDYMD Then
                            '出庫の開始時間　＜　始業の開始時間
                            If T0005INProw("STTIME") < WW_STTIME Then
                                '始業→日跨り→出庫の場合、翌日を設定
                                T0005INProw("STDATE") = WW_ENDYMD
                                T0005INProw("ENDDATE") = WW_ENDYMD
                            End If
                    ...
                    Case "G1"
                        ...
                        Dim WW_ACT As Integer = 0
                        Dim WW_STDATE As DateTime = CDate(T0005INProw("STDATE") & " " & T0005INProw("STTIME"))
                        Dim WW_ENDDATE As DateTime = WW_STDATE.AddMinutes(Val(KSYASAIrow("FIELD33")))
                        T0005INProw("ENDDATE") = WW_ENDDATE.ToString("yyyy/MM/dd")
                    ...
                    Case "B1", "B4", "B8", "B9", "BA", "BX", "B5", "BB", "BC"
                        ...
                        '作業区分（B5）：洗車
                        '作業区分（BB,BC）：休憩　-> BB 休憩
                        If KSYASAIrow("FIELD13") = "B1" Then T0005INProw("WORKKBN") = "BA"
                        ...
                        If KSYASAIrow("FIELD13") = "B1" Then
                            ...
                            T0005INProw("WORKTIME") = DateDiff("n", WW_STDATE, WW_ENDDATE)
                        End If
                        ...
                        'B1（荷積準備）は、作業時間がある場合、BA（待機）として出力する
                        If KSYASAIrow("FIELD13") = "B1" AndAlso T0005INProw("WORKTIME") = 0 Then
                            WW_WRITE_FLG = False
                        End If

                        If WW_WRITE_FLG = True Then
                            WW_SOUDISTANCE_T += WW_SOUDISTANCE
                            WW_JIDISTANCE_T += WW_JIDISTANCE
                            WW_KUDISTANCE_T += WW_KUDISTANCE
                            WW_IPPDISTANCE_T += WW_IPPDISTANCE
            ...
            Else
                ...
                WW_KOSKUDISTANCE_T += WW_KOSKUDISTANCE
                '2020/05/18 ADD
                Dim z As Integer = T0005INPtbl.Rows.Count - 1
                Dim WW_row As DataRow = T0005INPtbl.Rows(z)
                WW_row("SOUDISTANCE") = (Val(WW_row("SOUDISTANCE")) + WW_SOUDISTANCE).ToString("#,0.00")
                WW_row("JIDISTANCE") = (Val(WW_row("JIDISTANCE")) + WW_JIDISTANCE).ToString("#,0.00")
                WW_row("IPPJIDISTANCE") = (Val(WW_row("IPPJIDISTANCE")) + WW_IPPJIDISTANCE).ToString("#,0.00")
                WW_row("KUDISTANCE") = (Val(WW_row("KUDISTANCE")) + WW_KUDISTANCE).ToString("#,0.00")
                WW_row("IPPKUDISTANCE") = (Val(WW_row("IPPKUDISTANCE")) + WW_IPPKUDISTANCE).ToString("#,0.00")
                WW_row("IPPDISTANCE") = (Val(WW_row("IPPDISTANCE")) + WW_IPPDISTANCE).ToString("#,0.00")
                WW_row("KOSDISTANCE") = (Val(WW_row("KOSDISTANCE")) + WW_KOSDISTANCE).ToString("#,0.00")
                WW_row("KOSJIDISTANCE") = (Val(WW_row("KOSJIDISTANCE")) + WW_KOSJIDISTANCE).ToString("#,0.00")
                WW_row("KOSKUDISTANCE") = (Val(WW_row("KOSKUDISTANCE")) + WW_KOSKUDISTANCE).ToString("#,0.00")
                '2020/05/18 ADD END
                WW_LAST = True
            End If

            '積卸敷地内距離 = 総走行距離（F3レコードで取得した積卸敷地内距離を含む距離）－ 走行距離
            Dim WW_sa As Integer = 0
            Dim WW_jisa As Integer = 0
            Dim WW_kusa As Integer = 0
            If WW_JIDISTANCETTL + WW_KUDISTANCETTL - WW_SOUDISTANCE_T > 0 Then
                WW_sa = WW_JIDISTANCETTL + WW_KUDISTANCETTL - WW_SOUDISTANCE_T
            End If
            'If WW_JIDISTANCETTL - WW_JIDISTANCE_T > 0 Then
            '    WW_jisa = WW_JIDISTANCETTL - WW_JIDISTANCE_T
            'End If
            'If WW_KUDISTANCETTL - WW_KUDISTANCE_T > 0 Then
            '    WW_kusa = WW_KUDISTANCETTL - WW_KUDISTANCE_T
            'End If
            ...
            T0005HINProw("SOUDISTANCE") = (WW_SOUDISTANCE_T + WW_sa).ToString("#,0.00")
            T0005HINProw("IPPDISTANCE") = (WW_IPPDISTANCE_T + WW_sa).ToString("#,0.00")
            ...
            '〇行数分繰り返して設定作業を行う
            Dim WW_sa2 As Integer = WW_sa
            Dim WW_find1 As Boolean = False
            Dim WW_find2 As Boolean = False
            Dim WW_find3 As Boolean = False
            For Each WW_row As DataRow In T0005INPtbl.Rows
                '両方処理したらループを抜ける
                '先頭レコードに差分を加算する
                If WW_row("WORKKBN") <> "F3" AndAlso
                   WW_row("YMD") = T0005HINProw("YMD") AndAlso
                   WW_row("NIPPONO") = T0005HINProw("NIPPONO") AndAlso
                  (Val(WW_row("JIDISTANCE")) > 0 OrElse Val(WW_row("KUDISTANCE")) > 0) Then
                    WW_row("SOUDISTANCE") = Val(WW_row("SOUDISTANCE") + WW_sa2).ToString("#,0.00")
                    WW_row("IPPDISTANCE") = Val(WW_row("IPPDISTANCE") + WW_sa2).ToString("#,0.00")
                    If Val(WW_row("JIDISTANCE")) > 0 AndAlso WW_sa2 > 0 Then
                        WW_row("JIDISTANCE") = Val(WW_row("JIDISTANCE") + WW_sa2).ToString("#,0.00")
                        WW_row("IPPJIDISTANCE") = Val(WW_row("IPPJIDISTANCE") + WW_sa2).ToString("#,0.00")
                        WW_jisa = WW_sa2
                        WW_sa2 = 0
                    End If
                    If Val(WW_row("KUDISTANCE")) > 0 AndAlso WW_sa2 > 0 Then
                        WW_row("KUDISTANCE") = Val(WW_row("KUDISTANCE") + WW_sa2).ToString("#,0.00")
                        WW_row("IPPKUDISTANCE") = Val(WW_row("IPPKUDISTANCE") + WW_sa2).ToString("#,0.00")
                        WW_kusa = WW_sa2
                        WW_sa2 = 0
                    End If
                    WW_find1 = True
                ...
                If WW_row("WORKKBN") = "F3" AndAlso
                   ...
                   WW_row("NIPPONO") = T0005HINProw("NIPPONO") Then
                    ...
                    If WW_LAST Then
                        WW_row("SOUDISTANCE") = (WW_SOUDISTANCE_T + WW_sa).ToString("#,0.00")
                        WW_row("JIDISTANCE") = (WW_JIDISTANCE_T + WW_jisa).ToString("#,0.00")
                        WW_row("IPPJIDISTANCE") = (WW_IPPJIDISTANCE_T + WW_jisa).ToString("#,0.00")
                        WW_row("KUDISTANCE") = (WW_KUDISTANCE_T + WW_kusa).ToString("#,0.00")
                        WW_row("IPPKUDISTANCE") = (WW_IPPKUDISTANCE_T + WW_kusa).ToString("#,0.00")
                        WW_row("IPPDISTANCE") = (WW_IPPDISTANCE_T + WW_sa).ToString("#,0.00")
                ...
                If WW_row("YMD") >= "2020/01/10" Then
                    'Z1（終業）レコードの開始、終了メータに積卸敷地内距離を設定する
                    If WW_row("WORKKBN") = "Z1" AndAlso
                       WW_row("YMD") = T0005HINProw("YMD") AndAlso
                       WW_row("NIPPONO") = T0005HINProw("NIPPONO") Then
                        WW_row("STMATER") = WW_sa.ToString("#,0.00")
                        WW_row("ENDMATER") = WW_sa.ToString("#,0.00")
                        WW_find3 = True
                    End If

                    If WW_find1 = True AndAlso WW_find2 = True AndAlso WW_find3 = True Then
                        Exit For
                    End If
                Else
                    If WW_find1 = True AndAlso WW_find2 = True Then
                        Exit For
                    End If
                End If
            Next

            T0005HINProw("JIDISTANCE") = (WW_JIDISTANCE_T + WW_jisa).ToString("#,0.00")
            T0005HINProw("IPPJIDISTANCE") = (WW_IPPJIDISTANCE_T + WW_jisa).ToString("#,0.00")
            T0005HINProw("KUDISTANCE") = (WW_KUDISTANCE_T + WW_kusa).ToString("#,0.00")
            T0005HINProw("IPPKUDISTANCE") = (WW_IPPKUDISTANCE_T + WW_kusa).ToString("#,0.00")
    ...
    Protected Sub EditTwoManForKouei(ByVal I_TBL As DataTable, ByRef O_RTN As String)
        ...
        Try
            ...
            For Each WW_KEYrow As DataRow In I_TBL.Rows
                ...
                WW_WORKtbl.Clear()
                '出庫が１件もない場合、先頭行に造成する
                If WW_TWOMANtbl.Rows.Count > 0 AndAlso WW_F1cnt = 0 Then
                ...
                End If
                '帰庫が１件もない場合、最終行に造成する
                If WW_TWOMANtbl.Rows.Count > 0 AndAlso WW_F3cnt = 0 Then
            ...
            '---------------------------------------------------
            CS0026TBLSORT.TABLE = T0005INPtbl
            CS0026TBLSORT.SORTING = "YMD, STAFFCODE, HDKBN DESC, STDATE, STTIME, SEQ, ENDDATE, ENDTIME, WORKKBN"
            CS0026TBLSORT.FILTER = ""
            T0005INPtbl = CS0026TBLSORT.sort()
...
#End Region
    ...
    ''' <param name="O_RTN"></param>
    ''' <remarks></remarks>
    Protected Sub ConvT0005tblData(ByRef T0005INPtbl As DataTable, ByVal I_LEGACY_MODE As Boolean, ByRef O_RTN As String)
        ...
        For i As Integer = 0 To T0005INPtbl.Rows.Count - 1
            ...
            If T0005INProw("GSHABAN") <> "" Then
                Select Case T0005INProw("TERMKBN")
                    ...
                    Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG, GRT00005WRKINC.TERM_TYPE.COSMO
                        'ENEXで新JX（jx_jotxxxxxx.csv、tg_hotxxxxxx.csvの場合）従来の別名車番ではなく光英車番となったため
                        '車番部署マスタを変更すればＯＫだが、ここでは部署マスタを変更せずに光英マスターで変換する
                        'If T0005INProw("CAMPCODE") = GRT00005WRKINC.C_COMP_ENEX AndAlso I_LEGACY_MODE = False Then
                        '    WW_KEYWORD = "GSHABANSHATANK2"
                        'Else
            ...
            Dim WW_T4GET2 As String = "OFF"
            Dim T4_TORICODE As String = ""
            Dim T4_SHUKABASHO As String = ""
            Dim T4_SHUKADATE As String = ""
            Dim T4_OILTYPE As String = ""
            Dim T4_TODOKECODE As String = ""
            Dim T4_PRODUCTCODE As String = ""
            Dim T4_PRODUCT1 As String = ""
            Dim T4_PRODUCT2 As String = ""
            Dim T4_SURYO As Double = 0
            Dim WW_RTN As String = C_MESSAGE_NO.NORMAL
            If T0005INProw("HDKBN") = "D" Then
                    ...
                    If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
                        If T0005INProw("WORKKBN") = "B3" AndAlso T0005INProw("TORICODE") = "" Then
                            ...
                            If isNormal(WW_RTN) Then
                                T0005INProw("TORICODE") = T4_TORICODE
                            Else
                                T0005INProw("TORICODE") = ""
                            End If
                            WW_T4GET = "ON"
                    ...
                    If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
                        If T0005INProw("SHUKABASHO") = "" Then
                            If WW_T4GET = "OFF" Then
                                GetOilTypeForT0004Tbl(T0005INProw, T4_TORICODE, T4_SHUKABASHO, T4_SHUKADATE, T4_OILTYPE, WW_RTN)
                                If isNormal(WW_RTN) Then
                                    T0005INProw("SHUKABASHO") = T4_SHUKABASHO
                                Else
                                    T0005INProw("SHUKABASHO") = ""
                                End If
                                WW_T4GET = "ON"
                        ...
                        If T0005INProw("SHUKADATE") = "" Then
                            If WW_T4GET = "OFF" Then
                                GetOilTypeForT0004Tbl(T0005INProw, T4_TORICODE, T4_SHUKABASHO, T4_SHUKADATE, T4_OILTYPE, WW_RTN)
                                If isNormal(WW_RTN) Then
                                    T0005INProw("SHUKADATE") = T4_SHUKADATE
                                Else
                                    T0005INProw("SHUKADATE") = ""
                                End If
                                WW_T4GET = "ON"
                            Else
                                T0005INProw("SHUKADATE") = T4_SHUKADATE
                ...
                If T0005INProw("WORKKBN") = "B3" Then
                    ...
                    If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
                        If T0005INProw("TODOKECODE") = "" Then
                            If WW_T4GET2 = "OFF" Then
                                ...
                                If isNormal(WW_RTN) Then
                                    T0005INProw("TODOKECODE") = T4_TODOKECODE
                                Else
                                    T0005INProw("TODOKECODE") = ""
                                End If
                            Else
                                T0005INProw("TODOKECODE") = T4_TODOKECODE
                            End If
                    ...
                    If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
                        ...
                        '石油以外
                        If WW_T4GET2 = "OFF" Then
                            GetTodokeForT0004Tbl(T0005INProw, T4_TODOKECODE, T4_PRODUCTCODE, T4_PRODUCT1, T4_PRODUCT2, T4_SURYO, WW_RTN)
                        End If
                        '○品名１～８（石油の場合）
                        '油種を取得
                        For WW_SEQ As Integer = 1 To 8
                            ...
                            End If
                        Next
                        If T4_OILTYPE = "01" OrElse T0005INProw("OILTYPE1") = "01" Then
                        Else
                            '○品名１のみ
                            For WW_SEQ As Integer = 1 To 1
                                Dim WW_OILTYPE As String = "OILTYPE" & WW_SEQ.ToString("0")
                                Dim WW_PRODUCT1 As String = "PRODUCT1" & WW_SEQ.ToString("0")
                                Dim WW_PRODUCT2 As String = "PRODUCT2" & WW_SEQ.ToString("0")
                                Dim WW_PRODUCTCODE As String = "PRODUCTCODE" & WW_SEQ.ToString("0")
                                ...
                                If T0005INProw(WW_PRODUCTCODE) <> "" Then
                                Else
                                    T0005INProw(WW_OILTYPE) = T4_OILTYPE
                                    T0005INProw(WW_PRODUCT1) = T4_PRODUCT1
                                    T0005INProw(WW_PRODUCT2) = T4_PRODUCT2
                                ...
                                '数量
                                If Val(T0005INProw(WW_SURYO)) <= 0 Then
                                    T0005INProw(WW_SURYO) = T4_SURYO
                                End If
                            Next
                        End If
                    Else
            ...
            If T0005INProw("WORKKBN") = "B3" Then
                If T0005INProw("SHUKADATE") = "" Then
                    ...
                    'ENEX以外で石油の場合、出荷日が空白であれば当日日付を設定（救済）
                    If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
                        If T0005INProw("SHUKADATE") = "" AndAlso T0005INProw("OILTYPE1") = "01" Then
                            T0005INProw("SHUKADATE") = T0005INProw("YMD")
                        End If
                    End If
                End If
            End If
    ...
    Protected Sub UpdateGridData(ByRef O_RTN As String)
        ...
        Try
            ...
            CS0026TBLSORT.TABLE = T0005tbl
            CS0026TBLSORT.SORTING = "SELECT, YMD, STAFFCODE, HDKBN DESC, STDATE, STTIME, SEQ, ENDDATE, ENDTIME, WORKKBN"
            CS0026TBLSORT.FILTER = ""
            T0005tbl = CS0026TBLSORT.sort()
    ...
    Protected Sub EditT0005Tbl(ByRef O_RTN As String)
        ...
        Try
            ...
            Dim WW_TEXT As String = ""
            Dim WW_G1DATE As Date = "1950/01/01"
            ...
            For i As Integer = 0 To T0005INPtbl.Rows.Count - 1
                ...
                If isNormal(WW_RTN_SW) AndAlso WW_CONVERT <> "" Then
                ...
                If WW_T0005INProw("HDKBN") = "H" Then
                    ...
                    For j As Integer = i + 1 To T0005INPtbl.Rows.Count - 1
                        ...
                        If WW_T0005INProwj("WORKKBN") = "B3" Then
                            ...
                            If work.WF_SEL_CAMPCODE.Text = GRT00005WRKINC.C_COMP_NJS Then
                                '配送作業以外の荷卸しは、押し間違いと判断しBY（配送作業）とする
                                If IsDate(WW_T0005INProwj("STDATE") & " " & WW_T0005INProwj("STTIME")) Then
                                    Dim WW_B3DATE As Date = CDate(WW_T0005INProwj("STDATE") & " " & WW_T0005INProwj("STTIME"))
                                    If WW_G1DATE > "1950/01/01" AndAlso WW_G1DATE < WW_B3DATE Then
                                        WW_T0005INProwj("WORKKBN") = "BY"
                                        WW_T0005INProwj("TODOKEDATE") = ""
                                    End If
                                End If
                            End If
                        End If
                        ...
                        If work.WF_SEL_CAMPCODE.Text = GRT00005WRKINC.C_COMP_NJS Then
                            '配送作業（ボタン）の終了時間を保存する
                            If WW_T0005INProwj("WORKKBN") = "G1" Then
                                If IsDate(WW_T0005INProwj("ENDDATE") & " " & WW_T0005INProwj("ENDTIME")) Then
                                    WW_G1DATE = CDate(WW_T0005INProwj("ENDDATE") & " " & WW_T0005INProwj("ENDTIME"))
                                End If
                            End If
                        End If
                                ...
                                If WW_A1F1row("CTRL") = "OFF" Then
                                    ...
                                    Select Case WW_A1F1row("TERMKBN")
                                        ...
                                        Case Else  '光英（JX,TG,COSMO）
                                            '救済（T4から取得してみる）
                                            Dim WW_T4STTIME As String = String.Empty
                                            Dim WW_row As DataRow = WW_T0005INPtbl.NewRow
                                            WW_row.ItemArray = WW_A1F1row.ItemArray
                                            CodeToCode("GSHABANSHATANK", WW_row("GSHABAN"), WW_CONVERT, WW_RTN_SW)
                                            WW_row("GSHABAN") = WW_CONVERT
                                            GetSTTimeForT0004(WW_row, WW_T4STTIME, O_RTN)
                                            If Not isNormal(O_RTN) Then Exit Sub
            ...
            If WW_YMDERR = "OFF" AndAlso I_ROW("YMD") <> "" Then
                ...
                If T0005COM.IsInspectionOrg(work.WF_SEL_CAMPCODE.Text, I_ROW("SHIPORG"), WW_RTN) Then
                    If IsNothing(work.CreateTSHABANList(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text, WW_RTN).Items.FindByValue(I_ROW("GSHABAN"))) Then
                        Dim WW_list = work.CreateSHABAN2OILList(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text, WW_RTN)
                        If I_ROW("GSHABAN") <> "" AndAlso WW_list.Items.FindByValue(I_ROW("GSHABAN")).Text = "02" Then
    ...
    Protected Sub CheckAllT0005tbl(ByRef IO_TBL As DataTable, ByRef O_ERRCD As String)
        ...
        '始業、終業を除く（退避）
        Dim WW_A1Z1tbl As DataTable = New DataTable
        If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
            'ENEX以外の場合、始業、就業、配送（NJS）、出庫、帰庫をチェック対象外とする
            CS0026TBLSORT.TABLE = IO_TBL
            CS0026TBLSORT.FILTER = "HDKBN = 'H' or (WORKKBN = 'A1' or WORKKBN = 'G1' or WORKKBN = 'Z1' or WORKKBN = 'F1' or WORKKBN = 'F3')"
            CS0026TBLSORT.SORTING = "YMD, STAFFCODE, HDKBN DESC, STDATE, STTIME, ENDDATE, ENDTIME, WORKKBN, SEQ"
            WW_A1Z1tbl = CS0026TBLSORT.Sort()

            CS0026TBLSORT.FILTER = "HDKBN = 'D' and WORKKBN <> 'A1' and WORKKBN <> 'G1' and WORKKBN <> 'Z1' and WORKKBN <> 'F1' and WORKKBN <> 'F3'"
            IO_TBL = CS0026TBLSORT.Sort()
        Else
            'ENEXの場合、始業、就業、配送（NJS）をチェック対象外とする
            CS0026TBLSORT.TABLE = IO_TBL
        ...
        For i As Integer = 0 To IO_TBL.Rows.Count - 1
            ...
            If i >= 1 Then
                If IO_TBL.Rows(i - 1)("NIPPONO") = IO_TBL.Rows(i)("NIPPONO") Then
                ...
                Else
                    If i = IO_TBL.Rows.Count - 1 Then
                        WW_MOVETIME = T0005COM.HHMMToMinutes(IO_TBL.Rows(i)("MOVETIME"))
                    Else
                        WW_MOVETIME = 0
                    End If
                End If
        ...
        If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
            ...
            For i As Integer = 0 To IO_TBL.Rows.Count - 1
                ...
                If i >= 1 Then
                    ...
                    WW_MOVETIME = DateDiff("n",
                                              IO_TBL.Rows(i - 1)("ENDDATE") + " " + IO_TBL.Rows(i - 1)("ENDTIME"),
                                              IO_TBL.Rows(i)("STDATE") + " " + IO_TBL.Rows(i)("STTIME")
                                             )
                    IO_TBL.Rows(i)("MOVETIME") = T0005COM.MinutesToHHMM(WW_MOVETIME)
                Else
                    If i = IO_TBL.Rows.Count - 1 Then
                        WW_MOVETIME = T0005COM.HHMMToMinutes(IO_TBL.Rows(i)("MOVETIME"))
                    Else
                    ...
                    End If
                End If
                Select Case IO_TBL.Rows(i)("WORKKBN")
                    Case "F1"
                        WW_F1CNT += 1
                    Case "F3"
                        WW_F3CNT += 1
    ...
    Protected Sub GetSTTimeForT0004(ByVal I_ROW As DataRow, ByRef O_STTIME As String, ByRef O_RTN As String)
        ...
        Try
            ...
            Using SQLcon As SqlConnection = CS0050Session.getConnection
                ...
                SQLStr =
                           " SELECT isnull(rtrim(STTIME),'') as STTIME " _
                         & "   FROM T0004_HORDER                                " _
                         & "     WHERE    CAMPCODE        = @P01                " _
                         & "       and    SHIPORG         = @P02                " _
                         & "       and    SHUKODATE       = @P03                " _
    ...
    ''' <summary>
    ''' 配送受注ＤＢを検索し、取引先、出荷場所、油種を取得（救済処置）
    ''' </summary>
    ''' <param name="IO_ROW">検索対象行</param>
    ''' <param name="O_RTN">可否判定</param>
    ''' <remarks></remarks>
    Protected Sub GetOilTypeForT0004Tbl(ByVal IO_ROW As DataRow,
                                        ByRef O_TORICODE As String,
                                        ByRef O_SHUKABASHO As String,
                                        ByRef O_SHUKADATE As String,
                                        ByRef O_OILTYPE As String,
                                        ByRef O_RTN As String)
        ...
        Try
            ...
            Using SQLcon As SqlConnection = CS0050Session.getConnection
                ...
                SQLStr =
                           " SELECT isnull(rtrim(TORICODE),'')    as TORICODE " _
                         & "       ,isnull(rtrim(SHUKABASHO),'')  as SHUKABASHO " _
                         & "       ,isnull(format(SHUKADATE, 'yyyy/MM/dd'),'')   as SHUKADATE " _
                ...
                Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                    ...
                    While SQLdr.Read
                        '○出力編集
                        O_TORICODE = SQLdr("TORICODE")
                        O_SHUKABASHO = SQLdr("SHUKABASHO")
                        O_SHUKADATE = SQLdr("SHUKADATE")
                        O_OILTYPE = SQLdr("OILTYPE")
                        O_RTN = C_MESSAGE_NO.NORMAL
                        Exit While
                    End While
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "T0004_HORDER Select")
            CS0011LOGWRITE.INFSUBCLASS = "GetOilTypeForT0004Tbl"                  'SUBクラス名
            CS0011LOGWRITE.INFPOSI = "DB:T0004_HORDER Select"           '
            CS0011LOGWRITE.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWRITE.TEXT = ex.ToString()
            CS0011LOGWRITE.MESSAGENO = C_MESSAGE_NO.DB_ERROR
    ...
    ''' <summary>
    ''' 配送受注ＤＢを検索し、届先、品名、数量を取得（救済処置）
    ''' </summary>
    ''' <param name="IO_ROW">検索対象行</param>
    ''' <param name="O_RTN">可否判定</param>
    ''' <remarks></remarks>
    Protected Sub GetTodokeForT0004Tbl(ByVal IO_ROW As DataRow,
                                        ByRef O_TODOKECODE As String,
                                        ByRef O_PRODUCTCODE As String,
                                        ByRef O_PRODUCT1 As String,
                                        ByRef O_PRODUCT2 As String,
                                        ByRef O_SURYO As Double,
                                        ByRef O_RTN As String)
        ...
        Try
            ...
            Using SQLcon As SqlConnection = CS0050Session.getConnection
                ...
                SQLStr =
                           " SELECT isnull(rtrim(TODOKECODE),'')    as TODOKECODE " _
                         & "       ,isnull(rtrim(PRODUCTCODE),'')  as PRODUCTCODE " _
                         & "       ,isnull(rtrim(PRODUCT1),'')     as PRODUCT1 " _
                         & "       ,isnull(rtrim(PRODUCT2),'')    as PRODUCT2 " _
                         & "       ,isnull(SURYO,0)               as SURYO " _
                         & "   FROM T0004_HORDER "
                ...
                Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                    ...
                    While SQLdr.Read
                        '○出力編集
                        O_TODOKECODE = SQLdr("TODOKECODE")
                        O_PRODUCTCODE = SQLdr("PRODUCTCODE")
                        O_PRODUCT1 = SQLdr("PRODUCT1")
                        O_PRODUCT2 = SQLdr("PRODUCT2")
                        O_SURYO = SQLdr("SURYO")
                        O_RTN = C_MESSAGE_NO.NORMAL
                        Exit While
                    End While
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "T0004_HORDER Select")
            CS0011LOGWRITE.INFSUBCLASS = "GetTodokeForT0004Tbl"                  'SUBクラス名
            CS0011LOGWRITE.INFPOSI = "DB:T0004_HORDER Select"           '
            CS0011LOGWRITE.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWRITE.TEXT = ex.ToString()
            CS0011LOGWRITE.MESSAGENO = C_MESSAGE_NO.DB_ERROR
    ...
    ''' <param name="O_RTN">可否判定</param>
    ''' <remarks></remarks>
    Protected Sub GetOilTypeForT0004Tbl(ByRef IO_ROW As DataRow, ByRef O_OILTYPE As String, ByRef O_RTN As String)
        ...
        Try
            ...
            Using SQLcon As SqlConnection = CS0050Session.getConnection
                ...
                Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                    ...
                    While SQLdr.Read
                        '○出力編集
                        If IO_ROW("TORICODE") = "" Then
                            IO_ROW("TORICODE") = SQLdr("TORICODE")
                        End If
                        If IO_ROW("SHUKABASHO") = "" Then
                            IO_ROW("SHUKABASHO") = SQLdr("SHUKABASHO")
                        End If
                        O_OILTYPE = SQLdr("SHUKABASHO")
                        O_RTN = C_MESSAGE_NO.NORMAL
                        Exit While
                    End While
    ...
    Protected Sub CodeToCode(ByVal I_FIELD As String, ByVal I_VALUE As String, ByRef O_CONV As String, ByRef O_RTN As String)
        ...
        If Not String.IsNullOrEmpty(I_VALUE) Then
            ...
            Select Case I_FIELD
                ...
                Case "GSHABANSHATANK2"
                    Dim param As New Hashtable
                    param.Item(GRIS0005LeftBox.C_PARAMETERS.LP_COMPANY) = work.WF_SEL_CAMPCODE.Text
                    param.Item(GRIS0005LeftBox.C_PARAMETERS.LP_LIST) = work.CreateSHABANK2G2(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text)
                    leftview.CodeToName(LIST_BOX_CLASSIFICATION.LC_EXTRA_LIST, I_VALUE, O_CONV, O_RTN, param)
    ...
    Protected Sub CodeToName(ByVal I_FIELD As String,
                               ...
                               ByRef O_RTN As String,
                               Optional ByVal args As Hashtable = Nothing)
        ...
        If Not String.IsNullOrEmpty(I_VALUE) Then
            Select Case I_FIELD
                ...
                Case "TODOKECODE"
                    ...
                    Else
                        leftview.CodeToName(LIST_BOX_CLASSIFICATION.LC_DISTINATION, I_VALUE, O_TEXT, O_RTN, args)
                    End If
    ...
    Protected Sub AddColumnForT0005WKTbl()
        ...
        T0005WKtbl.Columns.Add("ERR", GetType(String))
        ...
        Try
            ...
            Using SQLcon As SqlConnection = CS0050Session.getConnection
                ...
                Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                    ...
                    PARA1.Value = IO_ROW("CAMPCODE")
                    If IO_ROW("TODOKECODE") = "" Then
                        PARA2.Value = IO_ROW("wTODOKECODE")
                    Else
                        PARA2.Value = IO_ROW("TODOKECODE")
                    End If
                    PARA3.Value = IO_ROW("SHIPORG")

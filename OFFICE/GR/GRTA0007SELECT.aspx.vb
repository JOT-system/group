...
Public Class GRTA0007SELECT
    ...
    Protected Sub WF_ButtonDO_V2_Click()
        ...
        If Not Master.MAPpermitcode = C_PERMISSION.INVALID Then
            ...
            CS0007CheckAuthority.MAPID = GRTA0007WRKINC_V2.MAPID
            ...
            If isNormal(CS0007CheckAuthority.ERR) Then
                ...
                If Master.MAPpermitcode < C_PERMISSION.REFERLANCE Then
                    Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRTA0007WRKINC_V2.MAPID)
                    Exit Sub
                End If
            Else
                Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRTA0007WRKINC_V2.MAPID)
    ...
    Protected Sub SetMapValue()
        ...
        If Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.MENU Then                                                   'メニューからの画面遷移
            ...
            '○画面項目設定（変数より）処理
            SetInitialValue()
        ElseIf Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.TA0007 OrElse
               Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.TA0007_V2 Then                                              '実行画面からの画面遷移
            '■■■ 実行画面からの画面遷移 ■■■
            '画面設定
        ...
        Master.Btn_Control(work.WF_SEL_CAMPCODE.Text, GRTA0007WRKINC.MAPIDS, "WF_ButtonDO", WW_DUMMY)
        Master.Btn_Control(work.WF_SEL_CAMPCODE.Text, GRTA0007WRKINC.MAPIDS, "WF_ButtonDO_V2", WW_DUMMY)

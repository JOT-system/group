...
Public Class GRT00004HORDER
                    ...
                    If CompareOrder(T00004row, T00004tbl.Rows(j)) Then
                        If T00004tbl.Rows(j)("DELFLG") <> C_DELETE_FLG.DELETE Then
                            ...
                            If findSeq = True Then
                                T00004tbl.Rows(j)("HIDDEN") = "1"
                            ElseIf T00004tbl.Rows(j)("SEQ") = "01" Then
                                findSeq = True
                            End If
                            Dim wkVal As Double
                            If Double.TryParse(T00004tbl.Rows(j)("SURYO"), wkVal) Then
                                SURYO_SUM += wkVal
                            End If
                            If Double.TryParse(T00004tbl.Rows(j)("JSURYO"), wkVal) Then
                                JSURYO_SUM += wkVal
                            End If
    ...
    Protected Sub CODENAME_set(ByRef T00004row As DataRow)
        ...
        Else
            ...
            If T00004row("HTANI") = "" Then
                'List配送単位
                Dim product = JOTMASTER.GetProduct(T00004row("PRODUCTCODE"))
        ...
        '出荷場所名称
        If koueiFlag AndAlso T00004row("SHUKABASHO").ToString.Contains("!") Then
            '光英オーダーかつコードに『!』が含まれる場合は名称取得しない
            '※JOTコード変換前の光英コード・名称が設定されている為
        Else
        ...
        Else
            ...
            CODENAME_get("TODOKECODE", T00004row("TODOKECODE"), T00004row("TODOKECODENAME"), WW_DUMMY)
        End If
        ...
        '届先追加情報
        Dim datTodoke As JOT_MASTER.TODOKESAKI = JOTMASTER.GetTodoke(T00004row("TODOKECODE"))
        ...
        If Not koueiFlag Then
            '従業員追加情報
            Dim datStaff As JOT_MASTER.STAFF = JOTMASTER.GetStaff(T00004row("STAFFCODE"))
    ...
    Protected Sub WF_Grid_DBclick()
        ...
        EditOperationText(T00004tbl, True, WW_LINECNT)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub EditTumiokiText(ByRef T00004row As DataRow)
        ...
        If T00004row("TUMIOKIKBN") = "1" Then
            ...
            End If
        Else
            T00004row("TUMIOKI") = String.Empty
        End If
...
#Region "T0004テーブル関連"
    ...
    Private Sub DBselect_T4SELECT()
        ...
        For Each T00004row In T00004tbl.Rows
            ...
            If Date.TryParse(T00004row("ARRIVTIME"), WW_TIME) Then
    ...
    Protected Sub DBupdate_T00004UPDtblget(ByVal O_RTN As String)
        ...
        For Each T00004row In T00004tbl.Rows
            '削除チェックがONの時、削除更新
            If T00004row("ROWDEL") = "1" Then
                If T00004row("OPERATION") <> C_LIST_OPERATION_CODE.ERRORED Then

                    T00004row("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
                    T00004row("DELFLG") = C_DELETE_FLG.DELETE
                    T00004row("HIDDEN") = 1
                End If
            End If
        ...
        For i As Integer = 0 To T00004UPDtbl.Rows.Count - 1
            ...
            If T00004UPDrow("OPERATION") = C_LIST_OPERATION_CODE.ERRORED Then
                For j As Integer = 0 To T00004UPDtbl.Rows.Count - 1
...
#Region "T0004テーブル入力関連"
            ...
            If T00004INProw("OPERATION") <> C_LIST_OPERATION_CODE.NODATA AndAlso
                ...
                For Each T00004TBLrow In T00004tbl.Rows
                    If T00004TBLrow("JXORDERID") = T00004INProw("JXORDERID") Then
                        ...
                        T00004INProw("STTIME") = T00004TBLrow("STTIME")
                    End If
                Next
            End If
        Next
                    ...
                    If T00004tbl.Rows(j)("GSHABAN") = T00004INProw("GSHABAN") AndAlso
                       ...
                       T00004tbl.Rows(j)("DELFLG") <> "1" Then
                        ...
                        Else
                            ...
                            T00004INProw("HIDDEN") = "1"   '非表示
                            T00004INProw("SELECT") = "0"   '明細表示対象外
                        End If
                    Else
    ...
    Protected Sub INPtbl_CHEK(ByRef O_RTNCODE As String)
        ...
        For i As Integer = 0 To T00004INPtbl.Rows.Count - 1
            ...
            If T00004INProw("STORICODE") = "" Then
                Dim stori = JOTMASTER.GetSTori(WF_TORICODE.Text)
                If Not IsNothing(stori) Then
                    WF_STORICODE.Text = stori.Item1
                    WF_STORICODE_TEXT.Text = stori.Item2
                End If
            End If
            ...
            If WW_CHKFLG = "ON" Then
                ...
                If isNormal(WW_CS0024FCHECKERR) Then
                    ...
                    Else
                        ...
                        CODENAME_get("SHUKABASHO", T00004INProw("SHUKABASHO"), WW_TEXT, WW_RTN_SW, work.createDistinationParam(work.WF_SEL_CAMPCODE.Text, T00004INProw("SHIPORG"), "", "2", False))
            ...
            If WW_CHKFLG = "ON" Then
                ...
                If isNormal(WW_CS0024FCHECKERR) Then
                    ...
                    Else
                        ...
                        '○LeftBox存在チェック
                        CODENAME_get("TODOKECODE", T00004INProw("TODOKECODE"), WW_TEXT, WW_RTN_SW, work.createDistinationParam(work.WF_SEL_CAMPCODE.Text, T00004INProw("SHIPORG"), "", "1", False))
            ...
            If Not String.IsNullOrEmpty(T00004INProw("JXORDERID")) AndAlso
               T00004INProw("JXORDERSTATUS") = JXORDER_WARNING Then

                WW_ERRLIST.Add(C_MESSAGE_NO.WORNING_RECORD_EXIST)
            End If
    ...
    Private Sub FileSaveDisplayInput()
        ...
        If T00004tbl Is Nothing OrElse T00004tbl.Rows.Count = 0 Then
            Return
        End If
                ...
                For Each row In T00004tbl.Rows
                    If row("LINECNT") = i Then
                        ...
                        Else
                            row(fieldId.Key) = "0"
                        End If
                        'If before <> row(fieldId.Key) Then
    ...
    Protected Sub WF_Field_DBClick()
        ...
        Else
            ...
            Select Case WW_FIELD
                ...
                Case "TODOKECODE"                            '届先
                    prmData = work.createDistinationParam(work.WF_SEL_CAMPCODE.Text, WF_SHIPORG.Text, "", "1", False)
                Case "SHUKABASHO"                            '出荷場所
                    prmData = work.createDistinationParam(work.WF_SEL_CAMPCODE.Text, WF_SHIPORG.Text, "", "2", False)
                Case "HTANI"                             '配送単位
    ...
    Protected Sub WF_ButtonSel_Click()
        ...
        Else
            ...
            If WF_FIELD_REP.Value = "PRODUCTCODE" Then
                Dim datProduct As JOT_MASTER.PRODUCT = JOTMASTER.GetProduct(WW_SelectValue)
...
#Region "UPLOADファイル"
    ...
    Protected Sub UPLOAD_EXCEL()
        ...
        '■■■ UPLOAD_XLSデータ取得 ■■■
        If work.WF_SEL_CAMPCODE.Text = GRT00004WRKINC.C_CAMPCODE.NJS Then
    ...
    Protected Sub XLStoINPtbl(ByRef O_RTN As String)
        ...
        For Each uploadRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            '■■■ 数量ゼロは読み飛ばし ■■■
            If Val(T00004INProw("JSURYO")) = 0 AndAlso Val(T00004INProw("SURYO")) = 0 Then
                Continue For
            End If
            ...
            If WW_COLUMNS.IndexOf("PRODUCTCODE") < 0 Then
                If Not String.IsNullOrEmpty(T00004INProw("OILTYPE")) AndAlso
                    ...
                    T00004INProw("PRODUCTCODE") = T00004INProw("CAMPCODE").ToString.PadLeft(2, "0") & T00004INProw("OILTYPE").ToString.PadLeft(2, "0") & T00004INProw("PRODUCT1").ToString.PadLeft(2, "0") & T00004INProw("PRODUCT2").ToString.PadLeft(5, "0")
                End If
            ElseIf Not String.IsNullOrEmpty(T00004INProw("PRODUCTCODE")) AndAlso T00004INProw("PRODUCTCODE").ToString.Length = 11 Then
                '油種未存在は品名コードから作成
                If WW_COLUMNS.IndexOf("OILTYPE") < 0 Then
            ...
            'JXオーダーID項目補填
            If String.IsNullOrEmpty(T00004INProw("JXORDERID")) Then
                'JXオーダーIDが空 かつ
                '取引先がJX or COSMO　かつ
                '油種が石油
                If (T00004INProw("TORICODE") = GRT00004WRKINC.C_TORICODE_JX OrElse
                    T00004INProw("TORICODE") = GRT00004WRKINC.C_TORICODE_COSMO) AndAlso
                    T00004INProw("OILTYPE") = GRT00004WRKINC.C_PRODUCT_OIL Then
                    ComplementJxOrderID(T00004INProw)
                End If
            End If
            'JXオーダーの場合はオーダーステータス設定
        ...
        'JXオーダーID項目補填が実行された場合は解放
        If Not IsNothing(KOUEIMNG) Then
            KOUEIMNG.Dispose()
            KOUEIMNG = Nothing
        End If
        If Not IsNothing(KOUEIMASTER) Then
            KOUEIMASTER.Dispose()
            KOUEIMASTER = Nothing
        End If
    End Sub
        ...
        Using jsrCvt As JSRCODE_MASTER = New JSRCODE_MASTER()
            ...
            For Each uploadRow As DataRow In uploadData
                ...
                If String.IsNullOrEmpty(datTodoke.TODOKECODE) Then
                    'T00004INProw("TODOKECODE") = uploadRow("TODOKECODE")
                    T00004INProw("TODOKECODE") = "!" & datTodoke.JSRTODOKECODE & "!"
                Else
                    T00004INProw("TODOKECODE") = datTodoke.TODOKECODE
                End If
                T00004INProw("TODOKETIME") = uploadRow("WORKTIME3")
                ...
                If String.IsNullOrEmpty(datProduct.PRODUCTCODE) Then
                    'T00004INProw("PRODUCTCODE") = uploadRow("PRODUCTCODE")
                    T00004INProw("PRODUCTCODE") = "!" & datProduct.JSRPRODUCT & "!"
                Else
                ...
                T00004INProw("JDAISU") = "0"
                '契約番号
                If WW_COLUMNS.Contains("CONTRACTNO") Then
                    T00004INProw("REMARKS1") = uploadRow("CONTRACTNO")
                Else
                    T00004INProw("REMARKS1") = ""
                End If
                '社内備考
                If WW_COLUMNS.Contains("SYANAINOTES") Then
                    T00004INProw("REMARKS2") = uploadRow("SYANAINOTES")
                Else
                    T00004INProw("REMARKS2") = ""
                End If
                '社外備考
                If WW_COLUMNS.Contains("SYAGAINOTES") Then
                    T00004INProw("REMARKS3") = uploadRow("SYAGAINOTES")
                Else
                    T00004INProw("REMARKS3") = ""
                End If
                T00004INProw("REMARKS4") = ""
                ...
                '同一出庫日最新トリップ№取得
                Dim latestTrip As Integer = GetLatestTripNo(T00004INProw)

                Dim tripCnt As Integer = latestTrip + 1
                T00004INProw("TRIPNO") = tripCnt.ToString("000")
                ...
                If Not IsNothing(WW_SHUKODATE) AndAlso Not IsNothing(WW_KIKODATE) Then
                    ...
                    For i As Integer = 1 To days
                        ...
                        '積置区分名称編集（積置→積配）
                        EditTumiokiText(T00004INPAddrow)
                        T00004INPtbl.Rows.Add(T00004INPAddrow)
                    Next
                End If
    ...
    Protected Function GetLatestTripNo(ByVal row As DataRow) As Integer
        ...
        Dim t4inp = T00004INPtbl.AsEnumerable.
                    ...
                    OrderBy(Function(x) x.Item("TODOKETIME"))
        Dim t4tbl = T00004tbl.AsEnumerable.
                    Where(Function(x) x.Item("SHIPORG") = work.WF_SEL_SHIPORG.Text AndAlso
                                      x.Item("DELFLG") = C_DELETE_FLG.ALIVE AndAlso
                                      x.Item("SHUKODATE") = row("SHUKODATE") AndAlso
                                      x.Item("GSHABAN") = row("GSHABAN") AndAlso
        ...
        Dim t4 = t4tbl.Union(t4inp)
        If t4.Count > 0 Then
            '届先時刻順で最終のTRIPNOを取得
            tripNo = Val(t4.Last.Item("TRIPNO"))
        End If
    ...
    Protected Sub UPLOAD_KOUEI(ByVal I_FILES As List(Of FileInfo), ByRef O_RTN As String)
        ...
        For Each file In I_FILES
            ...
            If KOUEIMNG.ReadCSV(file, WW_ERR_MES) <> True Then
                If KOUEIMNG.ERR = C_MESSAGE_NO.ALREADY_UPDATE_ERROR Then
                    'データ重複
                    rightview.AddErrorReport(WW_ERR_MES)
                    Master.Output(C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, C_MESSAGE_TYPE.ABORT)
                Else
                    Master.Output(KOUEIMNG.ERR, C_MESSAGE_TYPE.ABORT, "光英及び石油輸送システム担当へ連絡")
                End If
                O_RTN = KOUEIMNG.ERR
                Exit Sub
            End If
    ...
    Protected Sub MC006tbl_Edit(ByRef O_RTN As String)
        ...
        Using SQLcon As SqlConnection = CS0050SESSION.getConnection
            ...
            MC006UPDATE.Update(KOUEIMNG.GetOrder(), KOUEIMASTER, JOTMASTER)
    ...
    Protected Sub KOUEItoINPtbl(ByRef O_RTN As String)
        ...
        Dim WW_INDEX As Integer = 0
        Dim tripList As List(Of DataRow) = New List(Of DataRow)
        ...
        For Each trip In tripAll
            ...
            For Each csv In trip
                ...
                Select Case Int32.Parse(csvRow.TRIPSEQ)
                    ...
                    Case 99 '終業
                        rowEnd = csvRow
                    Case Else
                        If (csvRow.OWNERINFO = "|基地" OrElse csvRow.OWNERINFO = "|車庫") AndAlso rowTodoke.Count = 0 Then
                            '出荷
                            rowShip.Add(csvRow)
                        ElseIf csvRow.COURSEBASEKEY <> "" Then
                            '配送
            ...
            If IsNothing(rowStart) OrElse rowShip.Count = 0 OrElse rowTodoke.Count = 0 OrElse IsNothing(rowKiko) OrElse IsNothing(rowEnd) Then
                Master.Output(C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, C_MESSAGE_TYPE.ABORT, "光英及び石油輸送システム担当へ連絡")
            ...
            Dim WW_SHUKODATE As Date
            If Not DateTime.TryParseExact(rowTodoke.First.KIJUNDATE, "yyyyMMdd", Nothing, Nothing, WW_SHUKODATE) Then
                Master.Output(C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, C_MESSAGE_TYPE.ABORT, "光英及び石油輸送システム担当へ連絡")
            ...
            Else
                ...
                Dim tmpStaff = KOUEIMASTER.GetStaff2No(rowStart.KOUEITYPE, splitStaff(0))
            ...
            Else
                ...
                Dim tmpSubStaff = KOUEIMASTER.GetStaff2No(rowStart.KOUEITYPE, splitSubStaff(0))
            ...
            If String.IsNullOrEmpty(rowStart.STTIME) Then
                datSttime = ""
            Else
                datSttime = (Math.Floor(rowStart.STTIME / 60)).ToString("00") & ":" & (rowStart.STTIME Mod 60).ToString("00")
            End If
            '2020/08/27 ADD END
            ...
            If Not IsNothing(tmpShukabasho) Then
                ...
                datShukabasho = JOTMASTER.GetTodoke(todokeCode)
                If IsNothing(datShukabasho) Then
                    '届先マスタ未存在
                    '事前に登録処理が動くためこのケースはないハズ
                    datShukabasho = New JOT_MASTER.TODOKESAKI With {
            ...
            Dim shaban = WF_ListKSHABAN.Items.FindByValue(rowStart.SHABANB)
            ...
            For Each row In rowTodoke
                ...
                '届先マスタ取得（届先）
                Dim datTodoke As JOT_MASTER.TODOKESAKI = Nothing
                '光英届先マスタ取得
                ...
                If Not IsNothing(tmpTodoke) Then
                    ...
                    datTodoke = JOTMASTER.GetTodoke(todokeCode)
                    If IsNothing(datTodoke) Then
                        '届先マスタ未存在
                        '事前に登録処理が動くためこのケースはないハズ
                        datTodoke = New JOT_MASTER.TODOKESAKI With {
                            .TODOKECODE = "!" & todokeCode & "!",
                            .NAMES = "★マスタエラー★"
                        }
                ...
                For Each kproduct In productList
                    ...
                    Dim datProduct As JOT_MASTER.PRODUCT = JOTMASTER.GetKProduct(kproduct.PRODUCTCD)
                    If IsNothing(datProduct) Then
                        '品名マスタ未存在
                        datProduct = New JOT_MASTER.PRODUCT With {
                            .OILTYPE = GRT00004WRKINC.C_PRODUCT_OIL,
                            .PRODUCT1 = GRT00004WRKINC.C_PRODUCT_WHITEOIL,
                            .PRODUCT2 = "",
                    ...
                    T00004INProw("DROPNO") = dropCnt.ToString("000")
                    ...
                    '入力テーブル追加
                    tripList.Add(T00004INProw)

                Next

                'If dropStatus <> 0 Then
                '    '明細で警告発生時は同一ドロップ全ての明細に反映
                '    For Each seqRow As DataRow In tripList

                '        seqRow("JXORDERSTATUS") = JXORDER_WARNING
                '    Next
                'End If
            Next
            T00004INPtbl.Merge(tripList.CopyToDataTable)
    ...
    Sub ErrMsgEditForKOUEI(ByRef order As GRW0001KOUEIORDER.KOUEI_ORDER, ByVal I_FIELDNM As String, I_MSG As String, ByVal I_ERRCD As String)
        ...
        WW_ERR_MES = WW_ERR_MES & ControlChars.NewLine & "  --> 車番   =" & order.SHABANCD & " , "
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub ComplementJxOrderID(ByRef T00004row As DataRow)

        '光英タイプ
        Dim koueiType As String = String.Empty
        If T00004row("TORICODE") = GRT00004WRKINC.C_TORICODE_JX Then
            koueiType = GRW0001KOUEIORDER.KOUEITYPE_PREFIX.JXTG
        ElseIf T00004row("TORICODE") = GRT00004WRKINC.C_TORICODE_COSMO Then
            koueiType = GRW0001KOUEIORDER.KOUEITYPE_PREFIX.COSMO
        End If

        '出庫日
        Dim tmpDate As Date
        Date.TryParse(T00004row("SHUKODATE"), tmpDate)
        Dim shukodate As String = tmpDate.ToString("yyyyMMdd")
        '業務車番
        Dim gshaban As String = T00004row("GSHABAN")
        'トリップ
        Dim trip As Integer = Val(T00004row("TRIPNO"))

        '光英車番取得
        Dim rshaban As String = String.Empty
        Dim shaban As String = String.Empty
        Dim status As String = String.Empty

        If gshaban.Contains("!") Then
            '光英変換エラー時は光英コードのまま（前後の「!」は外す）
            shaban = gshaban.Replace("!", "")
            status = JXORDER_WARNING
        Else
            '業務車番から光英車番を取得
            Dim item = WF_ListKSHABAN.Items.FindByText(gshaban)
            If Not IsNothing(item) Then
                shaban = item.Value
            End If
        End If

        If Not String.IsNullOrEmpty(shaban) Then

            Dim tmpOrderId As String = String.Format("{0}{1}{2}{3}", koueiType.ToUpper.Substring(0, 1), shukodate, shaban, trip)
            Dim targetKey = tmpOrderId & C_VALUE_SPLIT_DELIMITER & GRW0001KOUEIORDER.TRIPSEQ_TYPE.START
            If IsNothing(KOUEIMNG) Then
                '光英オーダー管理
                KOUEIMNG = New GRW0001KOUEIORDER With {
                    .CAMPCODE = work.WF_SEL_CAMPCODE.Text,
                    .ORGCODE = work.WF_SEL_SHIPORG.Text,
                ...
                If KOUEIMNG.ReadOrder() <> True Then
                    Master.Output(KOUEIMNG.ERR, C_MESSAGE_TYPE.ERR)
                End If
            End If
            Dim KOUEItbl = KOUEIMNG.GetOrder()
            If KOUEItbl.ContainsKey(targetKey) Then
                T00004row("JXORDERID") = tmpOrderId
                T00004row("JXORDERSTATUS") = status
            End If

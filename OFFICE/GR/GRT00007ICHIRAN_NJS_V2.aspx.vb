...
''' </summary>
''' <remarks></remarks>
Public Class GRT00007ICHIRAN_NJS_V2
    Inherits Page
    ...
    Private T0007UPDATE As New GRT0007UPDATE_V2                     '勤怠更新
    ...
    Protected Sub GridViewInitialize()
        ...
        Dim TBLview = From a In T0007tbl.AsEnumerable
                      Where a.Item("HIDDEN") = 0 AndAlso a.Item("LINECNT") >= 1 AndAlso a.Item("LINECNT") <= WW_GridCnt
                      Order By a.Item("LINECNT")
        'Using TBLview As DataView = New DataView(T0007tbl)
        ...
        If TBLview.Count = 0 Then
            CS0013ProfView.SRCDATA = T0007tbl.Clone
        Else
        ...
        If Not String.IsNullOrEmpty(work.WF_T7SEL_XMLsaveTmp.Text) Then
            System.IO.File.Delete(work.WF_T7SEL_XMLsaveTmp.Text)
        End If
        If Not String.IsNullOrEmpty(work.WF_T7SEL_XMLsavePARM.Text) Then
            System.IO.File.Delete(work.WF_T7SEL_XMLsavePARM.Text)
        End If
    ...
    Protected Sub WF_ButtonExtract_Click()
        ...
        If IsDate(WF_WORKDATE.Text) Then
            WF_WORKDATE.Text = CDate(WF_WORKDATE.Text).ToString("yyyy/MM/dd")
        ...
        Dim t7tblH = From x In T0007tbl.AsEnumerable()
                     Where x.Item("HDKBN") = "H"
                     Order By x.Item("STAFFCODE"), x.Item("WORKDATE"), x.Item("RECODEKBN"), x.Item("STDATE"), x.Item("STTIME"), x.Item("HDKBN") Descending
        Dim t7tblD = From x In T0007tbl.AsEnumerable()
                     Where x.Item("HDKBN") = "D"
                     Order By x.Item("STAFFCODE"), x.Item("WORKDATE"), x.Item("RECODEKBN"), x.Item("STDATE"), x.Item("STTIME"), x.Item("HDKBN") Descending
        ...
        For Each WW_T0007row In t7tblH
            ...
            End If
        Next
        If t7tblH.Any AndAlso t7tblD.Any Then
            T0007tbl = t7tblH.Union(t7tblD).CopyToDataTable
    ...
    Protected Sub WF_ButtonNIPPO_Click()
        ...
        WW_Filter = "SELECT = '1' and HDKBN = 'H' and (STATUS like '*日報*' or STATUS like '*休憩*') and RECODEKBN = '0' and WORKDATE >= #" & WW_STDATE & "# and WORKDATE <= #" & WW_ENDDATE & "#"
        CS0026TblSort.TABLE = T0007tbl
        CS0026TblSort.FILTER = WW_Filter
        CS0026TblSort.SORTING = "SELECT, STAFFCODE, WORKDATE, HDKBN DESC, STDATE, STTIME, ENDDATE, ENDTIME, RECODEKBN"
        WW_T0007SELtbl = CS0026TblSort.sort()
        ...
        For Each T0007INProw As DataRow In WW_T0007SELtbl.Rows
            ...
            If T0007INProw("STATUS") Like "*日報*" Or T0007INProw("STATUS") Like "*休憩*" Then
        ...
        For i As Integer = T0007tbl.Rows.Count - 1 To 0 Step -1
            ...
            If T7row("WORKDATE") >= WW_STDATE And T7row("WORKDATE") <= WW_ENDDATE Then
                If T7row("STATUS") Like "*日報*" Or T7row("STATUS") Like "*休憩*" Then
        ...
        NIPPOget_T7Format("NEW", WW_T0007tbl, T0005tbl)
        'End Using
        ...
        '残業計算
        '----------------------------------------------
        WW_Filter = "SELECT = '1' and STATUS <> '' and RECODEKBN = '0' and WORKDATE >= #" & WW_STDATE & "# and WORKDATE <= #" & WW_ENDDATE & "#"
        CS0026TblSort.TABLE = T0007tbl
        CS0026TblSort.FILTER = WW_Filter
        CS0026TblSort.SORTING = "SELECT, STAFFCODE, WORKDATE, HDKBN DESC, STDATE, STTIME, ENDDATE, ENDTIME, RECODEKBN"
        WW_T0007SELtbl = CS0026TblSort.sort()
    ...
    Protected Sub WF_ButtonUPDATE_Click()
        ...
        Dim WW_T00013tbl As DataTable = Nothing
        Try
            '○ 休憩・配送時間DB出力編集
            T0013UPDATE.AddColumnT0013UPDtbl(WW_T00013tbl)
            UpdTableEdit(WW_T0007UPDtbl, WW_T00013tbl, WW_DATENOW)
            ...
            For Each T13row As DataRow In WW_T00013tbl.Rows
                T0013UPDATE.UpdateT0013(T13row, WW_ERR)
        ...
        Catch ex As Exception
            CS0011LOGWRITE.INFSUBCLASS = "WF_ButtonUPDATE_Click"        'SUBクラス名
            CS0011LOGWRITE.INFPOSI = "DB:INSERT T0013_TIMEMANAGE"       '
            CS0011LOGWRITE.NIWEA = "A"                                  '
            CS0011LOGWRITE.TEXT = ex.ToString()
            CS0011LOGWRITE.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWRITE.CS0011LOGWrite()                             'ログ出力
    ...
    Protected Sub UpdTableEdit(ByVal I_TABLE As DataTable, ByRef O_TABLE As DataTable, ByVal I_NOW As DateTime)
        ...
        For Each I_ROW As DataRow In I_TABLE.Rows
            ...
            O_ROW("WORKKBN") = "BB"                                                             '作業区分

            For i As Integer = 1 To 10
                Dim WW_T13STTIME As String = "T13BBSTTIME" & i.ToString("00")
                Dim WW_T13ENDTIME As String = "T13BBENDTIME" & i.ToString("00")
                Dim WW_STTIME As String = "STTIME" & i.ToString("00")
                Dim WW_ENDTIME As String = "ENDTIME" & i.ToString("00")

                '開始時刻
                If IsDate(I_ROW(WW_T13STTIME)) Then
                    O_ROW(WW_STTIME) = I_ROW(WW_T13STTIME)
                Else
                    O_ROW(WW_STTIME) = "00:00"
                End If

                '終了時刻
                If IsDate(I_ROW(WW_T13ENDTIME)) Then
                    O_ROW(WW_ENDTIME) = I_ROW(WW_T13ENDTIME)
                Else
                    O_ROW(WW_ENDTIME) = "00:00"
                End If
            Next

            O_ROW("TTLTIME") = I_ROW("T13BBTTLTIME")                                            '合計時間
            ...
            '----------------------------------------------------------
            '配送
            O_ROW = O_TABLE.NewRow

            O_ROW("CAMPCODE") = I_ROW("CAMPCODE")                                               '会社コード
            O_ROW("TAISHOYM") = I_ROW("TAISHOYM")                                               '対象年月
            O_ROW("STAFFCODE") = I_ROW("STAFFCODE")                                             '従業員コード
            O_ROW("WORKDATE") = I_ROW("WORKDATE")                                               '勤務年月日
            O_ROW("WORKKBN") = "G1"                                                             '作業区分

            For i As Integer = 1 To 10
                Dim WW_T13STTIME As String = "T13G1STTIME" & i.ToString("00")
                Dim WW_T13ENDTIME As String = "T13G1ENDTIME" & i.ToString("00")
                Dim WW_STTIME As String = "STTIME" & i.ToString("00")
                Dim WW_ENDTIME As String = "ENDTIME" & i.ToString("00")
            ...
            O_ROW("TTLTIME") = I_ROW("T13G1TTLTIME")                                            '合計時間
            If O_ROW("TTLTIME") = 0 Then
                O_ROW("DELFLG") = C_DELETE_FLG.DELETE                                           '削除フラグ
            Else
                O_ROW("DELFLG") = C_DELETE_FLG.ALIVE                                            '削除フラグ
    ...
    Protected Sub WF_ButtonEND_Click()
        ...
        ''■■■ 画面戻先URL取得 ■■■
        'CS0017RETURNURLget.MAPID = GRT00007WRKINC_V2.MAPIDINJS
    ...
    Protected Sub WF_ButtonSel_Click()
        ...
        Select Case WF_FIELD.Value
            ...
            Case "WF_WORKDATE"
                '出庫日 
                WF_WORKDATE.Text = CDate(WW_SelectValues(0)).ToString("yyyy/MM/dd")
                WF_WORKDATE.Focus()
        End Select
    ...
    '            CS0049UPROFview.MAPID = GRT00007WRKINC_V2.MAPIDINJS
    ...
    Private Sub GRID_INITset()
        ...
        Try
            ...
            Dim SQLStr As String =
               ...
               & "       isnull((select rtrim(M1.NAMES) from M0001_CAMP M1 " _
               & "               where M1.CAMPCODE    = @CAMPCODE " _
               & "               and   M1.STYMD      <= @NOW " _
               & "               and   M1.ENDYMD     >= @NOW " _
               & "               and   M1.DELFLG     <> '1' " _
               & "       ),'') as CAMPNAMES , " _
               & "       format(CAL.WORKINGYMD, 'yyyy/MM') as TAISHOYM , " _
               ...
               & "       isnull((select rtrim(M2M.NAMES) from M0002_ORG M2M " _
               & "               where M2M.CAMPCODE   = @CAMPCODE " _
               ...
               & "       isnull((select rtrim(M2H.NAMES) from M0002_ORG M2H " _
               & "               where M2H.CAMPCODE   = @CAMPCODE " _
               ...
               & "       isnull((select rtrim(F8.VALUE1) from MC001_FIXVALUE F8 " _
               & "               where F8.CAMPCODE    = @CAMPCODE " _
               ...
               & "       else  isnull(rtrim(CAL.WORKINGKBN),'') " _
               & "       end ) as HOLIDAYKBN , " _
               & "       isnull((select rtrim(F9.VALUE1) from MC001_FIXVALUE F9 " _
               & "              where F9.CAMPCODE    = @CAMPCODE " _
               ...
               & "       isnull((select rtrim(F10.VALUE1) from MC001_FIXVALUE F10 " _
               & "               where    F10.CAMPCODE    = @CAMPCODE " _
               ...
               & "       isnull((select rtrim(F11.VALUE1) from MC001_FIXVALUE F11 " _
               & "               where    F11.CAMPCODE    = @CAMPCODE " _
               ...
               & "       isnull((select rtrim(F4.VALUE2) from MC001_FIXVALUE F4 " _
               & "               where F4.CAMPCODE    = @CAMPCODE " _
               ...
               & "       isnull((select B4.WORKINGH from MB004_WORKINGH B4 " _
               & "       where B4.CAMPCODE    = @CAMPCODE " _
               ...
               & "       isnull(A.TOKUKYUMUKYUNISSU,0) as TOKUKYUMUKYUNISSU , " _
               & "       isnull(A.TOKUKYUMUKYUNISSUCHO,0) as TOKUKYUMUKYUNISSUCHO , " _
               & "       isnull(A.ROSAIYUKYNIUSSU,0) as ROSAIYUKYNIUSSU , " _
               & "       isnull(A.ROSAIYUKYNIUSSUCHO,0) as ROSAIYUKYNIUSSUCHO , " _
               & "       isnull(A.KOKANGOYUKYUNISSU,0) as KOKANGOYUKYUNISSU , " _
               & "       isnull(A.KOKANGOYUKYUNISSUCHO,0) as KOKANGOYUKYUNISSUCHO , " _
               & "       isnull(A.KOKANGOMUKYUNISSU,0) as KOKANGOMUKYUNISSU , " _
               & "       isnull(A.KOKANGOMUKYUNISSUCHO,0) as KOKANGOMUKYUNISSUCHO , " _
               ...
               & "       isnull((select rtrim(format(T05.UPDYMD,'yyyyMMddHHmmss')) from T0005_NIPPO T05 " _
               & "               where T05.CAMPCODE    = @CAMPCODE " _
               ...
               & "       ),'') as T5ENTRYDATE , " _
               & "       isnull((select rtrim(T05.L1KAISO) from T0005_NIPPO T05 " _
               & "               where T05.CAMPCODE    = @CAMPCODE " _
               & "               and   T05.STAFFCODE   = MB.STAFFCODE " _
               & "               and   T05.YMD         = CAL.WORKINGYMD " _
               & "               and   T05.DELFLG     <> '1' " _
               ...
               & "               and   T05.ENTRYDATE   = (select MAX(ENTRYDATE) from T0005_NIPPO where CAMPCODE = @CAMPCODE and STAFFCODE = MB.STAFFCODE and YMD = CAL.WORKINGYMD and DELFLG <> '1' and SEQ = 1) " _
               & "       ),'') as L1KAISO , " _
               & "       isnull((select rtrim(T05.LATITUDE) from T0005_NIPPO T05 " _
               & "               where T05.CAMPCODE    = @CAMPCODE " _
               & "               and   T05.STAFFCODE   = MB.STAFFCODE " _
               & "               and   T05.YMD         = CAL.WORKINGYMD " _
               & "               and   T05.DELFLG     <> '1' " _
               ...
               & "               and   T05.ENTRYDATE   = (select MAX(ENTRYDATE) from T0005_NIPPO where CAMPCODE= @CAMPCODE and STAFFCODE = MB.STAFFCODE and YMD = CAL.WORKINGYMD and DELFLG <> '1' and SEQ = 1) " _
               & "       ),'') as LATITUDE , " _
               & "       isnull((select rtrim(T05.LONGITUDE) from T0005_NIPPO T05 " _
               & "               where T05.CAMPCODE    = @CAMPCODE " _
               & "               and   T05.STAFFCODE   = MB.STAFFCODE " _
               & "               and   T05.YMD         = CAL.WORKINGYMD " _
               & "               and   T05.DELFLG     <> '1' " _
               ...
               & "       isnull((select rtrim(format(MAX(T13.UPDYMD),'yyyyMMddHHmmss')) from T0013_TIMEMANAGE T13 " _
               & "               where T13.CAMPCODE    = @CAMPCODE " _
               & "               and   T13.TAISHOYM    = format(CAL.WORKINGYMD, 'yyyy/MM') " _
               & "               and   T13.STAFFCODE   = MB.STAFFCODE " _
               & "               and   T13.WORKDATE    = CAL.WORKINGYMD " _
               & "               and   T13.DELFLG     <> '1' " _
               & "       ),'') as T13ENTRYDATE , " _
               & "       isnull(T131.STTIME01,'00:00') as T13BBSTTIME01 , " _
               & "       isnull(T131.ENDTIME01,'00:00') as T13BBENDTIME01 , " _
               & "       isnull(T131.STTIME02,'00:00') as T13BBSTTIME02 , " _
               & "       isnull(T131.ENDTIME02,'00:00') as T13BBENDTIME02 , " _
               & "       isnull(T131.STTIME03,'00:00') as T13BBSTTIME03 , " _
               & "       isnull(T131.ENDTIME03,'00:00') as T13BBENDTIME03 , " _
               & "       isnull(T131.STTIME04,'00:00') as T13BBSTTIME04 , " _
               & "       isnull(T131.ENDTIME04,'00:00') as T13BBENDTIME04 , " _
               & "       isnull(T131.STTIME05,'00:00') as T13BBSTTIME05 , " _
               & "       isnull(T131.ENDTIME05,'00:00') as T13BBENDTIME05 , " _
               & "       isnull(T131.STTIME06,'00:00') as T13BBSTTIME06 , " _
               & "       isnull(T131.ENDTIME06,'00:00') as T13BBENDTIME06 , " _
               & "       isnull(T131.STTIME07,'00:00') as T13BBSTTIME07 , " _
               & "       isnull(T131.ENDTIME07,'00:00') as T13BBENDTIME07 , " _
               & "       isnull(T131.STTIME08,'00:00') as T13BBSTTIME08 , " _
               & "       isnull(T131.ENDTIME08,'00:00') as T13BBENDTIME08 , " _
               & "       isnull(T131.STTIME09,'00:00') as T13BBSTTIME09 , " _
               & "       isnull(T131.ENDTIME09,'00:00') as T13BBENDTIME09 , " _
               & "       isnull(T131.STTIME10,'00:00') as T13BBSTTIME10 , " _
               & "       isnull(T131.ENDTIME10,'00:00') as T13BBENDTIME10 , " _
               & "       isnull(T131.TTLTIME,'0') as T13BBTTLTIME , " _
               & "       isnull(T132.STTIME01,'00:00') as T13G1STTIME01 , " _
               & "       isnull(T132.ENDTIME01,'00:00') as T13G1ENDTIME01 , " _
               & "       isnull(T132.STTIME02,'00:00') as T13G1STTIME02 , " _
               & "       isnull(T132.ENDTIME02,'00:00') as T13G1ENDTIME02 , " _
               & "       isnull(T132.STTIME03,'00:00') as T13G1STTIME03 , " _
               & "       isnull(T132.ENDTIME03,'00:00') as T13G1ENDTIME03 , " _
               & "       isnull(T132.STTIME04,'00:00') as T13G1STTIME04 , " _
               & "       isnull(T132.ENDTIME04,'00:00') as T13G1ENDTIME04 , " _
               & "       isnull(T132.STTIME05,'00:00') as T13G1STTIME05 , " _
               & "       isnull(T132.ENDTIME05,'00:00') as T13G1ENDTIME05 , " _
               & "       isnull(T132.STTIME06,'00:00') as T13G1STTIME06 , " _
               & "       isnull(T132.ENDTIME06,'00:00') as T13G1ENDTIME06 , " _
               & "       isnull(T132.STTIME07,'00:00') as T13G1STTIME07 , " _
               & "       isnull(T132.ENDTIME07,'00:00') as T13G1ENDTIME07 , " _
               & "       isnull(T132.STTIME08,'00:00') as T13G1STTIME08 , " _
               & "       isnull(T132.ENDTIME08,'00:00') as T13G1ENDTIME08 , " _
               & "       isnull(T132.STTIME09,'00:00') as T13G1STTIME09 , " _
               & "       isnull(T132.ENDTIME09,'00:00') as T13G1ENDTIME09 , " _
               & "       isnull(T132.STTIME10,'00:00') as T13G1STTIME10 , " _
               & "       isnull(T132.ENDTIME10,'00:00') as T13G1ENDTIME10 , " _
               & "       isnull(T132.TTLTIME,'0') as T13G1TTLTIME , " _
               ...
               & "       isnull((select MB3.SEQ from MB002_STAFFORG MB3 " _
               & "               where MB3.CAMPCODE     = @CAMPCODE " _
               & "               and   MB3.STAFFCODE    = MB.STAFFCODE " _
               ...
               & "               where  CAMPCODE                            =   @CAMPCODE                               " _
               & "                 and  format(WORKINGYMD,'yyyy/MM')       >=   @TAISHOYM_F                               " _
               & "                 and  format(WORKINGYMD,'yyyy/MM')       <=   @TAISHOYM_T                               " _
               & "                 and  DELFLG                              <>  '1'                                     " _
               & "              UNION ALL                                                                               " _
               ...
               & "   and   T10.TAISHOYM    = format(CAL.WORKINGYMD, 'yyyy/MM') " _
               & "   and   T10.STAFFCODE   = MB.STAFFCODE " _
               & "   and   T10.WORKDATE    = CAL.WORKINGYMD " _
               & "   and   T10.DELFLG     <> '1' " _
               & "   and   isnull(A.HDKBN,'H')     = 'H' " _
               & "   and   isnull(A.RECODEKBN,'0') = '0' " _
               & " LEFT JOIN T0013_TIMEMANAGE T131 " _
               & "   ON    T131.CAMPCODE    = @CAMPCODE " _
               & "   and   T131.TAISHOYM    = format(CAL.WORKINGYMD, 'yyyy/MM') " _
               & "   and   T131.STAFFCODE   = MB.STAFFCODE " _
               & "   and   T131.WORKDATE    = CAL.WORKINGYMD " _
               & "   and   T131.WORKKBN     = 'BB' " _
               & "   and   T131.DELFLG     <> '1' " _
               & "   and   isnull(A.HDKBN,'H')     = 'H' " _
               & "   and   isnull(A.RECODEKBN,'0') = '0' " _
               & " LEFT JOIN T0013_TIMEMANAGE T132 " _
               & "   ON    T132.CAMPCODE    = @CAMPCODE " _
               & "   and   T132.TAISHOYM    = format(CAL.WORKINGYMD, 'yyyy/MM') " _
               & "   and   T132.STAFFCODE   = MB.STAFFCODE " _
               & "   and   T132.WORKDATE    = CAL.WORKINGYMD " _
               & "   and   T132.WORKKBN     = 'G1' " _
               & "   and   T132.DELFLG     <> '1' " _
               & "   and   isnull(A.HDKBN,'H')     = 'H' " _
               & "   and   isnull(A.RECODEKBN,'0') = '0' " _
               & " WHERE   MB.CAMPCODE     = @CAMPCODE "

            Dim SQLWhere As String = ""
            If work.WF_T7SEL_STAFFKBN.Text <> Nothing Then
                SQLWhere = SQLWhere & " and MB2.STAFFKBN = '" & Trim(work.WF_T7SEL_STAFFKBN.Text) & "' "
            End If
            If work.WF_T7SEL_STAFFCODE.Text <> Nothing Then
                SQLWhere = SQLWhere & " and MB.STAFFCODE = '" & Trim(work.WF_T7SEL_STAFFCODE.Text) & "' "
            End If
            If work.WF_T7SEL_STAFFNAME.Text <> Nothing Then
                SQLWhere = SQLWhere & " and MB2.STAFFNAMES like '%" & Trim(work.WF_T7SEL_STAFFNAME.Text) & "%' "
            End If
            ...
            Dim P_TAISHOYM_F As SqlParameter = SQLcmd.Parameters.Add("@TAISHOYM_F", System.Data.SqlDbType.NVarChar)
            Dim P_TAISHOYM_T As SqlParameter = SQLcmd.Parameters.Add("@TAISHOYM_T", System.Data.SqlDbType.NVarChar)
            ...
            Dim dtAft As Date = dtBef

            P_CAMPCODE.Value = work.WF_T7SEL_CAMPCODE.Text
            P_TAISHOYM_F.Value = dtBef.AddMonths(-1).ToString("yyyy/MM")
            P_TAISHOYM_T.Value = dtAft.AddMonths(1).ToString("yyyy/MM")
            ...
            For i As Integer = 0 To T0007tbl.Rows.Count - 1
                ...
                T0007row("NIGHTTIMETTL") = Val(T0007row("NIGHTTIME")) + Val(T0007row("NIGHTTIMECHO"))
                T0007row("ORVERTIMETTL") = Val(T0007row("ORVERTIME")) + Val(T0007row("ORVERTIMECHO"))
                T0007row("WNIGHTTIMETTL") = Val(T0007row("WNIGHTTIME")) + Val(T0007row("WNIGHTTIMECHO"))
                T0007row("SWORKTIMETTL") = Val(T0007row("SWORKTIME")) + Val(T0007row("SWORKTIMECHO"))
                T0007row("SNIGHTTIMETTL") = Val(T0007row("SNIGHTTIME")) + Val(T0007row("SNIGHTTIMECHO"))
                T0007row("HWORKTIMETTL") = Val(T0007row("HWORKTIME")) + Val(T0007row("HWORKTIMECHO"))
                T0007row("HNIGHTTIMETTL") = Val(T0007row("HNIGHTTIME")) + Val(T0007row("HNIGHTTIMECHO"))
                T0007row("WORKNISSUTTL") = Val(T0007row("WORKNISSU")) + Val(T0007row("WORKNISSUCHO"))
                T0007row("SHOUKETUNISSUTTL") = Val(T0007row("SHOUKETUNISSU")) + Val(T0007row("SHOUKETUNISSUCHO"))
                T0007row("KUMIKETUNISSUTTL") = Val(T0007row("KUMIKETUNISSU")) + Val(T0007row("KUMIKETUNISSUCHO"))
                T0007row("ETCKETUNISSUTTL") = Val(T0007row("ETCKETUNISSU")) + Val(T0007row("ETCKETUNISSUCHO"))
                T0007row("NENKYUNISSUTTL") = Val(T0007row("NENKYUNISSU")) + Val(T0007row("NENKYUNISSUCHO"))
                T0007row("TOKUKYUNISSUTTL") = Val(T0007row("TOKUKYUNISSU")) + Val(T0007row("TOKUKYUNISSUCHO"))
                T0007row("CHIKOKSOTAINISSUTTL") = Val(T0007row("CHIKOKSOTAINISSU")) + Val(T0007row("CHIKOKSOTAINISSUCHO"))
                T0007row("STOCKNISSUTTL") = Val(T0007row("STOCKNISSU")) + Val(T0007row("STOCKNISSUCHO"))
                T0007row("KYOTEIWEEKNISSUTTL") = Val(T0007row("KYOTEIWEEKNISSU")) + Val(T0007row("KYOTEIWEEKNISSUCHO"))
                T0007row("WEEKNISSUTTL") = Val(T0007row("WEEKNISSU")) + Val(T0007row("WEEKNISSUCHO"))
                T0007row("TOKUKYUMUKYUNISSUTTL") = Val(T0007row("TOKUKYUMUKYUNISSU")) + Val(T0007row("TOKUKYUMUKYUNISSUCHO"))
                T0007row("ROSAIYUKYNIUSSUTTL") = Val(T0007row("ROSAIYUKYNIUSSU")) + Val(T0007row("ROSAIYUKYNIUSSUCHO"))
                T0007row("KOKANGOYUKYUNISSUTTL") = Val(T0007row("KOKANGOYUKYUNISSU")) + Val(T0007row("KOKANGOYUKYUNISSUCHO"))
                T0007row("KOKANGOMUKYUNISSUTTL") = Val(T0007row("KOKANGOMUKYUNISSU")) + Val(T0007row("KOKANGOMUKYUNISSUCHO"))
                T0007row("DAIKYUNISSUTTL") = Val(T0007row("DAIKYUNISSU")) + Val(T0007row("DAIKYUNISSUCHO"))
                T0007row("NENSHINISSUTTL") = Val(T0007row("NENSHINISSU")) + Val(T0007row("NENSHINISSUCHO"))
                T0007row("SHUKCHOKNNISSUTTL") = Val(T0007row("SHUKCHOKNNISSU")) + Val(T0007row("SHUKCHOKNNISSUCHO"))
                T0007row("SHUKCHOKNISSUTTL") = Val(T0007row("SHUKCHOKNISSU")) + Val(T0007row("SHUKCHOKNISSUCHO"))
                T0007row("SHUKCHOKNHLDNISSUTTL") = Val(T0007row("SHUKCHOKNHLDNISSU")) + Val(T0007row("SHUKCHOKNHLDNISSUCHO"))
                T0007row("SHUKCHOKHLDNISSUTTL") = Val(T0007row("SHUKCHOKHLDNISSU")) + Val(T0007row("SHUKCHOKHLDNISSUCHO"))
                T0007row("TOKSAAKAISUTTL") = Val(T0007row("TOKSAAKAISU")) + Val(T0007row("TOKSAAKAISUCHO"))
                T0007row("TOKSABKAISUTTL") = Val(T0007row("TOKSABKAISU")) + Val(T0007row("TOKSABKAISUCHO"))
                T0007row("TOKSACKAISUTTL") = Val(T0007row("TOKSACKAISU")) + Val(T0007row("TOKSACKAISUCHO"))
                T0007row("TENKOKAISUTTL") = Val(T0007row("TENKOKAISU")) + Val(T0007row("TENKOKAISUCHO"))
                T0007row("HOANTIMETTL") = Val(T0007row("HOANTIME")) + Val(T0007row("HOANTIMECHO"))
                T0007row("KOATUTIMETTL") = Val(T0007row("KOATUTIME")) + Val(T0007row("KOATUTIMECHO"))
                T0007row("TOKUSA1TIMETTL") = Val(T0007row("TOKUSA1TIME")) + Val(T0007row("TOKUSA1TIMECHO"))
                T0007row("HAYADETIMETTL") = Val(T0007row("HAYADETIME")) + Val(T0007row("HAYADETIMECHO"))
                T0007row("PONPNISSUTTL") = Val(T0007row("PONPNISSU")) + Val(T0007row("PONPNISSUCHO"))
                T0007row("BULKNISSUTTL") = Val(T0007row("BULKNISSU")) + Val(T0007row("BULKNISSUCHO"))
                T0007row("TRAILERNISSUTTL") = Val(T0007row("TRAILERNISSU")) + Val(T0007row("TRAILERNISSUCHO"))
                T0007row("BKINMUKAISUTTL") = Val(T0007row("BKINMUKAISU")) + Val(T0007row("BKINMUKAISUCHO"))
                T0007row("UNLOADCNTTTL") = Val(T0007row("UNLOADCNT")) + Val(T0007row("UNLOADCNTCHO"))
                T0007row("HAIDISTANCETTL") = Val(T0007row("HAIDISTANCE")) + Val(T0007row("HAIDISTANCECHO"))
                T0007row("KAIDISTANCETTL") = Val(T0007row("KAIDISTANCE")) + Val(T0007row("KAIDISTANCECHO"))
                T0007row("NENMATUNISSUTTL") = Val(T0007row("NENMATUNISSU")) + Val(T0007row("NENMATUNISSUCHO"))
                T0007row("JIKYUSHATIMETTL") = Val(T0007row("JIKYUSHATIME")) + Val(T0007row("JIKYUSHATIMECHO"))
                T0007row("MODELDISTANCETTL") = Val(T0007row("MODELDISTANCE")) + Val(T0007row("MODELDISTANCECHO"))
                T0007row("HDAIWORKTIMETTL") = Val(T0007row("HDAIWORKTIME")) + Val(T0007row("HDAIWORKTIMECHO"))
                T0007row("HDAINIGHTTIMETTL") = Val(T0007row("HDAINIGHTTIME")) + Val(T0007row("HDAINIGHTTIMECHO"))
                T0007row("SDAIWORKTIMETTL") = Val(T0007row("SDAIWORKTIME")) + Val(T0007row("SDAIWORKTIMECHO"))
                T0007row("SDAINIGHTTIMETTL") = Val(T0007row("SDAINIGHTTIME")) + Val(T0007row("SDAINIGHTTIMECHO"))
                T0007row("WWORKTIMETTL") = Val(T0007row("WWORKTIME")) + Val(T0007row("WWORKTIMECHO"))
                T0007row("JYOMUTIMETTL") = Val(T0007row("JYOMUTIME")) + Val(T0007row("JYOMUTIMECHO"))
                T0007row("HWORKNISSUTTL") = Val(T0007row("HWORKNISSU")) + Val(T0007row("HWORKNISSUCHO"))
                T0007row("KAITENCNTTTL") = Val(T0007row("KAITENCNT")) + Val(T0007row("KAITENCNTCHO"))
                T0007row("SENJYOCNTTTL") = Val(T0007row("SENJYOCNT")) + Val(T0007row("SENJYOCNTCHO"))
                T0007row("UNLOADADDCNT1TTL") = Val(T0007row("UNLOADADDCNT1")) + Val(T0007row("UNLOADADDCNT1CHO"))
                T0007row("UNLOADADDCNT2TTL") = Val(T0007row("UNLOADADDCNT2")) + Val(T0007row("UNLOADADDCNT2CHO"))
                T0007row("UNLOADADDCNT3TTL") = Val(T0007row("UNLOADADDCNT3")) + Val(T0007row("UNLOADADDCNT3CHO"))
                T0007row("UNLOADADDCNT4TTL") = Val(T0007row("UNLOADADDCNT4")) + Val(T0007row("UNLOADADDCNT4CHO"))
                T0007row("SHORTDISTANCE1TTL") = Val(T0007row("SHORTDISTANCE1")) + Val(T0007row("SHORTDISTANCE1CHO"))
                T0007row("SHORTDISTANCE2TTL") = Val(T0007row("SHORTDISTANCE2")) + Val(T0007row("SHORTDISTANCE2CHO"))
                T0007row("LATITUDE") = ""
                T0007row("LONGITUDE") = ""
                T0007row("DATAKBN") = "K"
                T0007row("SHIPORG") = ""
                T0007row("SHIPORGNAMES") = ""
                T0007row("NIPPONO") = ""
                T0007row("GSHABAN") = ""
                T0007row("RUIDISTANCE") = "0"
                T0007row("JIDISTANCE") = "0"
                T0007row("KUDISTANCE") = "0"
                ...
                For j As Integer = 1 To 10
                    Dim WW_STTIME As String = "T13BBSTTIME" & j.ToString("00")
                    Dim WW_ENDTIME As String = "T13BBENDTIME" & j.ToString("00")
                    If IsDate(T0007row(WW_STTIME)) Then
                        T0007row(WW_STTIME) = CDate(T0007row(WW_STTIME)).ToString("HH:mm")
                    Else
                        T0007row(WW_STTIME) = "00:00"
                    End If
                    If IsDate(T0007row(WW_ENDTIME)) Then
                        T0007row(WW_ENDTIME) = CDate(T0007row(WW_ENDTIME)).ToString("HH:mm")
                    Else
                        T0007row(WW_ENDTIME) = "00:00"
                    End If

                    WW_STTIME = "T13G1STTIME" & j.ToString("00")
                    WW_ENDTIME = "T13G1ENDTIME" & j.ToString("00")
                    If IsDate(T0007row(WW_STTIME)) Then
                        T0007row(WW_STTIME) = CDate(T0007row(WW_STTIME)).ToString("HH:mm")
                    Else
                        T0007row(WW_STTIME) = "00:00"
                ...
                If work.WF_T7SEL_LIMITFLG.Text = "0" Then
                    '日報取込判定
                    Dim WW_NIPPOLINK() As String = Split(T0007row("NIPPOLINKCODE"), "_")
                    If WW_NIPPOLINK.Length < 2 Then
                        ReDim Preserve WW_NIPPOLINK(1)
                        WW_NIPPOLINK(1) = ""
                    End If
                    If T0007row("T5ENTRYDATE") <> "" AndAlso T0007row("HDKBN") = "H" AndAlso T0007row("RECODEKBN") = "0" AndAlso String.IsNullOrEmpty(WW_NIPPOLINK(0)) Then
                        T0007row("STATUS") = "日報取込"
                    End If

                    If T0007row("T13ENTRYDATE") <> "" AndAlso T0007row("HDKBN") = "H" AndAlso T0007row("RECODEKBN") = "0" AndAlso String.IsNullOrEmpty(WW_NIPPOLINK(1)) Then
                        If String.IsNullOrEmpty(T0007row("STATUS")) Then
                            T0007row("STATUS") = "休憩,配送取込"
                        End If
                    End If

                    If T0007row("T5ENTRYDATE") <> "" AndAlso T0007row("HDKBN") = "H" AndAlso T0007row("RECODEKBN") = "0" AndAlso Not String.IsNullOrEmpty(WW_NIPPOLINK(0)) AndAlso WW_NIPPOLINK(0) <> T0007row("T5ENTRYDATE") Then
                        T0007row("STATUS") = "日報変更"
                    End If

                    If T0007row("T13ENTRYDATE") <> "" AndAlso T0007row("HDKBN") = "H" AndAlso T0007row("RECODEKBN") = "0" AndAlso Not String.IsNullOrEmpty(WW_NIPPOLINK(1)) AndAlso WW_NIPPOLINK(1) <> T0007row("T13ENTRYDATE") Then
                        If String.IsNullOrEmpty(T0007row("STATUS")) Then
                            T0007row("STATUS") = "休憩,配送変更"
                        End If
                    End If
                End If
        ...
        '日報を勤怠フォーマットに変換し、マージする
        Dim WW_T0007tbl As DataTable = T0007tbl.Clone
        NIPPOget_T7Format("OLD", WW_T0007tbl, T0005tbl)
    ...
    Protected Sub Scrole_SUB()
        ...
        'ソート
        Dim t7tbl = T0007tbl.AsEnumerable.OrderBy(Function(x) x.Item("LINECNT"))

        '表示対象行カウント(絞り込み対象)
        '　※　絞込（Cells(4)： 0=表示対象 , 1=非表示対象)
        For Each WW_T0007row In t7tbl.Where(Function(x) x.Item("HIDDEN") = "0")
            WW_DataCNT += 1
        ...
        Dim t7view = t7tbl.
                        Where(Function(x) x.Item("HIDDEN") = 0 AndAlso
                                          x.Item("EXTRACTCNT") >= WW_GridPosition AndAlso
                                          x.Item("EXTRACTCNT") <= (WW_GridPosition + WW_GridCnt - 1)).
        ...
        Else
            ...
            WF_GridPosition.Text = t7view.First.Item("EXTRACTCNT")
        End If
        CS0013ProfView.TBLOBJ = pnlListArea
        CS0013ProfView.SCROLLTYPE = CS0013ProfView.SCROLLTYPE_ENUM.None
        CS0013ProfView.LEVENT = "ondblclick"
    ...
    Private Sub T0007_InsertEdit(ByRef iTbl As DataTable, ByVal iDATENOW As Date, ByRef oTBL As DataTable, ByRef oRtn As String)
        ...
        For i As Integer = 0 To iTbl.Rows.Count - 1
            ...
            If String.IsNullOrEmpty(iRow("NIPPOLINKCODE")) Then
                WW_row("NIPPOLINKCODE") = iRow("NIPPOLINKCODE")                 '日報連結コード
            Else
                Dim WW_LINKCODE As String = Mid(iRow("NIPPOLINKCODE"), 1, InStr(iRow("NIPPOLINKCODE"), "_")) & iDATENOW.ToString("yyyyMMddHHmmss")
                WW_row("NIPPOLINKCODE") = WW_LINKCODE                               '日報連結コード
            End If
            ...
            WW_row("TOKUKYUMUKYUNISSU") = iRow("TOKUKYUMUKYUNISSU")                                '特休無給
            WW_row("TOKUKYUMUKYUNISSUCHO") = iRow("TOKUKYUMUKYUNISSUCHO")                          '特休無給調整
            WW_row("ROSAIYUKYNIUSSU") = iRow("ROSAIYUKYNIUSSU")                                    '労災
            WW_row("ROSAIYUKYNIUSSUCHO") = iRow("ROSAIYUKYNIUSSUCHO")                              '労災調整
            WW_row("KOKANGOYUKYUNISSU") = iRow("KOKANGOYUKYUNISSU")                                '子看有給
            WW_row("KOKANGOYUKYUNISSUCHO") = iRow("KOKANGOYUKYUNISSUCHO")                          '子看有給調整
            WW_row("KOKANGOMUKYUNISSU") = iRow("KOKANGOMUKYUNISSU")                                '子看無給
            WW_row("KOKANGOMUKYUNISSUCHO") = iRow("KOKANGOMUKYUNISSUCHO")                          '子看無給調整
    ...
    Public Sub T0007Insert(ByVal iRow As DataRow, ByRef I_SQLcon As SqlConnection)
        ...
        Dim SQLStr As String =
                ...
                & "        WEEKNISSUCHO," _
                & "        TOKUKYUMUKYUNISSU," _
                & "        TOKUKYUMUKYUNISSUCHO," _
                & "        ROSAIYUKYNIUSSU," _
                & "        ROSAIYUKYNIUSSUCHO," _
                & "        KOKANGOYUKYUNISSU," _
                & "        KOKANGOYUKYUNISSUCHO," _
                & "        KOKANGOMUKYUNISSU," _
                & "        KOKANGOMUKYUNISSUCHO," _
                & "        DAIKYUNISSU," _
                & "        DAIKYUNISSUCHO," _
                & "        NENSHINISSU," _
                & "        NENSHINISSUCHO," _
        ...
        Dim P_TOKUKYUMUKYUNISSU As SqlParameter = SQLcmd.Parameters.Add("@TOKUKYUMUKYUNISSU", System.Data.SqlDbType.Int)
        Dim P_TOKUKYUMUKYUNISSUCHO As SqlParameter = SQLcmd.Parameters.Add("@TOKUKYUMUKYUNISSUCHO", System.Data.SqlDbType.Int)
        Dim P_ROSAIYUKYNIUSSU As SqlParameter = SQLcmd.Parameters.Add("@ROSAIYUKYNIUSSU", System.Data.SqlDbType.Int)
        Dim P_ROSAIYUKYNIUSSUCHO As SqlParameter = SQLcmd.Parameters.Add("@ROSAIYUKYNIUSSUCHO", System.Data.SqlDbType.Int)
        Dim P_KOKANGOYUKYUNISSU As SqlParameter = SQLcmd.Parameters.Add("@KOKANGOYUKYUNISSU", System.Data.SqlDbType.Int)
        Dim P_KOKANGOYUKYUNISSUCHO As SqlParameter = SQLcmd.Parameters.Add("@KOKANGOYUKYUNISSUCHO", System.Data.SqlDbType.Int)
        Dim P_KOKANGOMUKYUNISSU As SqlParameter = SQLcmd.Parameters.Add("@KOKANGOMUKYUNISSU", System.Data.SqlDbType.Int)
        Dim P_KOKANGOMUKYUNISSUCHO As SqlParameter = SQLcmd.Parameters.Add("@KOKANGOMUKYUNISSUCHO", System.Data.SqlDbType.Int)
        ...
        P_TOKUKYUMUKYUNISSU.Value = iRow("TOKUKYUMUKYUNISSU")
        P_TOKUKYUMUKYUNISSUCHO.Value = iRow("TOKUKYUMUKYUNISSUCHO")
        P_ROSAIYUKYNIUSSU.Value = iRow("ROSAIYUKYNIUSSU")
        P_ROSAIYUKYNIUSSUCHO.Value = iRow("ROSAIYUKYNIUSSUCHO")
        P_KOKANGOYUKYUNISSU.Value = iRow("KOKANGOYUKYUNISSU")
        P_KOKANGOYUKYUNISSUCHO.Value = iRow("KOKANGOYUKYUNISSUCHO")
        P_KOKANGOMUKYUNISSU.Value = iRow("KOKANGOMUKYUNISSU")
        P_KOKANGOMUKYUNISSUCHO.Value = iRow("KOKANGOMUKYUNISSUCHO")
                    ...
                    If T0007row("WORKDATE") = WW_HEADrow("WORKDATE") And
                       ...
                       T0007row("SELECT") = "1" Then
                        ...
                        If T0007row("HDKBN") = "H" Then
                            ...
                            WW_T0007row("TOKUKYUMUKYUNISSUCHO") = Val(WW_HEADrow("TOKUKYUMUKYUNISSUTTL")) - T0007row("TOKUKYUMUKYUNISSU")
                            WW_T0007row("TOKUKYUMUKYUNISSUTTL") = Val(T0007row("TOKUKYUMUKYUNISSU")) + Val(T0007row("TOKUKYUMUKYUNISSUCHO"))
                            WW_T0007row("ROSAIYUKYNIUSSUCHO") = Val(WW_HEADrow("ROSAIYUKYNIUSSUTTL")) - T0007row("ROSAIYUKYNIUSSU")
                            WW_T0007row("ROSAIYUKYNIUSSUTTL") = Val(T0007row("ROSAIYUKYNIUSSU")) + Val(T0007row("ROSAIYUKYNIUSSUCHO"))
                            WW_T0007row("KOKANGOYUKYUNISSUCHO") = Val(WW_HEADrow("KOKANGOYUKYUNISSUTTL")) - T0007row("KOKANGOYUKYUNISSU")
                            WW_T0007row("KOKANGOYUKYUNISSUTTL") = Val(T0007row("KOKANGOYUKYUNISSU")) + Val(T0007row("KOKANGOYUKYUNISSUCHO"))
                            WW_T0007row("KOKANGOMUKYUNISSUCHO") = Val(WW_HEADrow("KOKANGOMUKYUNISSUTTL")) - T0007row("KOKANGOMUKYUNISSU")
                            WW_T0007row("KOKANGOMUKYUNISSUTTL") = Val(T0007row("KOKANGOMUKYUNISSU")) + Val(T0007row("KOKANGOMUKYUNISSUCHO"))
    ...
    Public Sub CreWORKKBN(ByRef ioTbl As DataTable, ByRef iT0005tbl As DataTable, ByVal iSTDATE As String, ByVal iENDDATE As String)
        ...
        WW_IDX = 0
        For Each WW_HEADrow As DataRow In WW_T0007HEADtbl.Rows
            If (WW_HEADrow("STATUS") Like "*日報変更*" Or WW_HEADrow("STATUS") Like "*休憩*") AndAlso
        ...
        iT0007view.RowFilter = "RECODEKBN ='0' and (STATUS Like '*日報*' or STATUS Like '*休憩*') and WORKDATE >= #" & iSTDATE & "# and WORKDATE <= #" & iENDDATE & "#"
        Dim wT0007tbl As DataTable = iT0007view.ToTable
    ...
    Public Sub HeadEdit(ByRef ioTbl As DataTable, ByRef iT0005tbl As DataTable, ByVal iSTDATE As String, ByVal iENDDATE As String, ByRef iMODELtbl As DataTable)
        ...
        For Each WW_HEADrow As DataRow In WW_T0007HEADtbl.Rows
            ...
            '日報取得
            '該当する日報を抽出し、新しいテーブルを作成する
            iT0005view.RowFilter = "YMD = #" & WW_HEADrow("WORKDATE") & "# and STAFFCODE ='" & WW_HEADrow("STAFFCODE") & "' and WORKKBN in ('F3','B3','B2')"
        ...
        For Each WW_HEADrow As DataRow In WW_T0007HEADtbl.Rows
            ...
            If iT0007view.Count > 0 Then
                ...
                If T0007COM.CheckHOLIDAY(iT0007view.Item(0).Row("HOLIDAYKBN"), iT0007view.Item(0).Row("PAYKBN")) Then
                    '1:法定休日、2:法定外休日
                    '01:年休, 02 : 特休, 04 : ｽﾄｯｸ, 05 : 協約週休, 06 : 週休（2020/04/07_削除）
                    '07:傷欠, 08 : 組欠, 09 : 他欠, 11 : 代休, 13 : 指定休, 15 : 振休
                    '18 : 労災欠, 19 : 特休無給, 20 : 子看有給, 21 : 子看無給
    ...
    Protected Sub T0007INProw_CHEK(ByRef RTN As String)
        ...
        CS0036FCHECK.MAPID = GRT00007WRKINC_V2.MAPIDINJS
        ...
        If T0007INProw("RECODEKBN") = "2" Then
            ...
            ' *** 特休無給合計(TOKUKYUMUKYUNISSUTTL)
            If T0007INProw("TOKUKYUMUKYUNISSUTTL") = Nothing Then
                'エラーレポート編集
                WW_CheckMES1 = "・更新できないレコード(特休無給日数無)です。"
                WW_CheckMES2 = ""
                ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
            Else
                CS0036FCHECK.FIELD = "TOKUKYUMUKYUNISSUTTL"
                CS0036FCHECK.VALUE = T0007INProw("TOKUKYUMUKYUNISSUTTL")
                CS0036FCHECK.CS0036FCHECK()
                If CS0036FCHECK.ERR = C_MESSAGE_NO.NORMAL Then
                    T0007INProw("TOKUKYUMUKYUNISSUTTL") = Val(CS0036FCHECK.VALUEOUT)
                Else
                    'エラーレポート編集
                    WW_CheckMES1 = "・更新できないレコード(特休無給日数エラー)です。"
                    WW_CheckMES2 = CS0036FCHECK.CHECKREPORT
                    ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
                End If
            End If

            ' *** 労災合計(ROSAIYUKYNIUSSUTTL)
            If T0007INProw("ROSAIYUKYNIUSSUTTL") = Nothing Then
                'エラーレポート編集
                WW_CheckMES1 = "・更新できないレコード(労災日数無)です。"
                WW_CheckMES2 = ""
                ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
            Else
                CS0036FCHECK.FIELD = "ROSAIYUKYNIUSSUTTL"
                CS0036FCHECK.VALUE = T0007INProw("ROSAIYUKYNIUSSUTTL")
                CS0036FCHECK.CS0036FCHECK()
                If CS0036FCHECK.ERR = C_MESSAGE_NO.NORMAL Then
                    T0007INProw("ROSAIYUKYNIUSSUTTL") = Val(CS0036FCHECK.VALUEOUT)
                Else
                    'エラーレポート編集
                    WW_CheckMES1 = "・更新できないレコード(労災日数エラー)です。"
                    WW_CheckMES2 = CS0036FCHECK.CHECKREPORT
                    ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
                End If
            End If

            ' *** 子看有給合計(KOKANGOYUKYUNISSUTTL)
            If T0007INProw("KOKANGOYUKYUNISSUTTL") = Nothing Then
                'エラーレポート編集
                WW_CheckMES1 = "・更新できないレコード(子看有給日数無)です。"
                WW_CheckMES2 = ""
                ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
            Else
                CS0036FCHECK.FIELD = "KOKANGOYUKYUNISSUTTL"
                CS0036FCHECK.VALUE = T0007INProw("KOKANGOYUKYUNISSUTTL")
                CS0036FCHECK.CS0036FCHECK()
                If CS0036FCHECK.ERR = C_MESSAGE_NO.NORMAL Then
                    T0007INProw("KOKANGOYUKYUNISSUTTL") = Val(CS0036FCHECK.VALUEOUT)
                Else
                    'エラーレポート編集
                    WW_CheckMES1 = "・更新できないレコード(子看有給日数エラー)です。"
                    WW_CheckMES2 = CS0036FCHECK.CHECKREPORT
                    ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
                End If
            End If

            ' *** 子看無給合計(KOKANGOMUKYUNISSUTTL)
            If T0007INProw("KOKANGOMUKYUNISSUTTL") = Nothing Then
                'エラーレポート編集
                WW_CheckMES1 = "・更新できないレコード(子看無給日数無)です。"
                WW_CheckMES2 = ""
                ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
            Else
                CS0036FCHECK.FIELD = "KOKANGOMUKYUNISSUTTL"
                CS0036FCHECK.VALUE = T0007INProw("KOKANGOMUKYUNISSUTTL")
                CS0036FCHECK.CS0036FCHECK()
                If CS0036FCHECK.ERR = C_MESSAGE_NO.NORMAL Then
                    T0007INProw("KOKANGOMUKYUNISSUTTL") = Val(CS0036FCHECK.VALUEOUT)
                Else
                    'エラーレポート編集
                    WW_CheckMES1 = "・更新できないレコード(子看無給日数エラー)です。"
                    WW_CheckMES2 = CS0036FCHECK.CHECKREPORT
                    ERRMSG_write(WW_CheckMES1, WW_CheckMES2, WW_LINEerr, "10023")
                End If
            End If
    ...
    Public Sub T00005ALLget(ByVal iSTAFFCODE As String, ByVal iYmdFrom As String, ByVal iYmdTo As String, ByRef oTbl As DataTable, ByRef oRtn As String)
        ...
        Try
            ...
            Dim SQLStr As String =
                ...
                & "       '' as SHIPORGNAMES , " _
                & "       isnull(format(A.YMD, 'yyyy/MM/dd'),'') as YMD , " _
                & "       isnull(rtrim(A.NIPPONO),'') as NIPPONO , " _
                & "       isnull(A.SEQ,'0') as SEQ , " _
                & "       isnull(rtrim(A.WORKKBN),'') as WORKKBN , " _
                & "       isnull((select rtrim(F1.VALUE2) from MC001_FIXVALUE F1 " _
                ...
                & "       isnull(format(A.STDATE, 'yyyy/MM/dd'),'') as STDATE , " _
                & "       isnull(format(STTIME, 'hh')+':'+format(STTIME, 'mm'),'00:00')  as STTIME , " _
                & "       isnull(format(A.ENDDATE, 'yyyy/MM/dd'),'') as ENDDATE , " _
                & "       isnull(format(ENDTIME, 'hh')+':'+format(ENDTIME, 'mm'),'00:00')  as ENDTIME , " _
                ...
                & "       isnull(cast(A.SOUDISTANCE as int),'0') as SOUDISTANCE , " _
                ...
                & "       isnull((select rtrim(MA6.SHARYOKBN) from MA006_SHABANORG MA6 " _
                & "               where MA6.CAMPCODE    = A.CAMPCODE " _
                ...
                & "              and   F2.KEYCODE     = (select SHARYOKBN from MA006_SHABANORG where CAMPCODE = A.CAMPCODE and MANGUORG = A.SHIPORG and GSHABAN = A.GSHABAN and DELFLG <> '1') " _
                & "              and   F2.STYMD      <= @P5 " _
                & "              and   F2.ENDYMD     >= @P5 " _
                & "              and   F2.DELFLG     <> '1' " _
                & "       ),'') as SHARYOKBNNAMES , " _
                & "       isnull((select rtrim(MA6.OILKBN) from MA006_SHABANORG MA6 " _
                & "               where MA6.CAMPCODE    = A.CAMPCODE " _
                & "               and   MA6.MANGUORG    = A.SHIPORG " _
                & "               and   MA6.GSHABAN     = A.GSHABAN " _
                & "               and   MA6.DELFLG     <> '1' " _
                ...
                & "              and   F5.KEYCODE     = (select OILKBN from MA006_SHABANORG where CAMPCODE = A.CAMPCODE and MANGUORG = A.SHIPORG and GSHABAN = A.GSHABAN and DELFLG <> '1') " _
                & "              and   F5.STYMD      <= @P5 " _
                & "              and   F5.ENDYMD     >= @P5 " _
                & "              and   F5.DELFLG     <> '1' " _
                & "       ),'') as OILPAYKBNNAMES , " _
                & "       isnull((select rtrim(MA6.SUISOKBN) from MA006_SHABANORG MA6 " _
                & "               where MA6.CAMPCODE    = A.CAMPCODE " _
                & "               and   MA6.MANGUORG    = A.SHIPORG " _
                & "               and   MA6.GSHABAN     = A.GSHABAN " _
                & "               and   MA6.DELFLG     <> '1' " _
                ...
                & "       isnull((select CAL.WORKINGWEEK from MB005_CALENDAR CAL " _
                & "               where CAL.CAMPCODE    = A.CAMPCODE " _
                ...
                & "              and   F7.KEYCODE     = (select CAL.WORKINGWEEK from MB005_CALENDAR CAL where CAL.CAMPCODE = A.CAMPCODE and CAL.WORKINGYMD = A.YMD and CAL.DELFLG <> '1') " _
                & "              and   F7.STYMD      <= @P5 " _
                & "              and   F7.ENDYMD     >= @P5 " _
                & "              and   F7.DELFLG     <> '1' " _
                & "       ),'') as WORKINGWEEKNAMES , " _
                & "       isnull((select CAL.WORKINGKBN from MB005_CALENDAR CAL " _
                & "               where CAL.CAMPCODE    = A.CAMPCODE " _
                & "               and   CAL.WORKINGYMD  = A.YMD " _
                & "               and   CAL.DELFLG     <> '1' " _
                & "       ),'') as HOLIDAYKBN , " _
                & "       isnull((select rtrim(F8.VALUE1) from MC001_FIXVALUE F8 " _
                & "              where F8.CAMPCODE    = @P1 " _
                & "              and   F8.CLASS       = 'HOLIDAYKBN' " _
                & "              and   F8.KEYCODE     = (select CAL.WORKINGKBN from MB005_CALENDAR CAL where CAL.CAMPCODE = A.CAMPCODE and CAL.WORKINGYMD = A.YMD and CAL.DELFLG <> '1') " _
                & "              and   F8.STYMD      <= @P5 " _
                & "              and   F8.ENDYMD     >= @P5 " _
                & "              and   F8.DELFLG     <> '1' " _
                & "       ),'') as HOLIDAYKBNNAMES , " _
                & "       isnull((select rtrim(B3.MORG) from MB001_STAFF B3 " _
                & "               where B3.CAMPCODE    = A.CAMPCODE " _
                ...
                & "       ),'') as MORG , " _
                & "       '' as MORGNAMES , " _
                & "       isnull((select rtrim(B3.HORG) from MB001_STAFF B3 " _
                & "               where B3.CAMPCODE    = A.CAMPCODE " _
                & "               and   B3.STAFFCODE   = A.STAFFCODE " _
                & "               and   B3.STYMD      <= A.YMD " _
                & "               and   B3.ENDYMD     >= A.YMD " _
                ...
                & "       '' as SORGNAMES , " _
                & "       isnull((select rtrim(B3.STAFFKBN) from MB001_STAFF B3 " _
                & "               where B3.CAMPCODE    = A.CAMPCODE " _
                & "               and   B3.STAFFCODE   = A.STAFFCODE " _
                & "               and   B3.STYMD      <= A.YMD " _
                & "               and   B3.ENDYMD     >= A.YMD " _
            ...
            If iSTAFFCODE <> "" Then
                SQLStr = SQLStr & " and   A.STAFFCODE = @P7 "
            End If
            ...
            SQLcmd.CommandTimeout = 300
            Dim SQLad As SqlDataAdapter = New SqlDataAdapter(SQLcmd)

            '■テーブル検索結果をテーブル退避
            SQLad.Fill(oTbl)

            CS0026TblSort.TABLE = oTbl
            CS0026TblSort.FILTER = ""
            CS0026TblSort.SORTING = "YMD , STAFFCODE , STDATE , STTIME, ENDDATE , ENDTIME"
            oTbl = CS0026TblSort.Sort()

            SQLad.Dispose() 'Reader(Close)
            SQLad = Nothing
    ...
    ' *** GridView用（日報）データ取得                                                   
    Private Sub NIPPOget_T7Format(ByVal iKBN As String, ByRef ioT7tbl As DataTable, ByVal iT0005tbl As DataTable)

        Dim WW_CONVERT As String = ""

        For i As Integer = 0 To iT0005tbl.Rows.Count - 1
            Dim T5row As DataRow = iT0005tbl.Rows(i)
        ...
        For i As Integer = 0 To CS0021PROFXLS.FIELD.Count - 1
            If CS0021PROFXLS.EFFECT(i) = "Y" AndAlso CS0021PROFXLS.POSIX(i) > 0 AndAlso CS0021PROFXLS.POSIY(i) > 0 Then
                ...
                If (CS0021PROFXLS.FIELD(i) = "KYOTEIWEEKNISSUTTL") OrElse
                   ...
                   (CS0021PROFXLS.FIELD(i) = "TOKUKYUMUKYUNISSUTTL") OrElse
                   (CS0021PROFXLS.FIELD(i) = "ROSAIYUKYNIUSSUTTL") OrElse
                   (CS0021PROFXLS.FIELD(i) = "KOKANGOYUKYUNISSUTTL") OrElse
                   (CS0021PROFXLS.FIELD(i) = "KOKANGOMUKYUNISSUTTL") OrElse
    ...
    Protected Sub T0007INPmake(ByRef iTBL As DataTable, ByRef iTTLFLG As String)
        ...
        For i As Integer = 0 To iTBL.Rows.Count - 1
            ...
            ' *** 特休無給合計(TOKUKYUMUKYUNISSUTTL)
            If WW_COLUMNS.IndexOf("TOKUKYUMUKYUNISSUTTL") < 0 Then
                T0007INProw("TOKUKYUMUKYUNISSUTTL") = ""
            Else
                T0007INProw("TOKUKYUMUKYUNISSUTTL") = iTBL.Rows(i)("TOKUKYUMUKYUNISSUTTL")
            End If

            ' *** 労災合計(ROSAIYUKYNIUSSUTTL)
            If WW_COLUMNS.IndexOf("ROSAIYUKYNIUSSUTTL") < 0 Then
                T0007INProw("ROSAIYUKYNIUSSUTTL") = ""
            Else
                T0007INProw("ROSAIYUKYNIUSSUTTL") = iTBL.Rows(i)("ROSAIYUKYNIUSSUTTL")
            End If

            ' *** 子看有給合計(KOKANGOYUKYUNISSUTTL)
            If WW_COLUMNS.IndexOf("KOKANGOYUKYUNISSUTTL") < 0 Then
                T0007INProw("KOKANGOYUKYUNISSUTTL") = ""
            Else
                T0007INProw("KOKANGOYUKYUNISSUTTL") = iTBL.Rows(i)("KOKANGOYUKYUNISSUTTL")
            End If

            ' *** 子看無給合計(KOKANGOMUKYUNISSUTTL)
            If WW_COLUMNS.IndexOf("KOKANGOMUKYUNISSUTTL") < 0 Then
                T0007INProw("KOKANGOMUKYUNISSUTTL") = ""
            Else
                T0007INProw("KOKANGOMUKYUNISSUTTL") = iTBL.Rows(i)("KOKANGOMUKYUNISSUTTL")
            End If
    ...
    Protected Sub MAPrefelence(ByRef O_MSG As String, ByRef O_RTN As String)
        ...
        '■■■ 選択画面の入力初期値設定 ■■■
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.T00007NJS_V2 Then

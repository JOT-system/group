...
''' </summary>
''' <remarks></remarks>
Public Class GRT00007ICHIRAN_JKT_V2
    Inherits Page
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = GRT00007WRKINC_V2.MAPIDIJKT
    ...
    Protected Sub WF_ButtonEND_Click()
        ...
        ''■■■ 画面戻先URL取得 ■■■
        'CS0017RETURNURLget.MAPID = GRT00007WRKINC_V2.MAPIDIJKT
    ...
    '            CS0049UPROFview.MAPID = GRT00007WRKINC_V2.MAPIDIJKT
    ...
    Private Sub GRID_INITset()
        ...
        Try
            ...
            For i As Integer = 0 To T0007tbl.Rows.Count - 1
                ...
                T0007row("LOADINGCNT1TTL") = Val(T0007row("LOADINGCNT1")) + Val(T0007row("LOADINGCNT1CHO"))
                T0007row("LOADINGCNT2TTL") = Val(T0007row("LOADINGCNT2")) + Val(T0007row("LOADINGCNT2CHO"))
                T0007row("LATITUDE") = ""
                T0007row("LONGITUDE") = ""
                T0007row("DATAKBN") = "K"
                T0007row("SHIPORG") = ""
        ...
        Try
            ...
            For Each WW_HEADrow As DataRow In wT0007tbl.Rows
                ...
                For j As Integer = T0005tbl.Rows.Count - 1 To 0 Step -1
                    ...
                    If WW_NIPPOrow("WORKKBN") = "B2" And
                      (WW_NIPPOrow("OILPAYKBN") = "02" Or WW_NIPPOrow("OILPAYKBN") = "03") And
                       WW_B3CNT = 0 Then
                        For i As Integer = 0 To WW_WKDTLtbl.Rows.Count - 1
                            Dim WW_WKDTLrow As DataRow = WW_WKDTLtbl.Rows(i)
                            If WW_WKDTLrow("STAFFCODE") = WW_NIPPOrow("STAFFCODE") And
    ...
    Private Sub T0007_InsertEdit(ByRef iTbl As DataTable, ByVal iDATENOW As Date, ByRef oTBL As DataTable, ByRef oRtn As String)
        ...
        For i As Integer = 0 To iTbl.Rows.Count - 1
            ...
            WW_row("UNLOADADDCNT4") = iRow("UNLOADADDCNT4")                                 '危険品荷卸４回数
            WW_row("UNLOADADDCNT4CHO") = iRow("UNLOADADDCNT4CHO")                           '危険品荷卸４回数調整
            WW_row("LOADINGCNT1") = 0                                                       '危険品積込１回数
            WW_row("LOADINGCNT1CHO") = 0                                                    '危険品積込１回数調整
    ...
    Public Sub T0007Insert(ByVal iRow As DataRow, ByRef I_SQLcon As SqlConnection)
        ...
        P_LOADINGCNT1.Value = 0
        P_LOADINGCNT1CHO.Value = 0
    ...
    Public Sub HeadEdit(ByRef ioTbl As DataTable, ByRef iT0005tbl As DataTable, ByVal iSTDATE As String, ByVal iENDDATE As String)
        ...
        For Each WW_HEADrow As DataRow In WW_T0007HEADtbl.Rows
            ...
            'For i As Integer = iT0005view.Count - 1 To 0 Step -1
            '    Dim WW_NIPPOrow As DataRow = iT0005view.Item(i).Row
            '    If WW_NIPPOrow("WORKKBN") = "F3" Then
            '        WW_B2CNT = 0
            '    End If
            ...
            '        If WW_B2CNT = 0 And
            '          (WW_NIPPOrow("OILPAYKBN") = "02" Or
            '           WW_NIPPOrow("OILPAYKBN") = "03") Then
            ...
            WW_HEADrow("UNLOADADDCNT4") = WW_UNLOADADDCNT4
            WW_HEADrow("UNLOADADDCNT4TTL") = WW_UNLOADADDCNT4 + WW_HEADrow("UNLOADADDCNT4CHO")
            WW_HEADrow("LOADINGCNT1") = 0
            WW_HEADrow("LOADINGCNT1TTL") = 0
    ...
    Protected Sub T0007INProw_CHEK(ByRef RTN As String)
        ...
        CS0036FCHECK.MAPID = GRT00007WRKINC_V2.MAPIDIJKT
    ...
    Public Sub T00005ALLget(ByVal iSTAFFCODE As String, ByVal iYmdFrom As String, ByVal iYmdTo As String, ByRef oTbl As DataTable, ByRef oRtn As String)
        ...
        Try
            ...
            Dim SQLStr As String =
                ...
                & "              and   F2.DELFLG     <> '1' " _
                & "       ),'') as SHARYOKBNNAMES , " _
                & "       CASE WHEN isnull(F1.VALUE1,'') <> '' " _
                & "            THEN F1.VALUE1 " _
                & "            ELSE (CASE WHEN isnull(F2.VALUE1,'') <> '' " _
                & "                       THEN F2.VALUE1 " _
                & "                       ELSE isnull((select rtrim(MA6.OILKBN) from MA006_SHABANORG MA6 " _
                & "                                    where MA6.CAMPCODE    = A.CAMPCODE " _
                & "                                    and   MA6.MANGUORG    = A.SHIPORG " _
                & "                                    and   MA6.GSHABAN     = A.GSHABAN " _
                & "                                    and   MA6.DELFLG     <> '1' " _
                ...
                & "               and   F5.KEYCODE     = (CASE WHEN isnull(F1.VALUE1,'') <> '' THEN F1.VALUE1 ELSE (CASE WHEN isnull(F2.VALUE1,'') <> '' THEN F2.VALUE1 ELSE (select OILKBN from MA006_SHABANORG where CAMPCODE = A.CAMPCODE and MANGUORG = A.SHIPORG and GSHABAN = A.GSHABAN and DELFLG <> '1') END) END) " _
                & "               and   F5.STYMD      <= @P5 " _
                & "               and   F5.ENDYMD     >= @P5 " _
                & "               and   F5.DELFLG     <> '1' " _
                & "       ),'') as OILPAYKBNNAMES , " _
                ...
                & "   and   F1.CLASS         = 'SPECIALTORI' " _
                & "   and   F1.KEYCODE       = A.L1TORICODE " _
                & "   and   F1.STYMD        <= @P5 " _
                & "   and   F1.ENDYMD       >= @P5 " _
                & "   and   F1.DELFLG       <> '1' " _
                & " LEFT JOIN MC001_FIXVALUE F2 " _
                & "   ON    F2.CAMPCODE      = A.CAMPCODE " _
                & "   and   F2.CLASS         = 'SPECIALSHUKABASHO' " _
                & "   and   F2.KEYCODE       = A.L1SHUKABASHO " _
                & "   and   F2.STYMD        <= @P5 " _
                & "   and   F2.ENDYMD       >= @P5 " _
                & "   and   F2.DELFLG       <> '1' " _
    ...
    Protected Sub MAPrefelence(ByRef O_MSG As String, ByRef O_RTN As String)
        ...
        '■■■ 選択画面の入力初期値設定 ■■■
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.T00007JKT_V2 Then

...
Public Class GRT00005NIPPO
    ...
    Protected Sub WF_WORKKBN_OnChange()
        ...
        Select Case WW_WORKKBN.text
            Case "A1", "Z1"
                ...
                IsEntryRepeaterItem("STMATER", False, "Y")
                IsEntryRepeaterItem("ENDMATER", False, "Y")
            ...
            Case "F3"
                ...
                IsEntryRepeaterItem("STANI7NAMES", False, "N")
                IsEntryRepeaterItem("STANI8NAMES", False, "N")
                IsEntryRepeaterItem("SOUDISTANCE", False, "Y")
                IsEntryRepeaterItem("RUIDISTANCE", False, "Y")
                IsEntryRepeaterItem("STMATER", False, "N")
                IsEntryRepeaterItem("ENDMATER", True)
                IsEntryRepeaterItem("JIDISTANCE", False, "Y")
                IsEntryRepeaterItem("KUDISTANCE", False, "Y")
                IsEntryRepeaterItem("IPPDISTANCE", False, "Y")
                IsEntryRepeaterItem("KOSDISTANCE", False, "Y")
                IsEntryRepeaterItem("IPPJIDISTANCE", False, "Y")
                IsEntryRepeaterItem("IPPKUDISTANCE", False, "Y")
                IsEntryRepeaterItem("KOSJIDISTANCE", False, "Y")
                IsEntryRepeaterItem("KOSKUDISTANCE", False, "Y")
    ...
    Protected Sub SetInitialGridData()
        ...
        Try
            ...
            '対象データ抽出
            CS0026TBLSORT.TABLE = T0005tbl
            CS0026TBLSORT.SORTING = "YMD, STAFFCODE, HDKBN DESC, STDATE, STTIME, SEQ, ENDDATE, ENDTIME"
            '削除データの退避
                ...
                If WW_YMD_FLG = "0" AndAlso T0005INProw("YMD") <> "" Then
                    ...
                    If T0005COM.IsInspectionOrg(work.WF_SEL_CAMPCODE.Text, T0005INProw("SHIPORG"), WW_RTN) Then
                        If IsNothing(work.CreateTSHABANList(work.WF_SEL_CAMPCODE.Text, work.WF_SEL_UORG.Text, O_RTN).Items.FindByValue(T0005INProw("GSHABAN"))) Then
                            ...
                            If T0005INProw("GSHABAN") <> "" AndAlso WW_list.Items.FindByValue(T0005INProw("GSHABAN")).Text = "02" Then
            ...
            If T0005INProw("WORKKBN") = "B3" Then
                ...
                If Val(T0005INProw("SURYO1")) = 0 AndAlso
                   ...
                   Val(T0005INProw("SURYO8")) = 0 Then
                    If work.WF_T7SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_NJS Then
                        'エラーレポート編集（NJS以外はエラー）
                    ...
                    If T0005INProw("WORKKBN") <> "F3" Then
                        For j As Integer = 0 To T0005INPtbl.Rows.Count - 1
                            If T0005INPtbl.Rows(j)("WORKKBN") = "F3" Then
                                If T0005INPtbl.Rows(j)("GSHABAN") = T0005INProw("GSHABAN") AndAlso
                                   T0005INPtbl.Rows(j)("NIPPONO") = T0005INProw("NIPPONO") Then
    ...
    Protected Sub CheckListTable(ByRef O_RTN As String)
        ...
        '---------------------------------------------------
        Dim WW_SEQ As Integer = 0
        Dim WW_CHKLIST As New Dictionary(Of String, List(Of Integer))
        Dim WW_CNT() As Integer = {0, 0, 0}

        O_RTN = C_MESSAGE_NO.NORMAL

        '運行日、従業員、タイトル区分（ヘッダ、明細）、開始時刻の順番で処理する
        Dim WW_TBLview As New DataView(T0005INPtbl)
        WW_TBLview.Sort = "YMD ASC, STAFFCODE ASC, HDKBN DESC, STDATE ASC, STTIME ASC, ENDDATE ASC, ENDTIME ASC, SEQ ASC"
        ...
        For i As Integer = 0 To WW_TBLview.Count - 1
            ...
            If WW_TBLview.Item(i)("DELFLG") = C_DELETE_FLG.ALIVE Then
                'カウンタークリア（日報番号毎）
                Dim WW_NIPPONO As String = WW_TBLview.Item(i)("NIPPONO")
                If Not WW_CHKLIST.ContainsKey(WW_NIPPONO) Then
                    WW_CHKLIST.Add(WW_NIPPONO, New List(Of Integer))
                    For j As Integer = 0 To 2
                        WW_CNT(j) = 0
                        WW_CHKLIST(WW_NIPPONO).Add(WW_CNT(j))
                    Next
                End If
                ...
                Select Case WW_TBLview.Item(i)("WORKKBN")
                    Case "B2"
                        WW_B2CNT += 1
                        If Mid(WW_TBLview.Item(i)("SHUKABASHO"), 1, 2) = C_ANOTHER_SYSTEMS_DISTINATION_PREFIX.JX Then
                            WW_CNT(0) += 1
                        ElseIf Mid(WW_TBLview.Item(i)("SHUKABASHO"), 1, 5) = C_ANOTHER_SYSTEMS_DISTINATION_PREFIX.COSMO Then
                            WW_CNT(1) += 1
                        Else
                            WW_CNT(2) += 1
                        End If
                    Case "B3"
                        ...
                        Else
                            WW_CNT(2) += 1
                        End If
                        If Mid(WW_TBLview.Item(i)("TODOKECODE"), 1, 2) = C_ANOTHER_SYSTEMS_DISTINATION_PREFIX.JX Then
                            WW_CNT(0) += 1
                        ElseIf Mid(WW_TBLview.Item(i)("TODOKECODE"), 1, 5) = C_ANOTHER_SYSTEMS_DISTINATION_PREFIX.COSMO Then
                            WW_CNT(1) += 1
                        Else
                            WW_CNT(2) += 1
                        End If
                ...
                If WW_TBLview.Item(i)("WORKKBN") = "A1" AndAlso
                   WW_TBLview.Item(i)("WORKKBN") = "Z1" AndAlso
                   WW_TBLview.Item(i)("WORKKBN") = "G1" Then
                    Continue For
                Else
                ...
                '統一コードのチェックリスト作成
                If WW_CHKLIST.ContainsKey(WW_NIPPONO) Then
                    WW_CHKLIST(WW_NIPPONO).Item(0) = WW_CNT(0)
                    WW_CHKLIST(WW_NIPPONO).Item(1) = WW_CNT(1)
                    WW_CHKLIST(WW_NIPPONO).Item(2) = WW_CNT(2)
                End If
            End If
        ...
        If work.WF_SEL_CAMPCODE.Text <> GRT00005WRKINC.C_COMP_ENEX Then
            'ENEX以外の場合、始業、配送、配送、出庫、帰庫をチェック対象外とする
            WW_TBLview.RowFilter = "(HDKBN = 'D' and DELFLG = '0') and (WORKKBN <> 'A1' and WORKKBN <> 'G1' and WORKKBN <> 'Z1' and WORKKBN <> 'F1' and WORKKBN <> 'F3')"
        Else
            'ENEXの場合、始業、配送、配送をチェック対象外とする
        ...
        For i As Integer = 0 To WW_TBLview.Count - 1
            ...
            If i >= 1 Then
                If WW_TBLview.Item(i - 1)("NIPPONO") = WW_TBLview.Item(i)("NIPPONO") Then
                ...
                Else
                    If i = WW_TBLview.Count - 1 Then
                        WW_MOVETIME = T0005COM.HHMMToMinutes(WW_TBLview.Item(i)("MOVETIME"))
                    Else
                        WW_MOVETIME = 0
                    End If
                End If
        ...
        If WW_TBLview.Count > 0 Then
            ...
            For Each chk In WW_CHKLIST
                If (chk.Value.Item(0) > 0 AndAlso chk.Value.Item(1) > 0 AndAlso chk.Value.Item(2) > 0) OrElse
                   (chk.Value.Item(0) = 0 AndAlso chk.Value.Item(1) > 0 AndAlso chk.Value.Item(2) > 0) OrElse
                   (chk.Value.Item(0) > 0 AndAlso chk.Value.Item(1) = 0 AndAlso chk.Value.Item(2) > 0) OrElse
                   (chk.Value.Item(0) > 0 AndAlso chk.Value.Item(1) > 0 AndAlso chk.Value.Item(2) = 0) Then
                    Dim WW_ERR_MES As String = ""
                    WW_ERR_MES = "・エラーが存在します。(出荷場所、届先コードエラー)です。"
                    WW_ERR_MES = WW_ERR_MES & ControlChars.NewLine & "  --> コード体系を統一してください（日報№：" & chk.Key & "）,"
                    rightview.AddErrorReport(ControlChars.NewLine & WW_ERR_MES)
                    WW_ERRLIST.Add(C_MESSAGE_NO.BOX_ERROR_EXIST)
                    T0005INPtbl.Rows(idx)("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
                    O_RTN = C_MESSAGE_NO.BOX_ERROR_EXIST

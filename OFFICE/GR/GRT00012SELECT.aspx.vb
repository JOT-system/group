...
Public Class GRT00012SELECT
    ...
    Protected Sub WW_MAPValueSet(ByRef O_RTN As String)
        ...
        If IsNothing(Master.MAPID) Then
            Master.MAPID = GRT00012WRKINC.MAPIDS
        End If
        'メニューからの画面遷移
        If Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.MENU Then
            ...
            SetInitialValue()
            '実行画面からの画面遷移
        ElseIf Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.T00012 Then                                                   '実行画面からの画面遷移
            '■■■ 実行画面からの画面遷移 ■■■
            '○画面項目設定処理
            '会社コード　
        ...
        '○RightBox情報設定
        rightview.MAPID = GRT00012WRKINC.MAPID
        rightview.MAPIDS = GRT00012WRKINC.MAPIDS

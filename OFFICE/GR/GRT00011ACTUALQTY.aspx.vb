...
Public Class GRT00011ACTUALQTY
                ...
                Using SQLcmd2 As New SqlCommand(SQLStr2, SQLcon)
                    ...
                    Using WW_T0011tbl As DataTable = T0011tbl.Clone
                        For i As Integer = 0 To T0011tbl.Rows.Count - 1
                            ...
                            End If
                            If IsDate(T0011row("ENDTIME")) Then
    ...
    Protected Sub ERRMSG_write(ByRef I_MESSAGE1 As String, ByRef I_MESSAGE2 As String, ByRef WW_LINEerr As String, ByRef T0011INProw As DataRow, ByVal I_ERRCD As String)
        ...
        End If
        WW_ERR_MES = WW_ERR_MES & ControlChars.NewLine & "  --> 項番        =" & T0011INProw("LINECNT") & " , "
                ...
                If IO_TBLrow("WORKKBN") = "B3" Then
                    ...
                    For idx As Integer = 1 To 8
                        ...
                        If isNormal(O_RTN) Then
                            ...
                            Else
                                If IO_TBLrow("OILTYPE" & idx) = "" AndAlso Val(IO_TBLrow("SURYO" & idx)) > 0 Then
                                    'エラーレポート編集
                                    ERRMSG_write("・更新できないレコード(数量" & idx & "エラー)です。", "入力できません", WW_LINEerr, IO_TBLrow, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR)
                                    IO_TBLrow("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
                                End If
                            End If
                        Else
                    ...
                    For idx As Integer = 1 To 8
                        ...
                        If isNormal(O_RTN) Then
                            ...
                            If Val(iTblrow("SURYO" & idx)) < 0 Then
                                ...
                                IO_TBL.Rows(WW_SaveIdx)("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
                            Else
                                If iTblrow("OILTYPE" & idx) = "" AndAlso Val(iTblrow("SURYO" & idx)) > 0 Then
                                    'エラーレポート編集
                                    ERRMSG_write("・更新できないレコード(数量" & idx & "エラー)です。", "入力できません", C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR, iTblrow, C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR)
                                    iTblrow("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
                                    IO_TBL.Rows(WW_SaveIdx)("OPERATION") = C_LIST_OPERATION_CODE.ERRORED
                                End If
                            End If

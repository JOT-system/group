...
Public Class GRTA0003SELECT
            ...
            If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                Select Case WF_ButtonClick.Value
                    Case "WF_ButtonDO"                              '■実行ボタン押下時処理
                        WF_ButtonDO_Click()
                    Case "WF_ButtonDO_V2"                           '■実行（新）ボタン押下時処理
    ...
    Protected Sub WF_ButtonDO_V2_Click()
        ...
        If Not Master.MAPpermitcode = C_PERMISSION.INVALID Then
            ...
            CS0007CheckAuthority.MAPID = GRTA0003WRKINC_V2.MAPID
            ...
            If isNormal(CS0007CheckAuthority.ERR) Then
                ...
                If Master.MAPpermitcode < C_PERMISSION.REFERLANCE Then
                    Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRTA0003WRKINC_V2.MAPID)
                    Exit Sub
                End If
            Else
                Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRTA0003WRKINC_V2.MAPID)
    ...
    Protected Sub SetMapValue()
        ...
        If Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.MENU Then                                                      'メニューからの画面遷移
            ...
            '○画面項目設定（変数より）処理
            SetInitialValue()
        ElseIf Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.TA0003 OrElse
               Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.TA0003_V2 Then     '実行画面からの画面遷移
            '画面設定
        ...
        '○RightBox情報設定
        rightview.MAPID = GRTA0003WRKINC.MAPID
        rightview.MAPIDS = GRTA0003WRKINC.MAPIDS

...
Public Class GRMC006UPDATE
    ...
    ''' 配車（T4）共通ルーチン
    ''' </summary>
    Private T0004COM As GRT00004COM.KOUEI_MASTER                        '配車（T4）共通ルーチン（届先）
    ''' <summary>
    ''' 光英取込区分（JX、TG)
    ''' </summary>
    ...
    Public Sub Update(ByVal I_CSVTYPE As String, ByVal I_LEGACY_MODE As Boolean, ByVal I_TBL As DataTable)
        ...
        Dim WW_FIND As String = "True"

        ERR = C_MESSAGE_NO.NORMAL
        T0004COM = New GRT00004COM.KOUEI_MASTER
        ...
        CS0026TblSort.SORTING = "FIELD4,FIELD10,FIELD6,FIELD8"
        CS0026TblSort.FILTER = "FIELD13 = 'E2'"
        Dim WW_E2tbl As DataTable = CS0026TblSort.Sort()

        '---------------------------------------------
        '実績（shasai.csv）からマスター登録：名称はJX,COSMO,TGの場合は光英マスタを参照して取得する。ない場合は空白
        '---------------------------------------------
        '出荷場所（稼働日 "FIELD4" 従業員 "FIELD6" SEQ "FIELD8" 日報番号 "FIELD10" 荷主 "FIELD34",、ﾄﾘｯﾌﾟ "FIELD16" 荷主 "FIELD34"、出荷場所 "FIELD35"、緯度 FIELD52、経度 FIELD53）
        Dim B2query = (From x As DataRow In I_TBL.Rows
                       Where x("FIELD13") = "B2"
                       Order By x("FIELD4"), x("FIELD10"), x("FIELD6"), Val(x("FIELD8"))
                       Select FIELD4 = x("FIELD4"), FIELD10 = x("FIELD10"), FIELD6 = Val(x("FIELD6")), FIELD8 = x("FIELD8"), FIELD16 = x("FIELD16"), FIELD34 = x("FIELD34"), FIELD35 = x("FIELD35"), FIELD52 = x("FIELD52"), FIELD53 = x("FIELD53")
                     ).Distinct

        Dim WW_IDX As Integer = 0
        Dim B2_KEY As String = ""
        Dim E2_KEY As String = ""
        For Each B2FILED In B2query
            If I_CSVTYPE = GRT00005WRKINC.TERM_TYPE.JX OrElse
               I_CSVTYPE = GRT00005WRKINC.TERM_TYPE.TG OrElse
               I_CSVTYPE = GRT00005WRKINC.TERM_TYPE.COSMO Then
                Dim WW_YMD As String = If(IsDBNull(B2FILED.FIELD4), Date.Now.ToString("yyyy/MM/dd"), Mid(B2FILED.FIELD4, 1, 4) & "/" & Mid(B2FILED.FIELD4, 5, 2) & "/" & Mid(B2FILED.FIELD4, 7, 2))
                Select Case I_CSVTYPE
                    Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_JX, B2FILED.FIELD35, String.Empty, "2", If(IsDBNull(B2FILED.FIELD52), String.Empty, B2FILED.FIELD52), If(IsDBNull(B2FILED.FIELD53), String.Empty, B2FILED.FIELD53), WW_YMD, WW_FIND)
                    Case GRT00005WRKINC.TERM_TYPE.COSMO
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_COSMO, B2FILED.FIELD35, String.Empty, "2", If(IsDBNull(B2FILED.FIELD52), String.Empty, B2FILED.FIELD52), If(IsDBNull(B2FILED.FIELD53), String.Empty, B2FILED.FIELD53), WW_YMD, WW_FIND)
                End Select
                '届先マスタ（出荷場所）追加（登録済で住所１が設定されている場合は更新しない）
                If WW_FIND <> "NotFound" Then
                    UpdateMC006Tbl(I_CSVTYPE, WW_YMD, WW_FIND, WW_RTN)
                    If Not isNormal(WW_RTN) Then
                        'SQLtrn.Rollback()
                        ERR = WW_RTN
                        Exit Sub
                ...
                End If
            Else
                'JXコスモ以外（電話受注分は、取引先を取得するためE2レコードする。また自システムコードのため光英マスター参照しない）
                Dim WW_YMD As String = If(IsDBNull(B2FILED.FIELD4), Date.Now.ToString("yyyy/MM/dd"), Mid(B2FILED.FIELD4, 1, 4) & "/" & Mid(B2FILED.FIELD4, 5, 2) & "/" & Mid(B2FILED.FIELD4, 7, 2))
                B2_KEY = B2FILED.FIELD4 & B2FILED.FIELD10 & B2FILED.FIELD6 & B2FILED.FIELD16
                For i As Integer = WW_IDX To WW_E2tbl.Rows.Count - 1
                    E2_KEY = WW_E2tbl.Rows(i)("FIELD4") & WW_E2tbl.Rows(i)("FIELD10") & WW_E2tbl.Rows(i)("FIELD6") & WW_E2tbl.Rows(i)("FIELD16")
                    If B2_KEY = E2_KEY Then
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, WW_E2tbl.Rows(i)("FIELD34"), WW_E2tbl.Rows(i)("FIELD35"), String.Empty, "2", B2FILED.FIELD52, B2FILED.FIELD53, WW_YMD, WW_FIND)
                        '届先マスタ（出荷場所）追加（登録済で住所１が設定されている場合は更新しない）
                        If WW_FIND <> "NotFound" Then
                            UpdateMC006Tbl(I_CSVTYPE, WW_YMD, WW_FIND, WW_RTN)
                            If Not isNormal(WW_RTN) Then
            ...
            'Dim E2query = (From x As DataRow In WW_E2tbl.Rows
            '               Where x("FIELD13") = "E2" And B2FILED.FIELD4 = x("FIELD4") And B2FILED.FIELD10 = x("FIELD10") And B2FILED.FIELD6 = x("FIELD6") And B2FILED.FIELD8 < Val(x("FIELD8")) And B2FILED.FIELD16 = x("FIELD16")
            ...
            '    Select Case I_CSVTYPE
            '        Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG
            '            SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_JX, E2FILED.FIELD35, String.Empty, "2", If(IsDBNull(B2FILED.FIELD52), String.Empty, B2FILED.FIELD52), If(IsDBNull(B2FILED.FIELD53), String.Empty, B2FILED.FIELD53), WW_YMD, WW_FIND)
            '        Case GRT00005WRKINC.TERM_TYPE.COSMO
            '            SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_COSMO, E2FILED.FIELD35, String.Empty, "2", If(IsDBNull(B2FILED.FIELD52), String.Empty, B2FILED.FIELD52), If(IsDBNull(B2FILED.FIELD53), String.Empty, B2FILED.FIELD53), WW_YMD, WW_FIND)
            '        Case Else
            '            SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, E2FILED.FIELD34, E2FILED.FIELD35, String.Empty, "2", B2FILED.FIELD52, B2FILED.FIELD53, WW_YMD, WW_FIND)
            '    End Select
            '    '届先マスタ（出荷場所）追加（登録済で住所１が設定されている場合は更新しない）
            '    UpdateMC006Tbl(WW_YMD, WW_FIND, WW_RTN)
            '    If Not isNormal(WW_RTN) Then
            '        'SQLtrn.Rollback()
            '        ERR = WW_RTN
            '        Exit Sub
        ...
        '届先（稼働日 "FIELD4" 従業員 "FIELD6" SEQ "FIELD8" 日報番号 "FIELD10" 荷主 "FIELD34",、ﾄﾘｯﾌﾟ "FIELD16" 届先 "FIELD36"、緯度 FIELD52、経度 FIELD53）
        Dim B3query = (From x As DataRow In I_TBL.Rows
                       Where x("FIELD13") = "B3"
                       Order By x("FIELD4"), x("FIELD10"), x("FIELD6"), Val(x("FIELD8"))
                       Select FIELD4 = x("FIELD4"), FIELD10 = x("FIELD10"), FIELD6 = x("FIELD6"), FIELD8 = Val(x("FIELD8")), FIELD52 = x("FIELD52"), FIELD53 = x("FIELD53")
                     ).Distinct

        'WW_IDX = 0
        'Dim B3_KEY As String = ""
        'Dim OLD_TRIP As String = ""
        'Dim WW_First As String = "OFF"
        For Each B3FILED In B3query
            'B3_KEY = B3FILED.FIELD4 & B3FILED.FIELD10 & B3FILED.FIELD6
            'For i As Integer = WW_IDX To WW_E2tbl.Rows.Count - 1
            '    E2_KEY = WW_E2tbl.Rows(i)("FIELD4") & WW_E2tbl.Rows(i)("FIELD10") & WW_E2tbl.Rows(i)("FIELD6")
            '    If B3_KEY = E2_KEY Then
            '        '同一トリップ番号のみ処理する（上記E2queryでは余計なE2（次のB3のE2も取得してしまうため））
            '        If WW_First = "OFF" Then
            '            WW_First = "ON"
            '            OLD_TRIP = WW_E2tbl.Rows(i)("FIELD16")
            '        End If

            '        If WW_E2tbl.Rows(i)("FIELD16") <> OLD_TRIP Then
            '            OLD_TRIP = WW_E2tbl.Rows(i)("FIELD16")
            '            WW_IDX = i
            '            Exit For
            '        End If
            ...
            '        Dim WW_YMD As String = If(IsDBNull(B3FILED.FIELD4), Date.Now.ToString("yyyy/MM/dd"), Mid(B3FILED.FIELD4, 1, 4) & "/" & Mid(B3FILED.FIELD4, 5, 2) & "/" & Mid(B3FILED.FIELD4, 7, 2))
            '        Select Case I_CSVTYPE
            '            Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG
            '                SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_JX, WW_E2tbl.Rows(i)("FIELD36"), WW_E2tbl.Rows(i)("FIELD37"), "1", If(IsDBNull(B3FILED.FIELD52), String.Empty, B3FILED.FIELD52), If(IsDBNull(B3FILED.FIELD53), String.Empty, B3FILED.FIELD53), WW_YMD, WW_FIND)
            '            Case GRT00005WRKINC.TERM_TYPE.COSMO
            '                SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_COSMO, WW_E2tbl.Rows(i)("FIELD36"), WW_E2tbl.Rows(i)("FIELD37"), "1", If(IsDBNull(B3FILED.FIELD52), String.Empty, B3FILED.FIELD52), If(IsDBNull(B3FILED.FIELD53), String.Empty, B3FILED.FIELD53), WW_YMD, WW_FIND)
            '            Case Else
            '                SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, WW_E2tbl.Rows(i)("FIELD34"), WW_E2tbl.Rows(i)("FIELD36"), WW_E2tbl.Rows(i)("FIELD37"), "1", B3FILED.FIELD52, B3FILED.FIELD53, WW_YMD, WW_FIND)
            '        End Select
            '        '届先マスタ（届先）追加（登録済で住所１が設定されている場合は更新しない）
            '        UpdateMC006Tbl(WW_YMD, WW_FIND, WW_RTN)
            '        If Not isNormal(WW_RTN) Then
            '            'SQLtrn.Rollback()
            '            ERR = WW_RTN
            ...
            Dim E2query = (From x As DataRow In WW_E2tbl.Rows
                           Where x("FIELD13") = "E2" And B3FILED.FIELD4 = x("FIELD4") And B3FILED.FIELD10 = x("FIELD10") And B3FILED.FIELD6 = x("FIELD6") And B3FILED.FIELD8 < Val(x("FIELD8"))
                           Order By x("FIELD8"), x("FIELD16"), x("FIELD34"), x("FIELD36"), Val(x("FIELD37"))
                           Select FIELD16 = x("FIELD16"), FIELD34 = x("FIELD34"), FIELD36 = x("FIELD36"), FIELD37 = x("FIELD37")
                              ).Distinct
            ...
            For Each E2FILED In E2query
                ...
                If WW_First = "OFF" Then
                    WW_First = "ON"
                    OLD_TRIP = E2FILED.FIELD16
                End If

                If E2FILED.FIELD16 <> OLD_TRIP Then
                    Exit For
                End If
                OLD_TRIP = E2FILED.FIELD16
                ...
                Select Case I_CSVTYPE
                    Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_JX, E2FILED.FIELD36, E2FILED.FIELD37, "1", If(IsDBNull(B3FILED.FIELD52), String.Empty, B3FILED.FIELD52), If(IsDBNull(B3FILED.FIELD53), String.Empty, B3FILED.FIELD53), WW_YMD, WW_FIND)
                    Case GRT00005WRKINC.TERM_TYPE.COSMO
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, GRT00005WRKINC.C_TORICODE_COSMO, E2FILED.FIELD36, E2FILED.FIELD37, "1", If(IsDBNull(B3FILED.FIELD52), String.Empty, B3FILED.FIELD52), If(IsDBNull(B3FILED.FIELD53), String.Empty, B3FILED.FIELD53), WW_YMD, WW_FIND)
                    Case Else
                        SetValueFromMC006(I_CSVTYPE, I_LEGACY_MODE, E2FILED.FIELD34, E2FILED.FIELD36, E2FILED.FIELD37, "1", B3FILED.FIELD52, B3FILED.FIELD53, WW_YMD, WW_FIND)
                End Select
                '届先マスタ（届先）追加（登録済で住所１が設定されている場合は更新しない）
                If WW_FIND <> "NotFound" Then
                    UpdateMC006Tbl(I_CSVTYPE, WW_YMD, WW_FIND, WW_RTN)
    ...
    ''' <remarks></remarks>
    Protected Sub SetValueFromMC006(ByVal I_CSVTYPE As String,
                                    ByVal I_LEGACY_MODE As Boolean,
                                    ByVal I_TORICODE As String,
                                    ByVal I_TODOKECODE As String,
                                    ByVal I_TODOKESEQ As String,
            ...
            If I_CSVTYPE = GRT00005WRKINC.TERM_TYPE.JX OrElse
               ...
               I_CSVTYPE = GRT00005WRKINC.TERM_TYPE.TG Then
                ...
                If I_CLASS = "1" Then
                    Select Case I_CSVTYPE
                        Case GRT00005WRKINC.TERM_TYPE.JX, GRT00005WRKINC.TERM_TYPE.TG
                            'WW_TODOKECODE = If(I_LEGACY_MODE, I_TODOKECODE.PadLeft(6, "0"), I_TODOKECODE.PadLeft(9, "0")) &
                            WW_TODOKECODE = If(I_LEGACY_MODE, I_TODOKECODE.PadLeft(6, "0"), I_TODOKECODE) &
                                            If(I_LEGACY_MODE, I_TODOKESEQ.PadLeft(3, "0"), "")
                            WW_TYPE = "jxtg"
                        Case GRT00005WRKINC.TERM_TYPE.COSMO
                            'WW_TODOKECODE = I_TODOKECODE.PadLeft(11, "0")
                            WW_TODOKECODE = I_TODOKECODE
                            WW_TYPE = "cosmo"
                        Case Else
                            WW_TODOKECODE = I_TODOKECODE
                    End Select
                ...
                Dim TDK = T0004COM.GetTodoke(WW_TYPE, WW_TODOKECODE)
                If Not IsNothing(TDK) Then
                    MC006Row("TODOKENAMES") = TDK.NAME
                    MC006Row("TODOKENAMEL") = TDK.NAME
                    MC006Row("LATITUDE") = If(String.IsNullOrEmpty(MC006Row("LATITUDE")), TDK.LATITUDE, MC006Row("LATITUDE"))
                    MC006Row("LONGITUDE") = If(String.IsNullOrEmpty(MC006Row("LONGITUDE")), TDK.LONGTIDUDE, MC006Row("LONGITUDE"))
                Else
                    '光英マスターに存在しない場合は、登録しない
                    O_RTN = "NotFound"
                    Exit Sub
                End If
            End If

            Dim SQLStr2 As String = " SELECT                                " _
                                  ...
                                  & " WHERE                                 " _
                                  & "      CAMPCODE                  = @P01 " _
                                  & " and  rtrim(TODOKECODE)         = @P02 " _
                                  & " and  STYMD                    <= @P03 " _
                                  & " and  ENDYMD                   >= @P03 " _
                                  & " and  DELFLG                   <> '1'  " _
                                  & "                                       "
            Using SQLcmd As New SqlCommand(SQLStr2, SQLcon, SQLtrn)
                Dim PARA01 As SqlParameter = SQLcmd.Parameters.Add("@P01", System.Data.SqlDbType.NVarChar, 20)
                Dim PARA02 As SqlParameter = SQLcmd.Parameters.Add("@P02", System.Data.SqlDbType.NVarChar, 20)
                Dim PARA03 As SqlParameter = SQLcmd.Parameters.Add("@P03", System.Data.SqlDbType.Date)

                PARA01.Value = MC006Row("CAMPCODE")
                PARA02.Value = MC006Row("TODOKECODE")
                PARA03.Value = I_YMD
                ...
                Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                    If SQLdr.HasRows Then
                        If SQLdr.Read Then
                            If SQLdr("ADDR1") <> "" Then
                                MC006Row("ADDR1") = SQLdr("ADDR1")
                                MC006Row("ADDR2") = SQLdr("ADDR2")
                                MC006Row("ADDR3") = SQLdr("ADDR3")
                                MC006Row("ADDR4") = SQLdr("ADDR4")
                                MC006Row("CITIES") = SQLdr("CITIES")
                                O_RTN = "True"
                            Else
                                If IsNumeric(MC006Row("LATITUDE")) AndAlso IsNumeric(MC006Row("LONGITUDE")) Then
    ...
    ''' <param name="O_RTN">可否判定</param>
    ''' <remarks></remarks>
    Protected Sub UpdateMC006Tbl(ByVal I_CSVTYPE As String, ByVal I_YMD As String, ByVal I_FIND As String, ByRef O_RTN As String)
        ...
        Try
            '届先マスタの住所１が未入力の場合とJX＆COSMOの場合、届先マスターを更新
            If I_FIND = "False" Then
                ...
                Using SQLcmd As SqlCommand = New SqlCommand(SQLStr, SQLcon, SQLtrn)
                    ...
                    Dim PARA40 As SqlParameter = SQLcmd.Parameters.Add("@P40", System.Data.SqlDbType.DateTime)
...
Public Class GRT0004UPDATE
        ...
        For Each WW_T0004row As DataRow In WW_T0004tbl.Rows
            ...
            For j As Integer = 0 To WW_T0004UPDtbl.Rows.Count - 1
                ...
                If WW_T0004row("TORICODE") = WW_T0004UPDrow("TORICODE") AndAlso
                   ...
                   WW_T0004row("SHUKODATE") = WW_T0004UPDrow("SHUKODATE") AndAlso
    ...
    Private Sub EditT0004(ByVal I_ROW As DataRow,
        ...
        '端末設置部署
        WW_T0004row("TERMORG") = I_ROW("SHIPORG")
        '受注番号
        ...
        For i As Integer = 0 To ListBoxGSHABAN.Items.Count - 1
            ...
            If WW_SPLIT.Count > 0 AndAlso WW_SPLIT(2) = I_ROW("GSHABAN") Then
    ...
    Private Sub GetStoriCode(ByVal I_ROW As DataRow, ByRef O_STORICODE As String)
        Dim WW_date As String = String.Empty
        Static dicSTori As Dictionary(Of String, String) = New Dictionary(Of String, String)

        Try
            Dim dicKey As String = String.Join("|", {I_ROW("CAMPCODE").ToString, I_ROW("SHIPORG").ToString, I_ROW("TORICODE").ToString})
            If dicSTori.TryGetValue(dicKey, O_STORICODE) Then
                Exit Sub
            End If
            ...
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon, SQLtrn)
                ...
                Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                    ...
                    While SQLdr.Read
                        ...
                        dicSTori(dicKey) = SQLdr("STORICODE").ToString
                    End While
...
Public Class GRT0005COM
    ...
    Public Sub ReEditTumiOkiNJST0005(ByVal I_CAMPCODE As String, ByRef IO_T5tbl As DataTable)
        ...
        CS0026TBLSort.TABLE = IO_T5tbl
        CS0026TBLSort.SORTING = "CAMPCODE, STAFFCODE, GSHABAN, NIPPONO, YMD, STDATE, STTIME DESC, CREWKBN DESC"
        CS0026TBLSort.FILTER = String.Empty
        IO_T5tbl = CS0026TBLSort.sort()
        '〇データ整備2（トリップ・ドロップ再付番）
        '　Keyブレーク用
        ...
        For i As Integer = 0 To IO_T5tbl.Rows.Count - 1
            ...
            Else
                '業務車番Break（業務車番、出庫日Break）
                If (T0005row("CAMPCODE") <> wCAMPCODE) OrElse (T0005row("STAFFCODE") <> wSTAFFCODE) OrElse (T0005row("GSHABAN") <> wGSHABAN) OrElse (T0005row("NIPPONO") <> wNIPPONO) OrElse (T0005row("YMD") <> wYMD) Then
        ...
        '〇ソート（乗務員、業務車番、出庫日、開始日時、乗務員区分）
        CS0026TBLSort.TABLE = IO_T5tbl
        CS0026TBLSort.SORTING = "CAMPCODE, STAFFCODE, GSHABAN, NIPPONO, YMD, STDATE, STTIME, CREWKBN"
        CS0026TBLSort.FILTER = String.Empty
        IO_T5tbl = CS0026TBLSort.sort()
        ...
        '〇ソート（乗務員、業務車番、出庫日、開始日時、乗務員区分）
        CS0026TBLSort.TABLE = IO_T5tbl
        CS0026TBLSort.SORTING = "CAMPCODE, STAFFCODE, GSHABAN, NIPPONO, YMD, STDATE, STTIME DESC, ENDTIME DESC, CREWKBN"
        CS0026TBLSort.FILTER = String.Empty
        IO_T5tbl = CS0026TBLSort.sort()
        ...
        For i As Integer = 0 To IO_T5tbl.Rows.Count - 1
            ...
            If (T0005row("HDKBN") <> "H") AndAlso (T0005row("WORKKBN") <> "A1") AndAlso (T0005row("WORKKBN") <> "Z1") Then
                If (T0005row("CAMPCODE") <> wCAMPCODE) OrElse
                   ...
                   (T0005row("NIPPONO") <> wNIPPONO) OrElse
                   (T0005row("YMD") <> wYMD) Then
        ...
        '〇ソート（業務車番、出庫日、開始日時、乗務員区分）
        CS0026TBLSort.TABLE = IO_T5tbl
        CS0026TBLSort.SORTING = "CAMPCODE, STAFFCODE, GSHABAN, NIPPONO, YMD, STDATE, STTIME, ENDTIME, CREWKBN"
        CS0026TBLSort.FILTER = String.Empty
        IO_T5tbl = CS0026TBLSort.sort()
        For i As Integer = 0 To IO_T5tbl.Rows.Count - 1
            Dim T0005row As DataRow = WW_I_T5tbl.NewRow

...
Public Class GRT00015SELECT
    ...
    Protected Sub Initialize()
        '○初期値設定
        Master.MAPID = GRT00015WRKINC.MAPIDS
        WF_FIELD.Value = ""
        WF_CAMPCODE.Focus()
        ...
        '○RightBox情報設定
        rightview.MAPID = GRT00015WRKINC.MAPID
        rightview.MAPIDS = GRT00015WRKINC.MAPIDS
    ...
    Protected Sub WF_ButtonRESTART_Click()
        ...
        Master.RecoverTable(T0015PARMtbl, work.WF_SEL_XMLsavePARM.Text)

        Dim T0015PARMrow = (From parm In T0015PARMtbl.AsEnumerable Select parm).FirstOrDefault
        If IsNothing(T0015PARMrow) Then
            Master.Output(C_MESSAGE_NO.FILE_IO_ERROR, C_MESSAGE_TYPE.ABORT, "一時保存条件パラメータ")
            Exit Sub
        End If

        '会社コード　
        work.WF_SEL_CAMPCODE.Text = T0015PARMrow("CAMPCODE")
        '出庫日
        work.WF_SEL_SHUKODATEF.Text = T0015PARMrow("SHUKODATEF")
        work.WF_SEL_SHUKODATET.Text = T0015PARMrow("SHUKODATET")
        '出荷日
        work.WF_SEL_SHUKADATEF.Text = T0015PARMrow("SHUKADATEF")
        work.WF_SEL_SHUKADATET.Text = T0015PARMrow("SHUKADATET")
        '届日　
        work.WF_SEL_TODOKEDATEF.Text = T0015PARMrow("TODOKEDATEF")
        work.WF_SEL_TODOKEDATET.Text = T0015PARMrow("TODOKEDATET")
        '出荷部署
        work.WF_SEL_SHIPORG.Text = T0015PARMrow("SHIPORG")
        '油種
        work.WF_SEL_OILTYPE.Text = T0015PARMrow("OILTYPE")
    ...
    Protected Sub WW_MAPValueSet()
        ...
        '■■■ 選択画面の入力初期値設定 ■■■
        work.WF_SEL_XMLsavePARM.Text = CS0050SESSION.UPLOAD_PATH & "\XML_TMP\" & Date.Now.ToString("yyyyMMdd") & "-" & Master.USERID & "-" & GRT00015WRKINC.MAPIDS & "-" & "PARM.txt"
        work.WF_SEL_XMLsaveTmp.Text = CS0050SESSION.UPLOAD_PATH & "\XML_TMP\" & Date.Now.ToString("yyyyMMdd") & "-" & Master.USERID & "-" & GRT00015WRKINC.MAPID & "-" & "TMP.txt"
        ...
        ElseIf Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.T00015 Then     '実行画面からの画面遷移
            ...
            If System.IO.File.Exists(work.WF_SEL_XMLsavePARM.Text) Then
                ...
                'テーブルデータ 復元
                work.PARMtbl_ColumnsAdd(T0015PARMtbl)
                If Not Master.RecoverTable(T0015PARMtbl, work.WF_SEL_XMLsavePARM.Text) Then
                    Exit Sub
                End If

                For Each PARMrow As DataRow In T0015PARMtbl.Rows
                    '会社コード　

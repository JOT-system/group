...
''' </summary>
''' <remarks></remarks>
Public Class GRT00009JIMKINTAI_V2
    Inherits Page
    ...
    Private T0007UPDATE As New GRT0007UPDATE_V2             '勤怠DB更新
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = GRT00009WRKINC_V2.MAPID
        Master.VERSION = "V2"
    ...
    Protected Sub WW_MAPValueSet(ByRef O_MSG As String, ByRef O_RTN As String)
        ...
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.T00009S Then          '検索画面からの遷移
            WF_BEFORE_MAPID.Value = GRT00009WRKINC_V2.MAPIDS
            ...
            If Trim(work.WF_SEL_HORG.Text) = "" Then
                prmdata = work.CreateStaffCodeParam(GL0005StaffList.LC_STAFF_TYPE.CLERK, work.WF_SEL_CAMPCODE.Text,
        ...
        '○ 勤怠個人
        If Master.MAPvariant Like GRT00009WRKINC_V2.VAR_ALL Then
            WF_ONLY.Value = "FALSE"
        Else
            WF_ONLY.Value = "TRUE"
        End If
        ...
        'その他作業部署取得
        Dim specialOrg As ListBox = T0007COM.getList(work.WF_SEL_CAMPCODE.Text, GRT00007WRKINC_V2.CONST_SPEC)
    ...
    Protected Sub GridViewInitialize()
        ...
        If work.WF_SEL_RESTARTFLG.Text = "TRUE" Then
            If Not Master.RecoverTable(T00009tbl, work.WF_SEL_XMLsaveTMP_V2.Text) Then
                Exit Sub
            End If
        Else
            '○ 画面表示データ取得
        ...
        If Not String.IsNullOrEmpty(work.WF_SEL_XMLsaveTMP_V2.Text) Then
            System.IO.File.Delete(work.WF_SEL_XMLsaveTMP_V2.Text)
        End If
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As SqlConnection)
        ...
        Try
            ...
            SQLstr =
                ...
                & "    , ISNULL(RTRIM(T007.ROSAIYUKYNIUSSU), '0')             AS ROSAIYUKYNIUSSU" _
                & "    , ISNULL(RTRIM(T007.ROSAIYUKYNIUSSUCHO), '0')          AS ROSAIYUKYNIUSSUCHO" _
                & "    , '0'                                                  AS ROSAIYUKYNIUSSUTTL" _
                & "    , ISNULL(RTRIM(T007.TOKUKYUMUKYUNISSU), '0')           AS TOKUKYUMUKYUNISSU" _
                & "    , ISNULL(RTRIM(T007.TOKUKYUMUKYUNISSUCHO), '0')        AS TOKUKYUMUKYUNISSUCHO" _
                & "    , '0'                                                  AS TOKUKYUMUKYUNISSUTTL" _
                & "    , ISNULL(RTRIM(T007.KOKANGOYUKYUNISSU), '0')           AS KOKANGOYUKYUNISSU" _
                & "    , ISNULL(RTRIM(T007.KOKANGOYUKYUNISSUCHO), '0')        AS KOKANGOYUKYUNISSUCHO" _
                & "    , '0'                                                  AS KOKANGOYUKYUNISSUTTL" _
                & "    , ISNULL(RTRIM(T007.KOKANGOMUKYUNISSU), '0')           AS KOKANGOMUKYUNISSU" _
                & "    , ISNULL(RTRIM(T007.KOKANGOMUKYUNISSUCHO), '0')        AS KOKANGOMUKYUNISSUCHO" _
                & "    , '0'                                                  AS KOKANGOMUKYUNISSUTTL" _
                ...
                & "    , ISNULL(RTRIM(T007.OUENKAISU), '0')                   AS OUENKAISU" _
                & "    , ISNULL(RTRIM(T007.OUENKAISUCHO), '0')                AS OUENKAISUCHO" _
                & "    , '0'                                                  AS OUENKAISUTTL" _
                & "    , ISNULL(RTRIM(T007.HANCHOKAISU), '0')                 AS HANCHOKAISU" _
                & "    , ISNULL(RTRIM(T007.HANCHOKAISUCHO), '0')              AS HANCHOKAISUCHO" _
                & "    , '0'                                                  AS HANCHOKAISUTTL" _
                & "    , ISNULL(RTRIM(T007.YOKSAIKENSADAISU), '0')            AS YOKSAIKENSADAISU" _
                & "    , ISNULL(RTRIM(T007.YOKSAIKENSADAISUCHO), '0')         AS YOKSAIKENSADAISUCHO" _
                & "    , '0'                                                  AS YOKSAIKENSADAISUTTL" _
                & "    , ISNULL(RTRIM(T007.KIMITUKENSADAISU), '0')            AS KIMITUKENSADAISU" _
                & "    , ISNULL(RTRIM(T007.KIMITUKENSADAISUCHO), '0')         AS KIMITUKENSADAISUCHO" _
                & "    , '0'                                                  AS KIMITUKENSADAISUTTL" _
            ...
            CS0026TBLSORT.SORTING = "CAMPCODE, STAFFCODE, WORKDATE, STDATE, STTIME, ENDDATE, ENDTIME, HDKBN DESC"
            ...
            For Each T00009row As DataRow In T00009tbl.Rows
                ...
                Dim WW_KEY As String = T00009row("CAMPCODE") & "," & T00009row("STAFFCODE")
                ...
                '2020/04/13 ADD
                WW_TOTAL = CInt(T00009row("ROSAIYUKYNIUSSU")) + CInt(T00009row("ROSAIYUKYNIUSSUCHO"))
                T00009row("ROSAIYUKYNIUSSUTTL") = WW_TOTAL.ToString()                                   '労災合計

                WW_TOTAL = CInt(T00009row("TOKUKYUMUKYUNISSU")) + CInt(T00009row("TOKUKYUMUKYUNISSUCHO"))
                T00009row("TOKUKYUMUKYUNISSUTTL") = WW_TOTAL.ToString()                                 '特休（無給）合計

                WW_TOTAL = CInt(T00009row("KOKANGOYUKYUNISSU")) + CInt(T00009row("KOKANGOYUKYUNISSUCHO"))
                T00009row("KOKANGOYUKYUNISSUTTL") = WW_TOTAL.ToString()                                 '子看護休暇日数（有給）合計

                WW_TOTAL = CInt(T00009row("KOKANGOMUKYUNISSU")) + CInt(T00009row("KOKANGOMUKYUNISSUCHO"))
                T00009row("KOKANGOMUKYUNISSUTTL") = WW_TOTAL.ToString()                                 '子看護休暇日数（有給）合計
                ...
                '2020/04/13　ADD
                WW_TOTAL = CInt(T00009row("OUENKAISU")) + CInt(T00009row("OUENKAISUCHO"))
                T00009row("OUENKAISUTTL") = WW_TOTAL.ToString()                                         '応援手当回数合計

                WW_TOTAL = CInt(T00009row("HANCHOKAISU")) + CInt(T00009row("HANCHOKAISUCHO"))
                T00009row("HANCHOKAISUTTL") = WW_TOTAL.ToString()                                       '班長会議手当回数合計

                WW_TOTAL = CInt(T00009row("YOKSAIKENSADAISU")) + CInt(T00009row("YOKSAIKENSADAISUCHO"))
                T00009row("YOKSAIKENSADAISUTTL") = WW_TOTAL.ToString()                                  '容器再検査台数合計

                WW_TOTAL = CInt(T00009row("KIMITUKENSADAISU")) + CInt(T00009row("KIMITUKENSADAISUCHO"))
                T00009row("KIMITUKENSADAISUTTL") = WW_TOTAL.ToString()                                  '気密検査台数合計
    ...
    Protected Sub ZeroToBlank(ByRef I_VIEW As DataView)
        ...
        For Each row As DataRow In I_VIEW.Table.Rows
            ...
            For Each col As DataColumn In I_VIEW.Table.Columns
                '下記項目は00:00をブランクに変更
                If col.ColumnName = "ACTTIME" OrElse
                ...
                '拘束開始は開始日=終了日かつ開始時刻＝終了時刻の時にブランクに変更
                If col.ColumnName = "BINDSTDATE" AndAlso row("STDATE") = row("ENDDATE") AndAlso row("STTIME") = row("ENDTIME") Then
                    Dim WW_TIME As String() = row(col).Split(":")
                ...
                '出社時刻は開始日=終了日かつ開始時刻＝終了時刻の時にブランクに変更
                If col.ColumnName = "ENDTIME" AndAlso row("STDATE") = row("ENDDATE") OrElse
                    col.ColumnName = "STTIME" AndAlso row("STDATE") = row("ENDDATE") AndAlso row("STTIME") = row("ENDTIME") Then
                    Dim WW_TIME As String() = row(col).Split(":")
    ...
    Protected Sub DisplayAdjust()
        ...
        For Each T00009TTLrow As DataRow In T00009TTLtbl.Rows
            ...
            '2020/04/13 ADD
            WF_ROSAIYUKYNIUSSUTTL.Text = T00009TTLrow("ROSAIYUKYNIUSSUTTL")             '労災欠勤日数（有給）合計
            WF_TOKUKYUMUKYUNISSUTTL.Text = T00009TTLrow("TOKUKYUMUKYUNISSUTTL")         '特別休暇日数（無給）合計
            WF_KOKANGOYUKYUNISSUTTL.Text = T00009TTLrow("KOKANGOYUKYUNISSUTTL")         '子看護休暇日数（有給）合計
            WF_KOKANGOMUKYUNISSUTTL.Text = T00009TTLrow("KOKANGOMUKYUNISSUTTL")         '子看護休暇日数（無給）合計
    ...
    Protected Sub WF_ButtonCALC_Click()
        ...
        '○ 各会社毎で残業計算を行う
        'エネックス
        If work.WF_SEL_CAMPCODE.Text = GRT00009WRKINC_V2.CAMP_ENEX Then
        ...
        '近石
        If work.WF_SEL_CAMPCODE.Text = GRT00009WRKINC_V2.CAMP_KNK Then
            T0007COM.T0007_KintaiCalc_KNK(WW_T00009SELtbl, T00009INPtbl)
        End If

        'ニュージェイズ
        If work.WF_SEL_CAMPCODE.Text = GRT00009WRKINC_V2.CAMP_NJS Then
            T0007COM.T0007_KintaiCalc_NJS(WW_T00009SELtbl, T00009INPtbl)
        End If

        'JKトランス
        If work.WF_SEL_CAMPCODE.Text = GRT00009WRKINC_V2.CAMP_JKT Then
    ...
    Protected Sub WF_ButtonSAVE_Click()
        ...
        '○ 一時保存ファイルに出力
        If Not Master.SaveTable(T00009tbl, work.WF_SEL_XMLsaveTMP_V2.Text) Then
            Exit Sub
        End If
    ...
    Protected Sub WF_ButtonUPDATE_Click()
        ...
        For Each UPDATErow As DataRow In WW_UPDATEtbl.Rows
            ...
            If UPDATErow("ENTRYFLG") = "1" Then
                ...
                CS0048Apploval.I_MAPID = Master.MAPID.Replace("_V2", "")
    ...
    Protected Sub RelatedCheck(ByRef O_RTN As String)
        ...
        For Each T00009row As DataRow In T00009tbl.Rows
            ...
            If T0007COM.CheckHOLIDAY("0", T00009row("PAYKBN")) Then
                ...
                '理由(その他)
                If T00009row("CAMPCODE") = GRT00009WRKINC_V2.CAMP_JKT Then
                    'JKTの場合、理由（その他）欄を備考として使用する
                Else
    ...
    Protected Sub InsertTableEdit(ByVal I_TABLE As DataTable, ByRef O_TABLE As DataTable, ByVal I_NOW As DateTime)
        ...
        For Each I_ROW As DataRow In I_TABLE.Rows
            ...
            '2020/04/13 ADD
            O_ROW("ROSAIYUKYNIUSSU") = I_ROW("ROSAIYUKYNIUSSU")                                 '労災欠勤日数（有給）
            O_ROW("ROSAIYUKYNIUSSUCHO") = I_ROW("ROSAIYUKYNIUSSUCHO")                           '労災欠勤日数（有給）調整
            O_ROW("TOKUKYUMUKYUNISSU") = I_ROW("TOKUKYUMUKYUNISSU")                             '特別休暇日数（無給）
            O_ROW("TOKUKYUMUKYUNISSUCHO") = I_ROW("TOKUKYUMUKYUNISSUCHO")                       '特別休暇日数（無給）調整
            O_ROW("KOKANGOYUKYUNISSU") = I_ROW("KOKANGOYUKYUNISSU")                             '子看護休暇日数（有給）
            O_ROW("KOKANGOYUKYUNISSUCHO") = I_ROW("KOKANGOYUKYUNISSUCHO")                       '子看護休暇日数（有給）調整
            O_ROW("KOKANGOMUKYUNISSU") = I_ROW("KOKANGOMUKYUNISSU")                             '子看護休暇日数（無給）
            O_ROW("KOKANGOMUKYUNISSUCHO") = I_ROW("KOKANGOMUKYUNISSUCHO")                       '子看護休暇日数（無給）調整
            ...
            '2020/04/13 ADD
            O_ROW("OUENKAISU") = I_ROW("OUENKAISU")                                             '応援手当回数
            O_ROW("OUENKAISUCHO") = I_ROW("OUENKAISUCHO")                                       '応援手当回数調整
            O_ROW("HANCHOKAISU") = I_ROW("HANCHOKAISU")                                         '班長会議手当回数
            O_ROW("HANCHOKAISUCHO") = I_ROW("HANCHOKAISUCHO")                                   '班長会議手当回数調整
            O_ROW("YOKSAIKENSADAISU") = I_ROW("YOKSAIKENSADAISU")                               '容器再検査台数
            O_ROW("YOKSAIKENSADAISUCHO") = I_ROW("YOKSAIKENSADAISUCHO")                         '容器再検査台数調整
            O_ROW("KIMITUKENSADAISU") = I_ROW("KIMITUKENSADAISU")                               '気密検査台数
            O_ROW("KIMITUKENSADAISUCHO") = I_ROW("KIMITUKENSADAISUCHO")                         '気密検査台数調整
    ...
    Protected Sub UpdateJimKintai(ByVal SQLcon As SqlConnection, ByVal I_TABLE As DataTable, ByVal I_NOW As DateTime)
        ...
        Dim SQLStr As String =
            ...
            & "    , @P161    , @P200    , @P201    , @P202    , @P203" _
            & "    , @P204    , @P205    , @P206    , @P207    , @P208" _
            & "    , @P209    , @P210    , @P211    , @P212    , @P213" _
            & "    , @P214    , @P215);"
        ...
        Try
            For Each WW_ROW As DataRow In I_TABLE.Rows
                ...
                Dim PARA200 As SqlParameter = SQLcmd.Parameters.Add("@P200", SqlDbType.Int)                 '労災欠勤日数（有給）
                Dim PARA201 As SqlParameter = SQLcmd.Parameters.Add("@P201", SqlDbType.Int)                 '労災欠勤日数（有給）調整
                Dim PARA202 As SqlParameter = SQLcmd.Parameters.Add("@P202", SqlDbType.Int)                 '特別休暇日数（無給）
                Dim PARA203 As SqlParameter = SQLcmd.Parameters.Add("@P203", SqlDbType.Int)                 '特別休暇日数（無給）調整
                Dim PARA204 As SqlParameter = SQLcmd.Parameters.Add("@P204", SqlDbType.Int)                 '子看護休暇日数（有給）
                Dim PARA205 As SqlParameter = SQLcmd.Parameters.Add("@P205", SqlDbType.Int)                 '子看護休暇日数（有給）調整
                Dim PARA206 As SqlParameter = SQLcmd.Parameters.Add("@P206", SqlDbType.Int)                 '子看護休暇日数（無給）
                Dim PARA207 As SqlParameter = SQLcmd.Parameters.Add("@P207", SqlDbType.Int)                 '子看護休暇日数（無給）調整
                Dim PARA208 As SqlParameter = SQLcmd.Parameters.Add("@P208", SqlDbType.Int)                 '応援手当回数
                Dim PARA209 As SqlParameter = SQLcmd.Parameters.Add("@P209", SqlDbType.Int)                 '応援手当回数調整
                Dim PARA210 As SqlParameter = SQLcmd.Parameters.Add("@P210", SqlDbType.Int)                 '班長会議手当回数
                Dim PARA211 As SqlParameter = SQLcmd.Parameters.Add("@P211", SqlDbType.Int)                 '班長会議手当回数調整
                Dim PARA212 As SqlParameter = SQLcmd.Parameters.Add("@P212", SqlDbType.Int)                 '容器再検査台数
                Dim PARA213 As SqlParameter = SQLcmd.Parameters.Add("@P213", SqlDbType.Int)                 '容器再検査台数調整
                Dim PARA214 As SqlParameter = SQLcmd.Parameters.Add("@P214", SqlDbType.Int)                 '気密検査台数
                Dim PARA215 As SqlParameter = SQLcmd.Parameters.Add("@P215", SqlDbType.Int)                 '気密検査台数調整
                ...
                '2020/04/13 ADD
                PARA200.Value = WW_ROW("ROSAIYUKYNIUSSU")
                PARA201.Value = WW_ROW("ROSAIYUKYNIUSSUCHO")
                PARA202.Value = WW_ROW("TOKUKYUMUKYUNISSU")
                PARA203.Value = WW_ROW("TOKUKYUMUKYUNISSUCHO")
                PARA204.Value = WW_ROW("KOKANGOYUKYUNISSU")
                PARA205.Value = WW_ROW("KOKANGOYUKYUNISSUCHO")
                PARA206.Value = WW_ROW("KOKANGOMUKYUNISSU")
                PARA207.Value = WW_ROW("KOKANGOMUKYUNISSUCHO")
                PARA208.Value = WW_ROW("OUENKAISU")
                PARA209.Value = WW_ROW("OUENKAISUCHO")
                PARA210.Value = WW_ROW("HANCHOKAISU")
                PARA211.Value = WW_ROW("HANCHOKAISUCHO")
                PARA212.Value = WW_ROW("YOKSAIKENSADAISU")
                PARA213.Value = WW_ROW("YOKSAIKENSADAISUCHO")
                PARA214.Value = WW_ROW("KIMITUKENSADAISU")
                PARA215.Value = WW_ROW("KIMITUKENSADAISUCHO")
    ...
    Protected Sub TOKEItblJimEdit(ByVal I_TABLE As DataTable, ByRef O_TABLE As DataTable, ByVal I_NOW As DateTime)
        ...
        For Each T00009row As DataRow In WW_T00009tbl.Rows
            Try
                ...
                '2020/04/13 ADD
                O_ROW("PAYROSAIYUKYNIUSSU") = T00009row("ROSAIYUKYNIUSSUTTL")                           '労災欠勤日数（有給）
                O_ROW("PAYTOKUKYUMUKYUNISSU") = T00009row("TOKUKYUMUKYUNISSUTTL")                       '特別休暇日数（無給）
                O_ROW("PAYKOKANGOYUKYUNISSU") = T00009row("KOKANGOYUKYUNISSUTTL")                       '子看護休暇日数（有給）
                O_ROW("PAYKOKANGOMUKYUNISSU") = T00009row("KOKANGOMUKYUNISSUTTL")                       '子看護休暇日数（無給）
                ...
                '2020/04/13 ADD
                O_ROW("PAYOUENKAISU") = 0                                                               '応援手当回数
                O_ROW("PAYHANCHOKAISU") = 0                                                             '班長会議手当回数
                O_ROW("PAYYOKSAIKENSADAISU") = 0                                                        '容器再検査台数
                O_ROW("PAYKIMITUKENSADAISU") = 0                                                        '気密検査台数
                O_ROW("APPKIJUN") = ""                                                                  '配賦基準
                O_ROW("APPKEY") = ""                                                                    '配賦統計キー
        ...
        For i As Integer = 0 To WW_T00009tbl.Rows.Count - 1
            Try
                ...
                If WW_DTLrow("SHARYOKBN") = "1" AndAlso WW_DTLrow("OILPAYKBN") = "01" Then
                    ...
                    '2020/04/13 ADD
                    O_ROW("PAYROSAIYUKYNIUSSU") = WW_HEADrow("ROSAIYUKYNIUSSUCHO")                      '労災欠勤日数（有給）
                    O_ROW("PAYTOKUKYUMUKYUNISSU") = WW_HEADrow("TOKUKYUMUKYUNISSUCHO")                  '特別休暇日数（無給）
                    O_ROW("PAYKOKANGOYUKYUNISSU") = WW_HEADrow("KOKANGOYUKYUNISSUCHO")                  '子看護休暇日数（有給）
                    O_ROW("PAYKOKANGOMUKYUNISSU") = WW_HEADrow("KOKANGOMUKYUNISSUCHO")                  '子看護休暇日数（無給）
                ...
                Else
                    ...
                    '2020/04/13 ADD
                    O_ROW("PAYROSAIYUKYNIUSSU") = 0                                                     '労災欠勤日数（有給）
                    O_ROW("PAYTOKUKYUMUKYUNISSU") = 0                                                   '特別休暇日数（無給）
                    O_ROW("PAYKOKANGOYUKYUNISSU") = 0                                                   '子看護休暇日数（有給）
                    O_ROW("PAYKOKANGOMUKYUNISSU") = 0                                                   '子看護休暇日数（無給）
    ...
    Protected Sub WF_ButtonUPDATE2_Click()
        ...
        '2020/04/13 ADD
        Master.EraseCharToIgnore(WF_ROSAIYUKYNIUSSUTTL.Text)        '労災欠勤日数（有給）
        Master.EraseCharToIgnore(WF_TOKUKYUMUKYUNISSUTTL.Text)      '特別休暇日数（無給）
        Master.EraseCharToIgnore(WF_KOKANGOYUKYUNISSUTTL.Text)      '子看護休暇日数（有給）
        Master.EraseCharToIgnore(WF_KOKANGOMUKYUNISSUTTL.Text)      '子看護休暇日数（無給）
        ...
        For Each T00009INProw As DataRow In T00009INPtbl.Rows
            If T00009INProw("HDKBN") = "H" AndAlso T00009INProw("RECODEKBN") = "2" Then
                ...
                '2020/04/13 ADD
                T00009INProw("ROSAIYUKYNIUSSUCHO") = Val(WF_ROSAIYUKYNIUSSUTTL.Text) - Val(T00009INProw("ROSAIYUKYNIUSSU"))
                T00009INProw("ROSAIYUKYNIUSSUTTL") = Val(T00009INProw("ROSAIYUKYNIUSSU")) + Val(T00009INProw("ROSAIYUKYNIUSSUCHO"))
                T00009INProw("TOKUKYUMUKYUNISSUCHO") = Val(WF_TOKUKYUMUKYUNISSUTTL.Text) - Val(T00009INProw("TOKUKYUMUKYUNISSU"))
                T00009INProw("TOKUKYUMUKYUNISSUTTL") = Val(T00009INProw("TOKUKYUMUKYUNISSU")) + Val(T00009INProw("TOKUKYUMUKYUNISSUCHO"))
                T00009INProw("KOKANGOYUKYUNISSUCHO") = Val(WF_KOKANGOYUKYUNISSUTTL.Text) - Val(T00009INProw("KOKANGOYUKYUNISSU"))
                T00009INProw("KOKANGOYUKYUNISSUTTL") = Val(T00009INProw("KOKANGOYUKYUNISSU")) + Val(T00009INProw("KOKANGOYUKYUNISSUCHO"))
                T00009INProw("KOKANGOMUKYUNISSUCHO") = Val(WF_KOKANGOMUKYUNISSUTTL.Text) - Val(T00009INProw("KOKANGOMUKYUNISSU"))
                T00009INProw("KOKANGOMUKYUNISSUTTL") = Val(T00009INProw("KOKANGOMUKYUNISSU")) + Val(T00009INProw("KOKANGOMUKYUNISSUCHO"))
    ...
    Protected Sub WF_ButtonEND_Click()
        ...
        If WF_DISP.Value = "Adjust" Then
            ...
            WF_DISP.Value = "List"
        Else
            If WF_BEFORE_MAPID.Value = GRT00009WRKINC_V2.MAPIDS Then
                '前画面に戻る
                Master.TransitionPrevPage("", "V2")
    ...
    Protected Sub WF_FILEUPLOAD()
        ...
        '○ 勤怠ALLのみ処理可能
        If Not Master.MAPvariant Like GRT00009WRKINC_V2.VAR_ALL Then
                ...
                For Each T00009row As DataRow In T00009tbl.Rows
                    If T00009row("WORKDATE") = T00009INProw("WORKDATE") AndAlso
                        ...
                        If T00009row("HDKBN") = "H" Then
                            ...
                            '2020/04/13 ADD
                            WW_T00009row("ROSAIYUKYNIUSSUCHO") = Val(T00009INProw("ROSAIYUKYNIUSSUTTL")) - T00009row("ROSAIYUKYNIUSSU")
                            WW_T00009row("ROSAIYUKYNIUSSUTTL") = Val(T00009row("ROSAIYUKYNIUSSU")) + Val(T00009row("ROSAIYUKYNIUSSUCHO"))
                            WW_T00009row("TOKUKYUMUKYUNISSUCHO") = Val(T00009INProw("TOKUKYUMUKYUNISSUTTL")) - T00009row("TOKUKYUMUKYUNISSU")
                            WW_T00009row("TOKUKYUMUKYUNISSUTTL") = Val(T00009row("TOKUKYUMUKYUNISSU")) + Val(T00009row("TOKUKYUMUKYUNISSUCHO"))
                            WW_T00009row("KOKANGOYUKYUNISSUCHO") = Val(T00009INProw("KOKANGOYUKYUNISSUTTL")) - T00009row("KOKANGOYUKYUNISSU")
                            WW_T00009row("KOKANGOYUKYUNISSUTTL") = Val(T00009row("KOKANGOYUKYUNISSU")) + Val(T00009row("KOKANGOYUKYUNISSUCHO"))
                            WW_T00009row("KOKANGOMUKYUNISSUCHO") = Val(T00009INProw("KOKANGOMUKYUNISSUTTL")) - T00009row("KOKANGOMUKYUNISSU")
                            WW_T00009row("KOKANGOMUKYUNISSUTTL") = Val(T00009row("KOKANGOMUKYUNISSU")) + Val(T00009row("KOKANGOMUKYUNISSUCHO"))
    ...
    Protected Sub ExcelInpMake(ByVal I_TABLE As DataTable, ByVal I_HANTEI As String)
        ...
        For Each TBLrow As DataRow In I_TABLE.Rows
            ...
            If WW_COLUMNS.IndexOf("ROSAIYUKYNIUSSUTTL") >= 0 Then
                T00009INProw("ROSAIYUKYNIUSSUTTL") = TBLrow("ROSAIYUKYNIUSSUTTL")
            Else
                T00009INProw("ROSAIYUKYNIUSSUTTL") = "0"
            End If

            '特別休暇日数（無給）合計
            If WW_COLUMNS.IndexOf("TOKUKYUMUKYUNISSUTTL") >= 0 Then
                T00009INProw("TOKUKYUMUKYUNISSUTTL") = TBLrow("TOKUKYUMUKYUNISSUTTL")
            Else
                T00009INProw("TOKUKYUMUKYUNISSUTTL") = "0"
            End If

            '子看護休暇日数（有給）合計
            If WW_COLUMNS.IndexOf("KOKANGOYUKYUNISSUTTL") >= 0 Then
                T00009INProw("KOKANGOYUKYUNISSUTTL") = TBLrow("KOKANGOYUKYUNISSUTTL")
            Else
                T00009INProw("KOKANGOYUKYUNISSUTTL") = "0"
            End If

            '子看護休暇日数（無給）合計
            If WW_COLUMNS.IndexOf("KOKANGOMUKYUNISSUTTL") >= 0 Then
                T00009INProw("KOKANGOMUKYUNISSUTTL") = TBLrow("KOKANGOMUKYUNISSUTTL")
            Else
                T00009INProw("KOKANGOMUKYUNISSUTTL") = "0"
            End If
    ...
    Protected Sub WF_DtabChange()
        ...
        Select Case work.WF_SEL_CAMPCODE.Text
            Case GRT00009WRKINC_V2.CAMP_ENEX       'ENEX
            ...
            Case GRT00009WRKINC_V2.CAMP_NJS        'ニュージェイズ
                WF_HWORKNISSUTTL_L.Visible = False              '休日出勤日非表示
                WF_HWORKNISSUTTL.Visible = False
            ...
            Case GRT00009WRKINC_V2.CAMP_JKT        'JKトランス
    ...
    Protected Sub INPTableCheck(ByRef T00009INProw As DataRow, ByRef O_RTN As String)
        ...
        If T00009INProw("RECODEKBN") = "0" Then
            ...
            Else
                ...
                If isNormal(WW_CS0036FCHECKERR) Then
                    ...
                    If isNormal(WW_RTN_SW) Then
                        '近石のみ勤怠区分特殊チェック
                        If T00009INProw("CAMPCODE") = GRT00009WRKINC_V2.CAMP_KNK Then
            ...
            Else
                ...
                If isNormal(WW_CS0036FCHECKERR) Then
                    ...
                    '5分単位で入力
                    If T00009INProw("CAMPCODE") = GRT00009WRKINC_V2.CAMP_ENEX OrElse
                        T00009INProw("CAMPCODE") = GRT00009WRKINC_V2.CAMP_KNK Then
        ...
        If T00009INProw("RECODEKBN") = "2" Then
            ...
            '2020/04/13 ADD
            '労災欠勤日数（有給）合計
            If String.IsNullOrEmpty(T00009INProw("ROSAIYUKYNIUSSUTTL")) Then
                WW_CheckMES1 = "・更新できないレコード(労災日数無)です。"
                WW_CheckMES2 = ""
                WW_CheckERR(WW_CheckMES1, WW_CheckMES2, T00009INProw)
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            Else
                Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "ROSAIYUKYNIUSSUTTL", T00009INProw("ROSAIYUKYNIUSSUTTL"), WW_CS0036FCHECKERR, WW_CS0036FCHECKREPORT, WW_S0013tbl)
                If isNormal(WW_CS0036FCHECKERR) Then
                    T00009INProw("ROSAIYUKYNIUSSUTTL") = Val(T00009INProw("ROSAIYUKYNIUSSUTTL"))
                Else
            ...
            '特別休暇日数（無給）合計
            If String.IsNullOrEmpty(T00009INProw("TOKUKYUMUKYUNISSUTTL")) Then
                WW_CheckMES1 = "・更新できないレコード(特休無給日数無)です。"
                WW_CheckMES2 = ""
                WW_CheckERR(WW_CheckMES1, WW_CheckMES2, T00009INProw)
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            Else
                Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "TOKUKYUMUKYUNISSUTTL", T00009INProw("TOKUKYUMUKYUNISSUTTL"), WW_CS0036FCHECKERR, WW_CS0036FCHECKREPORT, WW_S0013tbl)
                If isNormal(WW_CS0036FCHECKERR) Then
                    T00009INProw("TOKUKYUMUKYUNISSUTTL") = Val(T00009INProw("TOKUKYUMUKYUNISSUTTL"))
                Else
            ...
            '子看護休暇日数（有給）合計
            If String.IsNullOrEmpty(T00009INProw("KOKANGOYUKYUNISSUTTL")) Then
                WW_CheckMES1 = "・更新できないレコード(子看有給日数無)です。"
                WW_CheckMES2 = ""
                WW_CheckERR(WW_CheckMES1, WW_CheckMES2, T00009INProw)
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            Else
                Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "KOKANGOYUKYUNISSUTTL", T00009INProw("KOKANGOYUKYUNISSUTTL"), WW_CS0036FCHECKERR, WW_CS0036FCHECKREPORT, WW_S0013tbl)
                If isNormal(WW_CS0036FCHECKERR) Then
                    T00009INProw("KOKANGOYUKYUNISSUTTL") = Val(T00009INProw("KOKANGOYUKYUNISSUTTL"))
                Else
            ...
            '子看護休暇日数（無給）合計
            If String.IsNullOrEmpty(T00009INProw("KOKANGOMUKYUNISSUTTL")) Then
                WW_CheckMES1 = "・更新できないレコード(子看無給日数無)です。"
                WW_CheckMES2 = ""
                WW_CheckERR(WW_CheckMES1, WW_CheckMES2, T00009INProw)
                O_RTN = C_MESSAGE_NO.INVALID_REGIST_RECORD_ERROR
            Else
                Master.CheckFieldForTable(work.WF_SEL_CAMPCODE.Text, "KOKANGOMUKYUNISSUTTL", T00009INProw("KOKANGOMUKYUNISSUTTL"), WW_CS0036FCHECKERR, WW_CS0036FCHECKREPORT, WW_S0013tbl)
                If isNormal(WW_CS0036FCHECKERR) Then
                    T00009INProw("KOKANGOMUKYUNISSUTTL") = Val(T00009INProw("KOKANGOMUKYUNISSUTTL"))
                Else

...
Public Class GRT00007WRKINC
    ...
    Public Const MAPIDS_V2 As String = "T00007S_V2"         'MAPID(条件)_V2
    Public Const MAPIDI_V2 As String = "T00007I_V2"         'MAPID(一覧)_V2
    Public Const MAPIDINJS_V2 As String = "T00007INJS_V2"   'MAPID(一覧)_V2
    Public Const MAPIDIKNK_V2 As String = "T00007IKNK_V2"   'MAPID(一覧)_V2
    Public Const MAPIDIJKT_V2 As String = "T00007IJKT_V2"   'MAPID(一覧)_V2
    Public Const MAPID_V2 As String = "T00007_V2"           'MAPID(実行)_V2
    Public Const MAPIDNJS_V2 As String = "T00007NJS_V2"     'MAPID(実行)_V2
    Public Const MAPIDKNK_V2 As String = "T00007KNK_V2"     'MAPID(実行)_V2
    Public Const MAPIDJKT_V2 As String = "T00007JKT_V2"     'MAPID(実行)_V2
    ...
    Public Const CONST_JIKYU = "T0009_JIKYUSHA"             '時給者
    Public Const CONST_ZAN202004 = "2020/04"                '残業計算変更（2020/04から暦日で残業計算する）
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function CreateModelPatternDestParam(ByVal COMPCODE As String, ByVal ORGCODE As String, ByVal CLASSCODE As String, Optional ByVal DESTCODE As String = "") As Hashtable

        Dim prmData As New Hashtable
        Dim destList As New ListBox
        Dim SQLcon = CS0050SESSION.getConnection()
        SQLcon.Open()
        Dim SQLcmd As New SqlCommand()
        Dim SQLdr As SqlDataReader = Nothing
        ...
        '○ リストボックス作成
        Try
            Dim columnName1 As String = String.Empty
            Dim columnName2 As String = String.Empty
            If CLASSCODE = "1" Then
                columnName1 = "TODOKECODE"
                columnName2 = "SHUKABASHO"
            Else
                columnName1 = "SHUKABASHO"
                columnName2 = "TODOKECODE"
            End If


            '検索SQL文
            SQLStr.Append("SELECT                                         ")
            SQLStr.AppendFormat(" isnull(rtrim(m.{0}), '')        as CODE ", columnName1)
            SQLStr.Append("     , isnull(rtrim(t.NAMES), '')      as NAME ")
            SQLStr.Append("FROM   MC012_MODEL m                           ")
            SQLStr.Append("INNER JOIN MC006_TODOKESAKI t                  ")
            SQLStr.Append("        ON t.CAMPCODE   = m.CAMPCODE           ")
            SQLStr.AppendFormat(" AND t.TODOKECODE = m.{0}                ", columnName1)
            SQLStr.Append("       AND t.STYMD     <= @P3                  ")
            SQLStr.Append("       AND t.ENDYMD    >= @P3                  ")
            SQLStr.Append("       AND t.DELFLG    <> '1'                  ")
            SQLStr.Append("WHERE                                          ")
            SQLStr.Append("           m.CAMPCODE   = @P1                  ")
            SQLStr.Append("       AND m.UORG       = @P2                  ")
            SQLStr.Append("       AND m.DELFLG    <> '1'                  ")
            If Not String.IsNullOrEmpty(DESTCODE) Then
                SQLStr.AppendFormat("   AND m.{0}        = @P4                  ", columnName2)
            End If
            SQLStr.AppendFormat("GROUP BY   m.{0}, t.NAMES                ", columnName1)
            SQLStr.Append("      ORDER BY   CODE, NAME                    ")

            SQLcmd = New SqlCommand(SQLStr.ToString, SQLcon)
            PARA(1) = SQLcmd.Parameters.Add("@P1", System.Data.SqlDbType.NVarChar)
            PARA(2) = SQLcmd.Parameters.Add("@P2", System.Data.SqlDbType.NVarChar)
            PARA(3) = SQLcmd.Parameters.Add("@P3", System.Data.SqlDbType.Date)
            PARA(4) = SQLcmd.Parameters.Add("@P4", System.Data.SqlDbType.NVarChar)
            PARA(1).Value = COMPCODE
            PARA(2).Value = ORGCODE
            PARA(3).Value = Date.Now
            PARA(4).Value = DESTCODE
            SQLdr = SQLcmd.ExecuteReader()

            While SQLdr.Read
                destList.Items.Add(New ListItem(SQLdr("NAME"), SQLdr("CODE")))
            End While

            prmData.Item(C_PARAMETERS.LP_LIST) = destList
        ...
        CreateModelPatternDestParam = prmData
    ...
    Public Function CreatePRODUCTParam(ByVal I_COMPCODE As String, ByVal I_HORG As String) As Hashtable
        ...
        Try
            ...
            '検索SQL文
            SQLStr.AppendLine(" SELECT isnull(rtrim(B.OILTYPE),'')  as OILTYPE      ")
            SQLStr.AppendLine("       ,isnull(rtrim(B.PRODUCT1),'') as PRODUCT1     ")
            SQLStr.AppendLine("       ,isnull(rtrim(B.PRODUCT2),'') as PRODUCT2     ")
            SQLStr.AppendLine("       ,isnull(rtrim(B.NAMES),'')    as PRODUCT2NAME ")
            SQLStr.AppendLine("       ,isnull(rtrim(B.STANI),'')    as STANI        ")
            SQLStr.AppendLine(" FROM  MD002_PRODORG A                               ")
            SQLStr.AppendLine(" INNER JOIN MD001_PRODUCT B                          ")
            SQLStr.AppendLine("         ON    B.PRODUCTCODE = A.PRODUCTCODE         ")
            ...
            SQLStr.AppendLine(" GROUP BY B.OILTYPE , B.PRODUCT1 , B.PRODUCT2 , B.NAMES, B.STANI ")
            SQLStr.AppendLine(" ORDER BY B.OILTYPE , B.PRODUCT1 , B.PRODUCT2 , B.NAMES, B.STANI ")

...
Public Class GRT00009SELECT
            ...
            If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                Select Case WF_ButtonClick.Value
                    ...
                    Case "WF_ButtonDO"                  '実行ボタン押下
                        WF_ButtonDO_Click()
                    Case "WF_ButtonDO_V2"               '実行ボタン（新）押下
    ...
    Protected Sub WW_MAPValueSet()
        ...
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.MENU Then             'メニューからの画面遷移
            ...
            work.WF_SEL_XMLsaveTMP.Text = CS0050SESSION.UPLOAD_PATH & "\XML_TMP\" & Date.Now.ToString("yyyyMMdd") & "-" &
                Master.USERID & "-" & Master.MAPID & "-TMP.txt"
            work.WF_SEL_XMLsaveTMP_V2.Text = CS0050SESSION.UPLOAD_PATH & "\XML_TMP\" & Date.Now.ToString("yyyyMMdd") & "-" &
                Master.USERID & "-" & Master.MAPID & "_V2-TMP.txt"
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.T00009 OrElse
               Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.T00009_V2 Then       '実行画面からの遷移
            '画面項目設定処理
        ...
        '○ 再開ボタン活性、非活性判定
        If File.Exists(work.WF_SEL_XMLsaveTMP.Text) OrElse
           File.Exists(work.WF_SEL_XMLsaveTMP_V2.Text) Then
    ...
    Protected Sub WF_ButtonRESTART_Click()
        ...
        If Not Master.MAPpermitcode = C_PERMISSION.INVALID Then
            ...
            End If
            If File.Exists(work.WF_SEL_XMLsaveTMP_V2.Text) Then
    ...
    Protected Sub WF_ButtonDO_Click()
        ...
        '○ 勤怠締テーブル取得
        Dim WW_LIMITFLG As String = "0"
        If Trim(WF_HORG.Text) = "" Then
            'NJSの場合、配属部署（任意）入力のため、ログインユーザーの部署を使う
            T0007COM.T00008get(WF_CAMPCODE.Text, Master.USER_ORG, WF_TAISHOYM.Text, WW_LIMITFLG, WW_RTN_SW)
        Else
            T0007COM.T00008get(WF_CAMPCODE.Text, WF_HORG.Text, WF_TAISHOYM.Text, WW_LIMITFLG, WW_RTN_SW)
        End If
        If Not isNormal(WW_RTN_SW) Then
    ...
    Protected Sub WF_ButtonDO_V2_Click()
        ...
        If Not Master.MAPpermitcode = C_PERMISSION.INVALID Then
            ...
            CS0007CheckAuthority.MAPID = GRT00009WRKINC_V2.MAPID
            ...
            If isNormal(CS0007CheckAuthority.ERR) Then
                ...
                If Master.MAPpermitcode < C_PERMISSION.REFERLANCE Then
                    Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRT00009WRKINC_V2.MAPID)
                    Exit Sub
                End If
            Else
                Master.Output(C_MESSAGE_NO.AUTHORIZATION_ERROR, C_MESSAGE_TYPE.ABORT, "画面:" & GRT00009WRKINC_V2.MAPID)
                    ...
                    Case Else
                        ...
                        Select Case WF_FIELD.Value
                            Case "WF_STAFFCODE"         '従業員(コード)
                                If Trim(WF_HORG.Text) = "" Then
                                    prmData = work.CreateStaffCodeParam(GL0005StaffList.LC_STAFF_TYPE.CLERK,

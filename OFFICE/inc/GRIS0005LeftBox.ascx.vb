...
Public Class GRIS0005LeftBox
    ...
    Public Enum C_PARAMETERS
        ...
        ''' データリロード
        ''' </summary>
        ''' <remarks>TRUE/FALSE(Defalut) </remarks>
        LP_RELOAD
    End Enum
    ...
    Public Function GetActiveValue() As String()
        Select Case WF_LEFTMView.ActiveViewIndex
            ...
            Case 1
                Dim Value As String() = {"", ""}
                Value(0) = CDate(WF_Calendar.Text).ToString("yyyy/MM/dd")
                Value(1) = Value(0)
    ...
    Protected Function CreateCustomerList(ByVal Params As Hashtable, ByRef O_RTN As String) As ListBox
        '○取引先ListBox設定
        Dim key As String = If(Params.Item(C_PARAMETERS.LP_TYPEMODE), GL0003CustomerList.LC_CUSTOMER_TYPE.ALL) _
    ...
    Protected Function CreateDistinationList(ByVal Params As Hashtable, ByRef O_RTN As String) As ListBox
        '○取引先ListBox設定
        Dim key As String = If(Params.Item(C_PARAMETERS.LP_TYPEMODE), GL0004DestinationList.LC_DEST_TYPE.ALL) _
                              & If(Params.Item(C_PARAMETERS.LP_COMPANY), "-") _
        ...
        If Not LbMap.ContainsKey(key) Then
            Using GL0004DestinationList As New GL0004DestinationList With {
                  .TYPE = If(Params.Item(C_PARAMETERS.LP_TYPEMODE), GL0004DestinationList.LC_DEST_TYPE.ALL) _
                , .STYMD = If(Params.Item(C_PARAMETERS.LP_STYMD), Date.Now) _
                , .ENDYMD = If(Params.Item(C_PARAMETERS.LP_ENDYMD), Date.Now) _
                , .DEFAULT_SORT = If(Params.Item(C_PARAMETERS.LP_DEFAULT_SORT), String.Empty) _
    ...
    End Sub
    ''' <summary>
    ''' 内部保存テーブルデータ削除(Hashtable)
    ''' </summary>
    ''' <param name="KeyCode">HashtableKEY</param>
    ''' <param name="Params">パラメータ(LP_RELOAD)のみ参照</param>
    ''' <remarks></remarks>
    Protected Sub CheckReload(ByVal KeyCode As String, ByVal Params As Hashtable)
        '既にHash存在かつRELOAD設定の場合は一旦削除
        If LbMap.ContainsKey(KeyCode) AndAlso If(Params.Item(C_PARAMETERS.LP_RELOAD), False) Then
            LbMap.Remove(KeyCode)
        End If
    End Sub
